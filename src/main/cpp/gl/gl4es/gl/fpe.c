...
void APIENTRY_GL4ES fpe_oldprogram(fpe_state_t* state) {
    ...
    if(state->vertex_prg_id) {
        ...
        if(status!=GL_TRUE) {
            char buff[1000];
            gl4es_gles_glGetShaderInfoLog(glstate->fpe->vert, 1000, NULL, buff);
            if (globals4es.logshader) {
                SHUT_LOGD("MobileGlues-gl4es: FPE ARB Vertex program compile failed: ARB source is\n%s\n=======\nGLSL source is\n%s\nError is: %s\n",
                        old_vtx->string, old_vtx->shader->source, buff);
            }
            else {
                SHUT_LOGD("MobileGlues-gl4es: FPE ARB Vertex program compile failed: %s\n", buff);
            }
        }
        ...
        if(status!=GL_TRUE) {
            char buff[1000];
            gl4es_gles_glGetShaderInfoLog(glstate->fpe->vert, 1000, NULL, buff);
            SHUT_LOGD("MobileGlues-gl4es: FPE ARB Default Vertex program compile failed: %s\n", buff);
        }
    }
    ...
    if(state->fragment_prg_id) {
        ...
        if(status!=GL_TRUE) {
            char buff[1000];
            gl4es_gles_glGetShaderInfoLog(glstate->fpe->frag, 1000, NULL, buff);
            if (globals4es.logshader) {
                SHUT_LOGD("MobileGlues-gl4es: FPE ARB Fragment program compile failed: ARB source is\n%s\n=======\nGLSL source is\n%s\nError is: %s\n",
                        old_frg->string, old_frg->shader->source, buff);
            } else {
                SHUT_LOGD("MobileGlues-gl4es: FPE ARB Fragment program compile failed: %s\n", buff);
            }
        }
        ...
        if(status!=GL_TRUE) {
            char buff[1000];
            gl4es_gles_glGetShaderInfoLog(glstate->fpe->frag, 1000, NULL, buff);
            SHUT_LOGD("MobileGlues-gl4es: FPE ARB Default Fragment program compile failed: %s\n", buff);
        }
    }
    ...
    if(status!=GL_TRUE) {
        char buff[1000];
        gl4es_gles_glGetProgramInfoLog(glstate->fpe->prog, 1000, NULL, buff);
        if (globals4es.logshader) {
            SHUT_LOGD(
                    "MobileGlues-gl4es: FPE ARB Program link failed: %s\n with vertex %s%s%s%s%s and fragment %s%s%s%s%s\n",
                    buff,
                    state->vertex_prg_id ? "custom:\n" : "default",
                    state->vertex_prg_id ? old_vtx->string : "",
                    state->vertex_prg_id ? "\nconverted:\n" : "",
                    state->vertex_prg_id ? old_vtx->shader->source : "",
                    state->vertex_prg_id ? "\n" : "",
                    state->fragment_prg_id ? "custom:\n" : "default",
                    state->fragment_prg_id ? old_frg->string : "",
                    state->fragment_prg_id ? "\nconverted:\n" : "",
                    state->fragment_prg_id ? old_frg->shader->source : "",
                    state->fragment_prg_id ? "\n" : "");
        } else {
            SHUT_LOGD("MobileGlues-gl4es: FPE ARB Program link failed: %s\n", buff);
        }
    }
    DBG(SHUT_LOGD("MobileGlues-gl4es: Created program %d, with vertex=%d (old=%d) fragment=%d (old=%d), alpha=%d/%d\n", glstate->fpe->prog, glstate->fpe->vert, state->vertex_prg_id, glstate->fpe->frag, state->fragment_prg_id, state->alphatest, state->alphafunc);)
}
        ...
        if(fpe_GetProgramPSA(glstate->fpe->prog, &state)==0) {
            ...
            if(state.vertex_prg_id || state.fragment_prg_id) {
                ...
                if(status!=GL_TRUE) {
                    ...
                    gl4es_gles_glGetShaderInfoLog(glstate->fpe->vert, 1000, NULL, buff);
                    if(globals4es.logshader) {
                        SHUT_LOGD("MobileGlues-gl4es: FPE Vertex shader compile failed: source is\n%s\n\nError is: %s\n",
                                fpe_VertexShader(NULL, glstate->fpe_state)[0], buff);
                    } else {
                        SHUT_LOGD("MobileGlues-gl4es: FPE Vertex shader compile failed: %s\n", buff);
                    }
                }
                glstate->fpe->frag = gl4es_glCreateShader(GL_FRAGMENT_SHADER);
                ...
                if(status!=GL_TRUE) {
                    ...
                    if (globals4es.logshader) {
                        SHUT_LOGD(
                                "MobileGlues-gl4es: FPE Fragment shader compile failed: source is\n%s\n\nError is: %s\n",
                                fpe_FragmentShader(NULL, glstate->fpe_state)[0], buff);
                    } else {
                        SHUT_LOGD("MobileGlues-gl4es: FPE Fragment shader compile failed: %s\n",
                                  buff);
                    }
                }
                ...
                if(status!=GL_TRUE) {
                    ...
                    gl4es_gles_glGetProgramInfoLog(glstate->fpe->prog, 1000, NULL, buff);
                    if(globals4es.logshader) {
                        SHUT_LOGD("MobileGlues-gl4es: FPE Program link failed: source of vertex shader is\n%s\n\n", fpe_VertexShader(NULL, glstate->fpe_state)[0]);
                        SHUT_LOGD("MobileGlues-gl4es: source of fragment shader is \n%s\n\nError is: %s\n", fpe_FragmentShader(NULL, glstate->fpe_state)[0], buff);
                    } else
                        SHUT_LOGD("MobileGlues-gl4es: FPE Program link failed: %s\n", buff);
                }
...
program_t* APIENTRY_GL4ES fpe_CustomShader(program_t* glprogram, fpe_state_t* state)
{
    ...
    if(fpe->glprogram==NULL) {
        ...
        if(status!=GL_TRUE) {
            ...
            SHUT_LOGD("MobileGlues-gl4es: FPE Custom Vertex shader compile failed: %s\n", buff);
        ...
        if(status!=GL_TRUE) {
            ...
            SHUT_LOGD("MobileGlues-gl4es: FPE Custom Fragment shader compile failed: %s\n", buff);
            return glprogram;   // fallback to non-customized custom program..
        }
        ...
        // re-run the BindAttribLocation if any
        {
            ...
            kh_foreach_value(glprogram->attribloc, al,
                gl4es_gles_glBindAttribLocation(fpe->prog, al->index, al->name);
            );
        }
        ...
        if(status!=GL_TRUE) {
            ...
            SHUT_LOGD("MobileGlues-gl4es: FPE Custom Program link failed: %s\n", buff);
            return glprogram;   // fallback to non-customized custom program..
        }
        // now find the program
        khint_t k_program;
        ...
        // all done
        DBG(SHUT_LOGD("MobileGlues-gl4es: creating FPE Custom Program : %d(%p)\n", fpe->prog, fpe->glprogram);)
    }
...
program_t* APIENTRY_GL4ES fpe_CustomShader_DefaultVertex(program_t* glprogram, fpe_state_t* state_vertex)
{
    ...
    if(fpe->glprogram==NULL) {
        ...
        if(status!=GL_TRUE) {
            char buff[1000];
            gl4es_glGetShaderInfoLog(fpe->vert, 1000, NULL, buff);
            SHUT_LOGD("MobileGlues-gl4es: FPE Default Vertex shader compile failed: %s\n", buff);
            return glprogram;   // fallback to non-customized custom program..
        }
        fpe->frag = gl4es_glCreateShader(GL_FRAGMENT_SHADER);
        ...
        if(status!=GL_TRUE) {
            char buff[1000];
            gl4es_glGetProgramInfoLog(fpe->prog, 1000, NULL, buff);
            SHUT_LOGD("MobileGlues-gl4es: FPE Custom Program with Default Vertex link failed: %s\n", buff);
            return glprogram;   // fallback to non-customized custom program..
        }
        // now find the program
        khint_t k_program;
        ...
        if(m->parent_size) {
            ...
            switch(m->type) {
                ...
                case GL_FLOAT_MAT4:
                    ...
                    break;
                default:
                    SHUT_LOGD("MobileGlues-gl4es: Warning, sync uniform on father/son program with unknown uniform type %s\n", PrintEnum(m->type));
            }
        }
    );
    DBG(SHUT_LOGD("MobileGlues-gl4es: Uniform sync'd with %d and father (%d uniforms)\n", glprogram->id, cnt);)
}
// ********* Fixed Pipeling function wrapper *********

void APIENTRY_GL4ES fpe_glClientActiveTexture(GLenum texture) {
    DBG(SHUT_LOGD("MobileGlues-gl4es: fpe_glClientActiveTexture(%s)\n", PrintEnum(texture));)
}
...
void APIENTRY_GL4ES fpe_glSecondaryColorPointer(GLint size, GLenum type, GLsizei stride, const GLvoid *pointer) {
    DBG(SHUT_LOGD("MobileGlues-gl4es: fpe_glSecondaryColorPointer(%d, %s, %d, %p)\n", size, PrintEnum(type), stride, pointer);)
...
void APIENTRY_GL4ES fpe_glVertexPointer(GLint size, GLenum type, GLsizei stride, const GLvoid *pointer) {
    DBG(SHUT_LOGD("MobileGlues-gl4es: fpe_glVertexPointer(%d, %s, %d, %p), vertex_buffer=%p\n", size, PrintEnum(type), stride, pointer, glstate->vao->vertex);)
...
void APIENTRY_GL4ES fpe_glColorPointer(GLint size, GLenum type, GLsizei stride, const GLvoid *pointer) {
    DBG(SHUT_LOGD("MobileGlues-gl4es: fpe_glColorPointer(%d, %s, %d, %p)\n", size, PrintEnum(type), stride, pointer);)
...
void APIENTRY_GL4ES fpe_glNormalPointer(GLenum type, GLsizei stride, const GLvoid *pointer) {
    DBG(SHUT_LOGD("MobileGlues-gl4es: fpe_glNormalPointer(%s, %d, %p)\n", PrintEnum(type), stride, pointer);)
...
void APIENTRY_GL4ES fpe_glTexCoordPointerTMU(GLint size, GLenum type, GLsizei stride, const GLvoid *pointer, int TMU) {
    DBG(SHUT_LOGD("MobileGlues-gl4es: fpe_glTexCoordPointer(%d, %s, %d, %p) on tmu=%d\n", size, PrintEnum(type), stride, pointer, TMU);)
...
void APIENTRY_GL4ES fpe_glFogCoordPointer(GLenum type, GLsizei stride, const GLvoid *pointer) {
    DBG(SHUT_LOGD("MobileGlues-gl4es: fpe_glFogPointer(%s, %d, %p)\n", PrintEnum(type), stride, pointer);)
...
void APIENTRY_GL4ES fpe_glDrawArrays(GLenum mode, GLint first, GLsizei count) {
    DBG(SHUT_LOGD("MobileGlues-gl4es: fpe_glDrawArrays(%s, %d, %d), program=%d, instanceID=%u\n", PrintEnum(mode), first, count, glstate->glsl->program, glstate->instanceID);)
    scratch_t scratch = {0};
...
void APIENTRY_GL4ES fpe_glDrawElements(GLenum mode, GLsizei count, GLenum type, const GLvoid *indices) {
    ...
    if(glstate->vao->elements && glstate->vao->elements->real_buffer && indices>=glstate->vao->elements->data && indices<=((void*)((char*)glstate->vao->elements->data+glstate->vao->elements->size))) {
        use_vbo = 1;
        bindBuffer_real(GL_ELEMENT_ARRAY_BUFFER, glstate->vao->elements->real_buffer);
        indices = (GLvoid*)((uintptr_t)indices - (uintptr_t)(glstate->vao->elements->data));
        DBG(SHUT_LOGD("MobileGlues-gl4es: Using VBO %d for indices\n", glstate->vao->elements->real_buffer);)
    }
...
void APIENTRY_GL4ES fpe_glDrawArraysInstanced(GLenum mode, GLint first, GLsizei count, GLsizei primcount) {
    DBG(SHUT_LOGD("MobileGlues-gl4es: fpe_glDrawArraysInstanced(%s, %d, %d, %d), program=%d\n", PrintEnum(mode), first, count, primcount, glstate->glsl->program);)
    LOAD_GLES(glDrawArrays);
    ...
    for (GLint id=0; id<primcount; ++id) {
        ...
        if(glprogram->va_size[i])   // only check used VA...
        {
            ...
            if(w->divisor && w->enabled) {
                ...
                int stride=w->stride;
                if(!stride) stride=GL4ES_gl_sizeof(w->type)*w->size;
                ...
                if(memcmp(glstate->gleshard->vavalue[i], current, 4*sizeof(GLfloat))) {
                    ...
                    gl4es_gles_glVertexAttrib4fv(i, glstate->gleshard->vavalue[i]);
                }
            }
        }
        gl4es_gles_glDrawArrays(mode, first, count);
...
void APIENTRY_GL4ES fpe_glDrawElementsInstanced(GLenum mode, GLsizei count, GLenum type, const GLvoid *indices, GLsizei primcount) {
    DBG(SHUT_LOGD("MobileGlues-gl4es: fpe_glDrawElementsInstanced(%s, %d, %s, %p, %d), program=%d\n", PrintEnum(mode), count, PrintEnum(type), indices, primcount, glstate->glsl->program);)
    LOAD_GLES(glDrawElements);
    LOAD_GLES2(glVertexAttrib4fv);
    scratch_t scratch = {0};
    realize_glenv(mode==GL_POINTS, 0, count, type, indices, &scratch);
    ...
    if(glstate->vao->elements && glstate->vao->elements->real_buffer && indices>=glstate->vao->elements->data && indices<=((void*)((char*)glstate->vao->elements->data+glstate->vao->elements->size))) {
        ...
        inds = (void*)indices;
        bindBuffer_real(GL_ELEMENT_ARRAY_BUFFER, 0);
    }
    //realize_bufferIndex_real();    // not useful here
    for (GLint id=0; id<primcount; ++id) {
        GoUniformiv(glprogram, glprogram->builtin_instanceID, 1, 1, &id);
        for(int i=0; i<hardext.maxvattrib; i++) 
        if(glprogram->va_size[i])   // only check used VA...
        ...
        gl4es_gles_glDrawElements(mode, count, type, inds);
    }
    if(use_vbo)
        wantBufferIndex(0);
    free_scratch(&scratch);
...
void realize_glenv(int ispoint, int first, int count, GLenum type, const void* indices, scratch_t* scratch) {
    ...
    if(glstate->glsl->program) {
        ...
        if(glstate->gleshard->program != glstate->fpe->prog)
        {
            ...
            gl4es_gles_glUseProgram(glstate->gleshard->program);
            DBG(SHUT_LOGD("MobileGlues-gl4es: Use FPE program %d\n", glstate->gleshard->program);)
        }
    }
    program_t *glprogram = glstate->gleshard->glprogram;
    ...
    if(globals4es.fbounbind && glstate->fbo.current_fb->id) {
        ...
        while(tu_idx<MAX_TEX && glprogram->texunits[tu_idx].type && !need) {
            ...
            if(tex && tex->binded_fbo==glstate->fbo.current_fb->id) {
                DBG(SHUT_LOGD("MobileGlues-gl4es: Texture %d is used on Uniform %s (%d) and binded on FBO %d Attachement=%s\n", tex->glname, GetUniformName(glprogram, glprogram->texunits[tu_idx].id), glprogram->texunits[tu_idx].id, glstate->fbo.current_fb->id, PrintEnum(tex->binded_attachment));)
                need = 1;
            }
            ++tu_idx;
        }
        if(need && tex) {
            DBG(SHUT_LOGD("MobileGlues-gl4es: Need to Bind/Unbind FBO!");)
            LOAD_GLES2_OR_OES(glBindFramebuffer);
            LOAD_GLES2_OR_OES(glFramebufferTexture2D);
            //gl4es_gles_glFramebufferTexture2D(GL_FRAMEBUFFER, tex->binded_attachment, GL_TEXTURE_2D, 0, 0);
            gl4es_gles_glBindFramebuffer(GL_FRAMEBUFFER, 0);
            gl4es_gles_glBindFramebuffer(GL_FRAMEBUFFER, glstate->fbo.current_fb->id);
            //gl4es_gles_glFramebufferTexture2D(GL_FRAMEBUFFER, tex->binded_attachment, GL_TEXTURE_2D, tex->glname, 0);
        }
    }
    ...
    if(glprogram->va_size[i])   // only check used VA...
    {
        ...
        if(enabled && !w->buffer && !w->pointer) {
            DBG(SHUT_LOGD("MobileGlues-gl4es: Warning: VA %d Enabled with buffer:0 and NULL pointer, disabling\n", i));
            enabled = 0;
        }
        ...
        if(v->enabled != enabled || (v->enabled && w->divisor)) {
            ...
            DBG(SHUT_LOGD("MobileGlues-gl4es: VertexAttribArray[%d]:%s, divisor=%d\n", i, (enabled)?"Enable":"Disable", w->divisor);)
            if(v->enabled)
                gl4es_gles_glEnableVertexAttribArray(i);
            else
                gl4es_gles_glDisableVertexAttribArray(i);
        }
            ...
            if(dirty || v->size!=w->size || v->type!=w->type || v->normalized!=w->normalized 
                ...
                if((w->size==GL_BGRA || w->type==GL_DOUBLE) && scratch->size<8) { 
                    ...
                    if(w->size==GL_BGRA) {
                        ...
                        static int warn = 1;
                        if(warn) {
                            SHUT_LOGD("MobileGlues-gl4es: VertexAttribArray using GL_DOUBLE unimplemented!\n");
                            warn=0;
                        }
                    }
                } else {
                ...
                DBG(SHUT_LOGD("MobileGlues-gl4es: using Buffer %d\n", v->real_buffer);)
                bindBuffer_real(GL_ARRAY_BUFFER, v->real_buffer);

                if (v->integer) {
                    if(gl4es_gles_glVertexAttribIPointer)
                        gl4es_gles_glVertexAttribIPointer(i, v->size, v->type, v->stride, v->pointer);
                    else
                        gl4es_gles_glVertexAttribPointer(i, v->size, v->type, 0, v->stride, v->pointer);
                    DBG(SHUT_LOGD("MobileGlues-gl4es: glVertexAttribIPointer(%d, %d, %s, %d, %p)\n", i, v->size, PrintEnum(v->type), v->stride, v->pointer);)
                } else {
                    gl4es_gles_glVertexAttribPointer(i, v->size, v->type, v->normalized, v->stride, v->pointer);
                    DBG(SHUT_LOGD("MobileGlues-gl4es: glVertexAttribPointer(%d, %d, %s, %d, %d, %p)\n", i, v->size, PrintEnum(v->type), v->normalized, v->stride, v->pointer);)
                }
            }
        } else {
            // single value case
        ...
        if(v->enabled) {
            v->enabled = 0;
            DBG(SHUT_LOGD("MobileGlues-gl4es: VertexAttribArray[%d]:%s\n", i, "Disable");)
            gl4es_gles_glDisableVertexAttribArray(i);
        }
    }
}
...
void realize_blitenv(int alpha) {
    DBG(SHUT_LOGD("MobileGlues-gl4es: realize_blitenv(%d)\n", alpha);)
    LOAD_GLES2(glUseProgram);

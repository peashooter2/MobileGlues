...
void APIENTRY_GL4ES gl4es_glLineStipple(GLuint factor, GLushort pattern) {
    DBG(SHUT_LOGD("MobileGlues-gl4es: glLineStipple(%d, 0x%04X)\n", factor, pattern);)
...
GLfloat *gen_stipple_tex_coords(GLfloat *vert, GLushort *sindices, modeinit_t *modes, int stride, int length, GLfloat* noalloctex) {
    DBG(SHUT_LOGD("MobileGlues-gl4es: Generate stripple tex (stride=%d, noalloctex=%p) length=%d:", stride, noalloctex, length);)
    ...
    for (int k=0; k<length; k++) {
        ...
        DBG(SHUT_LOGD("MobileGlues-gl4es: [%s->%d] ", PrintEnum(mode), modes[k].ilen);)
        oldlen = len = 0.f;
        if(modes[k].ilen<2)
            continue;
        if(mode==GL_LINES || length>1)  // always line when multiple lines were merged
            for (; i < modes[k].ilen; i+=2) {
                ...
                DBG(SHUT_LOGD("MobileGlues-gl4es: %f->%f (%f,%f -> %f,%f)\t", oldlen, len, x1, y1, x2, y2);)
                if(sindices)
            ...
            x2=(v[0]/v[3])*w; y2=(v[1]/v[3])*h;
            vertPos+=stride;
            DBG(SHUT_LOGD("MobileGlues-gl4es: %f\t", len);)
            memset(texPos, 0, 4*sizeof(GLfloat));
            texPos[0] = len; texPos[3] = 1.0f;
            texPos+=texstride;
            ++i;

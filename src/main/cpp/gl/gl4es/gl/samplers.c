...
{
    DBG(SHUT_LOGD("MobileGlues-gl4es: glGenSamplers(%i, %p)\n", n, ids);)
    ...
    if(n-m) {
        errorGL();
        gl4es_gles_glGenFramebuffers(n-m, ids+m);
    }*/
    FLUSH_BEGINEND;
	noerrorShim();
    if (n<1) {
...
{
    DBG(SHUT_LOGD("MobileGlues-gl4es: gl4es_glBindSample(%u, %u)\n", unit, sampler);)
...
{
    DBG(SHUT_LOGD("MobileGlues-gl4es: glDeleteSamplers(%i, %p)\n", n, samplers);)
    if(n<0) {
        errorShim(GL_INVALID_VALUE);
        return;
    }
...
{
    DBG(SHUT_LOGD("MobileGlues-gl4es: glIsSampler(%u)\n", id);)
...
{
    DBG(SHUT_LOGD("MobileGlues-gl4es: samplerParameterfv(%p(%d), %s, %p)\n", sampler, sampler->glname, PrintEnum(pname), params);)
    switch(pname) {
        case GL_TEXTURE_MIN_FILTER:

...
static const GL4ES_colorlayout_t *get_color_map(GLenum format) {
    #define map(fmt, ...)                               \
        case fmt: {                                     \
        static GL4ES_colorlayout_t layout = {fmt, __VA_ARGS__}; \
        return &layout; }
    switch (format) {
        ...
        default:
            SHUT_LOGD("MobileGlues-gl4es: LIBGL: unknown pixel format %s\n", PrintEnum(format));
            break;
    }
    static GL4ES_colorlayout_t null = {0};
    return &null;
    #undef map
}
...
bool remap_pixel(const GLvoid *src, GLvoid *dst,
                 const GL4ES_colorlayout_t *src_color, GLenum src_type,
                 const GL4ES_colorlayout_t *dst_color, GLenum dst_type) {
    ...
    switch (src_type) {
        ...
        default:
            // TODO: add glSetError?
            SHUT_LOGD("MobileGlues-gl4es: LIBGL: Unsupported source data type: %s\n", PrintEnum(src_type));
            return false;
            break;
    }
    ...
    switch (dst_type) {
        ...
        default:
            SHUT_LOGD("MobileGlues-gl4es: LIBGL: Unsupported target data type: %s\n", PrintEnum(dst_type));
            return false;
            break;
    }
    return true;
...
bool transform_pixel(const GLvoid *src, GLvoid *dst,
    ...
    switch (src_type) {
        ...
        default:
            // TODO: add glSetError?
            SHUT_LOGD("MobileGlues-gl4es: LIBGL: transform_pixel: Unsupported source data type: %s\n", PrintEnum(src_type));
            return false;
            break;
    }
...
bool half_pixel(const GLvoid *src0, const GLvoid *src1,
    ...
    switch (src_type) {
        ...
        default:
            // TODO: add glSetError?
            SHUT_LOGD("MobileGlues-gl4es: LIBGL: half_pixel: Unsupported source data type: %s\n", PrintEnum(src_type));
            return false;
            break;
    }
    ...
    switch (src_type) {
       ...
       // TODO: force 565 to RGB? then we can change [4] -> 3
        ...
        default:
            SHUT_LOGD("MobileGlues-gl4es: LIBGL: half_pixel: Unsupported target data type: %s\n", PrintEnum(src_type));
            return false;
            break;
    }
    return true;
...
bool quarter_pixel(const GLvoid *src[16],
    ...
    switch (src_type) {
        ...
        default:
            // TODO: add glSetError?
            SHUT_LOGD("MobileGlues-gl4es: LIBGL: quarter_pixel: Unsupported source data type: %s\n", PrintEnum(src_type));
            return false;
            break;
    }
    ...
    switch (src_type) {
       ...
       // TODO: force 565 to RGB? then we can change [4] -> 3
        ...
        default:
            SHUT_LOGD("MobileGlues-gl4es: LIBGL: quarter_pixel Unsupported target data type: %s\n", PrintEnum(src_type));
            return false;
            break;
    }
    return true;
...
bool GL4ES_pixel_convert(const GLvoid *src, GLvoid **dst,
    ...
    const GL4ES_colorlayout_t *src_color, *dst_color;
    ...
    GLuint dst_size = height * widthalign(width * GL4ES_pixel_sizeof(dst_format, dst_type), align);
    GLuint dst_width2 = widthalign((stride?stride:width) * GL4ES_pixel_sizeof(dst_format, dst_type), align);
    GLuint dst_width = dst_width2 - (width * GL4ES_pixel_sizeof(dst_format, dst_type));
    GLuint src_width = widthalign(width * GL4ES_pixel_sizeof(src_format, src_type), align);
    GLuint src_widthadj = src_width -(width * GL4ES_pixel_sizeof(src_format, src_type));

    //SHUT_LOGD("MobileGlues-gl4es: pixel conversion: %ix%i - %s, %s (%d) ==> %s, %s (%d), transform=%i, align=%d, src_width=%d(%d), dst_width=%d(%d)\n", width, height, PrintEnum(src_format), PrintEnum(src_type),GL4ES_pixel_sizeof(src_format, src_type), PrintEnum(dst_format), PrintEnum(dst_type), GL4ES_pixel_sizeof(dst_format, dst_type), raster_need_transform(), align, src_width, src_widthadj, dst_width2, dst_width);
    ...
    if ((src_type == dst_type) && (dst_format == src_format)) {
        if (*dst == src)
            return true;
        if (!dst_size || !GL4ES_pixel_sizeof(src_format, src_type)) {
            LOGE("MobileGlues-gl4es: pixel conversion, unknown format size, anticipated abort\n");
            return false;
        }
    ...
    if (!dst_size || !GL4ES_pixel_sizeof(src_format, src_type)
        ...
        LOGE("MobileGlues-gl4es: pixel conversion, anticipated abort\n");
        return false;
    }
    GLsizei src_stride = GL4ES_pixel_sizeof(src_format, src_type);
    GLsizei dst_stride = GL4ES_pixel_sizeof(dst_format, dst_type);
    if (*dst == src || *dst == NULL)
        *dst = malloc(dst_size);
...
{
    const GL4ES_colorlayout_t *src_color;
    GLuint pixels = width * height;
    GLuint dst_size = pixels * GL4ES_pixel_sizeof(src_format, src_type);
    src_color = get_color_map(src_format);
    GLsizei src_stride = GL4ES_pixel_sizeof(src_format, src_type);
    if (*dst == src || *dst == NULL)
        *dst = malloc(dst_size);
...
bool GL4ES_pixel_scale(const GLvoid *old, GLvoid **new,
                 GLuint width, GLuint height,
    ...
    //SHUT_LOGD("MobileGlues-gl4es: scaling %ux%u -> %ux%u (%f/%f)\n", width, height, new_width, new_height, ratiox, ratioy);
    GLvoid *dst;
    uintptr_t src, pos, pixel;

    pixel_size = GL4ES_pixel_sizeof(format, type);
...
bool GL4ES_pixel_halfscale(const GLvoid *old, GLvoid **new,
                 GLuint width, GLuint height,
                 GLenum format, GLenum type) {
    if(!old) {
        *new = NULL;
...
/*    if (new_width*2!=width || new_height*2!=height) {
        SHUT_LOGD("MobileGlues-gl4es: LIBGL: halfscaling %ux%u failed\n", width, height);
        return false;
    }*/
    //SHUT_LOGD("MobileGlues-gl4es: LIBGL: halfscaling %ux%u -> %ux%u (%s / %s)\n", width, height, new_width, new_height, PrintEnum(format), PrintEnum(type));
    const GL4ES_colorlayout_t *src_color;
    ...
    if(!src_color->type) {
        if(!pixel_size) {
            SHUT_LOGD("MobileGlues-gl4es: LIBGL: Cannot halfscale unknown format/type %s/%s\n", PrintEnum(format), PrintEnum(type));
            free(dst);
            return 0;
        }
        for (int y = 0; y < new_height; y++) {
...
bool GL4ES_pixel_thirdscale(const GLvoid *old, GLvoid **new,
                 GLuint width, GLuint height,
                 GLenum format, GLenum type) {
    ...
    if (new_width*2!=width || new_height*2!=height || format!=GL_RGBA || type!=GL_UNSIGNED_BYTE) {
        //SHUT_LOGD("MobileGlues-gl4es: LIBGL: thirdscaling %ux%u failed\n", width, height);
        return false;
    }
//    SHUT_LOGD("MobileGlues-gl4es: LIBGL: halfscaling %ux%u -> %ux%u\n", width, height, new_width, new_height);
    const GL4ES_colorlayout_t *src_color;
    src_color = get_color_map(format);
    GLvoid *dst;
    uintptr_t src, pos, pix0, pix1, pix2, pix3;
    ...
    pixel_size = GL4ES_pixel_sizeof(format, type);
    dest_size = GL4ES_pixel_sizeof(format, GL_UNSIGNED_SHORT_4_4_4_4);
...
bool GL4ES_pixel_quarterscale(const GLvoid *old, GLvoid **new,
                 GLuint width, GLuint height,
                 GLenum format, GLenum type) {
    GLuint pixel_size, new_width, new_height;
...
/*    if (new_width*4!=width || new_height*4!=height) {
        SHUT_LOGD("MobileGlues-gl4es: LIBGL: quarterscaling %ux%u failed\n", width, height);
        return false;
    }*/
//    SHUT_LOGD("MobileGlues-gl4es: LIBGL: quarterscaling %ux%u -> %ux%u\n", width, height, new_width, new_height);
    const GL4ES_colorlayout_t *src_color;
    src_color = get_color_map(format);
    GLvoid *dst;
    ...
    if(!src_color->type) {
        if(!pixel_size) {
            SHUT_LOGD("MobileGlues-gl4es: LIBGL: Cannot quarterscale unknown format/type %s/%s\n", PrintEnum(format), PrintEnum(type));
            free(dst);
            return 0;
        }
        for (int y = 0; y < new_height; y++) {
...
bool GL4ES_pixel_doublescale(const GLvoid *old, GLvoid **new,
                 GLuint width, GLuint height,
                 GLenum format, GLenum type) {
    if(!old) {
        *new = NULL;
    ...
    new_width = width * 2;
    new_height = height * 2;
    //SHUT_LOGD("MobileGlues-gl4es: LIBGL: doublescaling %ux%u -> %ux%u (%s / %s)\n", width, height, new_width, new_height, PrintEnum(format), PrintEnum(type));
    const GL4ES_colorlayout_t *src_color;
    src_color = get_color_map(format);
    GLvoid *dst;
...
bool GL4ES_pixel_to_ppm(const GLvoid *pixels, GLuint width, GLuint height,
    ...
    if (format == GL_RGB && type == GL_UNSIGNED_BYTE) {
        ...
        if (! GL4ES_pixel_convert(pixels, (GLvoid **)&src, width, height, format, type, GL_RGB, GL_UNSIGNED_BYTE, 0, align)) {
            return false;
        }
    }

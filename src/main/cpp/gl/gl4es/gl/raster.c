...
void APIENTRY_GL4ES gl4es_glViewport(GLint x, GLint y, GLsizei width, GLsizei height) {
	if (glstate->fbo.current_fb->id != 0) {
    	gltexture_t *tex = gl4es_getTexture(glstate->fbo.current_fb->t_color[0], glstate->fbo.current_fb->color[0],0,0);
...
//SHUT_LOGD("MobileGlues-gl4es: LIBGL: glPixelZoom(%f, %f)\n", xfactor, yfactor);
}
...
//SHUT_LOGD("MobileGlues-gl4es: LIBGL: glPixelTransferf(%04x, %f)\n", pname, param);
    switch(pname) {
	case GL_RED_SCALE:
	...
	case GL_MAP_COLOR:
		...
		break;
	/*default:
		SHUT_LOGD("MobileGlues-gl4es: LIBGL: stubbed glPixelTransferf(%04x, %f)\n", pname, param);*/
	// the other...
    }
}
...
void bitmap_flush() {
    DBG(LOGD("MobileGlues-gl4es: bitmap_flush");)
...
/*SHUT_LOGD("MobileGlues-gl4es: glBitmap, xy={%f, %f}, xyorig={%f, %f}, size={%u, %u}, zoom={%f, %f}, viewport={%i, %i, %i, %i}\n", 	
...
/*SHUT_LOGD("MobileGlues-gl4es: glDrawPixels, xy={%f, %f}, size={%i, %i}, format=%s, type=%s, zoom={%f, %f}, viewport={%i, %i, %i, %i}\n", 	
    ...
    if (! GL4ES_pixel_convert(data, &dst, bmp_width, height,

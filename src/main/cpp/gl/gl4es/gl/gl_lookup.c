...
void* APIENTRY_GL4ES gl4es_GetProcAddress(const char *name) {
    DBG(SHUT_LOGD("MobileGlues-gl4es: glGetProcAddress(\"%s\")", name);)
    ...
    DBG(SHUT_LOGD("MobileGlues-gl4es: NULL\n");)
    if (!globals4es.silentstub) LOGD("MobileGlues-gl4es: GL4ES GetProcAddress: %s not found.\n", name);
    return NULL;
}

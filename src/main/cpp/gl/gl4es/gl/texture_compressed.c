                            ...
                            GLsizei imageSize, const GLvoid *data) 
{
    ...
    DBG(SHUT_LOGD("MobileGlues-gl4es: glCompressedTexImage2D on target=%s:%p, level=%d with size(%i,%i), internalformat=%s, imagesize=%i, upackbuffer=%p data=%p\n", PrintEnum(target), bound, level, width, height, PrintEnum(internalformat), imageSize, glstate->vao->unpack?glstate->vao->unpack->data:0, data);)
    // hack...
    ...
    if (isDXTc(internalformat)) {
        ...
        if (datab) {
            ...
            if(srgb)
                GL4ES_pixel_srgb_inplace(pixels, width, height);
            ...
            if(!globals4es.nodownsampling && !globals4es.avoid16bits) {
                ...
                if (!GL4ES_pixel_convert(pixels, &half, width, height, GL_RGBA, GL_UNSIGNED_BYTE, format, type, 0, glstate->texture.unpack_align)) {
                    format = GL_RGBA;
                    type = GL_UNSIGNED_BYTE;
                }
            }
        ...
        if(level) {
            ...
            while(nww!=1 || nhh!=1) {
                ...
                if(half) {  // half can be null if no data...
                    GL4ES_pixel_halfscale(ndata, &out, nww, nhh, GL_RGBA, GL_UNSIGNED_BYTE);
                ...
                nhh = nlevel(nhh, 1);
                if(half)
                    GL4ES_pixel_convert(ndata, &out, nww, nhh, GL_RGBA, GL_UNSIGNED_BYTE, format, type, 0, 1);
                ++leveln;
        ...
        {} //STUB of gl4es_gles_glCompressedTexImage2D(rtarget, level, internalformat, width, height, border, imageSize, datab);
        errorGL();
    }
                               ...
                               GLsizei imageSize, const GLvoid *data) 
{
    ...
    if (isDXTc(format)) {
        ...
        GLvoid *half=pixels;
        #if 0
        GL4ES_pixel_thirdscale(pixels, &half, width, height, GL_RGBA, GL_UNSIGNED_BYTE);
        int oldalign;
        gl4es_glGetIntegerv(GL_UNPACK_ALIGNMENT, &oldalign);
        ...
        {} //STUB of gl4es_gles_glCompressedTexSubImage2D(target, level, xoffset, yoffset, width, height, format, imageSize, datab);
    }
}

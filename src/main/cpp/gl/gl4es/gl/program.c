...
void APIENTRY_GL4ES gl4es_glAttachShader(GLuint program, GLuint shader) {
    DBG(SHUT_LOGD("MobileGlues-gl4es: glAttachShader(%d, %d)\n", program, shader);)
    FLUSH_BEGINEND;
    // sanity tests
...
void APIENTRY_GL4ES gl4es_glBindAttribLocation(GLuint program, GLuint index, const GLchar *name) {
    DBG(SHUT_LOGD("MobileGlues-gl4es: glBindAttribLocation(%d, %d, \"%s\")\n", program, index, name);)
    ...
    if(gl4es_gles_glBindAttribLocation) {
        if(strncmp(name, "_gl4es_", 7)==0) {
            gl4es_gles_glBindAttribLocation(glprogram->id, index, attribloc->name);
        }
        {} //STUB of gl4es_gles_glBindAttribLocation(glprogram->id, index, attribloc->name);
        errorGL();
    } else
        noerrorShim();
}
...
GLuint APIENTRY_GL4ES gl4es_glCreateProgram(void) {
    DBG(SHUT_LOGD("MobileGlues-gl4es: glCreateProgram()\n");)
    FLUSH_BEGINEND;
    static GLuint lastprogram = 0;
    GLuint program;
    // create the program
    LOAD_GLES2(glCreateProgram);
    if(gl4es_gles_glCreateProgram) {
        program = gl4es_gles_glCreateProgram();
        if(!program) {
            errorGL();
            return 0;
        }
...
void APIENTRY_GL4ES gl4es_glDeleteProgram(GLuint program) {
    DBG(SHUT_LOGD("MobileGlues-gl4es: glDeleteProgram(%d)\n", program);)
    if(!glstate) return;    // in case a program delete shaders after deleteing all context
    FLUSH_BEGINEND;
    CHECK_PROGRAM(void, program)
    // send to hardware
    LOAD_GLES2(glDeleteProgram);
    if(gl4es_gles_glDeleteProgram) {
        {} //STUB of gl4es_gles_glDeleteProgram(glprogram->id);
        errorGL();
    } else
        noerrorShim();
...
void APIENTRY_GL4ES gl4es_glDetachShader(GLuint program, GLuint shader) {
    DBG(SHUT_LOGD("MobileGlues-gl4es: glDetachShader(%d, %d)\n", program, shader);)
    FLUSH_BEGINEND;
    CHECK_PROGRAM(void, program)
    CHECK_SHADER(void, shader)
    // is shader attached?
    ...
    if(gl4es_gles_glDetachShader) {
        {} //STUB of gl4es_gles_glDetachShader(glprogram->id, glshader->id);
        errorGL();
    } else
        noerrorShim();
    // marked as detached
...
void APIENTRY_GL4ES gl4es_glGetActiveAttrib(GLuint program, GLuint index, GLsizei bufSize, GLsizei *length, GLint *size, GLenum *type, GLchar *name) {
    DBG(SHUT_LOGD("MobileGlues-gl4es: glGetActiveAttrib(%d, %d, %d, %p, %p, %p, %p)\n", program, index, bufSize, length, size, type, name);)
    FLUSH_BEGINEND;
    CHECK_PROGRAM(void, program)
...
GLint APIENTRY_GL4ES gl4es_glGetAttribLocation(GLuint program, const GLchar *name) {
    DBG(SHUT_LOGD("MobileGlues-gl4es: glGetAttribLocation(%d, %s)\n", program, name));
    FLUSH_BEGINEND;
    CHECK_PROGRAM(GLint, program);

    if(!glprogram->linked) {
        errorShim(GL_INVALID_OPERATION);
        DBG(SHUT_LOGD("MobileGlues-gl4es:  program unlinked: -1\n"));
        return -1;
    }
    noerrorShim();
    if(strncmp(name, "gl_", 3)==0) {
        DBG(SHUT_LOGD("MobileGlues-gl4es:  internal attrib: -1\n"));
        return -1;
    }
    ...
    if(loc!=-1) {
        DBG(SHUT_LOGD("MobileGlues-gl4es:  found in cache: %d\n", loc);)
        return loc;
    }
    // end
    DBG(SHUT_LOGD("MobileGlues-gl4es:  asked hardware: %d\n", loc);)
    return loc;
}
...
void APIENTRY_GL4ES gl4es_glGetActiveUniform(GLuint program, GLuint index, GLsizei bufSize, GLsizei *length, GLint *size, GLenum *type, GLchar *name) {
    DBG(SHUT_LOGD("MobileGlues-gl4es: glGetActiveUniform(%d, %d, %d, %p, %p, %p, %p)\n", program, index, bufSize, length, size, type, name);)
    FLUSH_BEGINEND;
    CHECK_PROGRAM(GLvoid, program);

    if(!glprogram->linked) {
        errorShim(GL_INVALID_OPERATION);
        DBG(SHUT_LOGD("MobileGlues-gl4es:  not linked\n");)
        return;
    }
    noerrorShim();
    if(strncmp(name, "gl_", 3)==0) {
        DBG(SHUT_LOGD("MobileGlues-gl4es:  internal uniform\n");)
        return;
    }
    ...
    if(glprogram->uniform) {
        ...
        kh_foreach_value(glprogram->uniform, m,
            if(m->internal_id == index) {
                ...
                DBG(SHUT_LOGD("MobileGlues-gl4es:  found %s (%zd), type=%s, size=%d\n", m->name, strlen(m->name), PrintEnum(m->type), m->size);)
                return;
            }
        );
    }
...
void APIENTRY_GL4ES gl4es_glGetProgramInfoLog(GLuint program, GLsizei maxLength, GLsizei *length, GLchar *infoLog) {
    DBG(SHUT_LOGD("MobileGlues-gl4es: glGetProgramInfoLog(%d, %d, %p, %p)\n", program, maxLength, length, infoLog);)
    FLUSH_BEGINEND;
    CHECK_PROGRAM(void, program)
    ...
    if(gl4es_gles_glGetProgramInfoLog) {
      gl4es_gles_glGetProgramInfoLog(glprogram->id, maxLength, length, infoLog);
        errorGL();
    } else {
...
void APIENTRY_GL4ES gl4es_glGetProgramiv(GLuint program, GLenum pname, GLint *params) {
    DBG(SHUT_LOGD("MobileGlues-gl4es: glGetProgramiv(%d, %s, %p)\n", program, PrintEnum(pname), params);)
    FLUSH_BEGINEND;
    CHECK_PROGRAM(void, program)
    ...
    switch(pname) {
        case GL_DELETE_STATUS:
            if(gl4es_gles_glGetProgramiv) {
              gl4es_gles_glGetProgramiv(glprogram->id, pname, params);
                errorGL();
            } else
                *params = GL_FALSE;
            break;
...
GLint APIENTRY_GL4ES gl4es_glGetUniformLocation(GLuint program, const GLchar *name) {
    DBG(SHUT_LOGD("MobileGlues-gl4es: glGetUniformLocation(%d, %s)\n", program, name);)
    FLUSH_BEGINEND;
    CHECK_PROGRAM(GLint, program)
    ...
    int res = -1;
    if(strncmp(name, "gl_", 3)==0) {
        DBG(SHUT_LOGD("MobileGlues-gl4es:  internal uniform: -1\n");)
        return res;
    }
    ...
    DBG(SHUT_LOGD("MobileGlues-gl4es:  location: %d\n", res);)
    return res;
}

GLboolean APIENTRY_GL4ES gl4es_glIsProgram(GLuint program) {
    DBG(SHUT_LOGD("MobileGlues-gl4es: glIsProgram(%d)\n", program);)
    FLUSH_BEGINEND;
    noerrorShim();
    if(!program) {
        return GL_FALSE;
...
static void fill_program(program_t *glprogram)
{
    ...
    // Grab all Uniform
  gl4es_gles_glGetProgramiv(glprogram->id, GL_ACTIVE_UNIFORMS, &n);
  gl4es_gles_glGetProgramiv(glprogram->id, GL_ACTIVE_UNIFORM_MAX_LENGTH, &maxsize);
    ...
    int tu_idx = 0;
    for (int i=0; i<n; i++) {
      gl4es_gles_glGetActiveUniform(glprogram->id, i, maxsize, NULL, &size, &type, name);
        DBG(e2=gl4es_gles_glGetError();)
        DBG(if(e2==GL_NO_ERROR))
        {
            // remove any ending "[]" that could be present
            if(name[strlen(name)-1]==']' && strrchr(name, '[')) (*strrchr(name, '['))='\0';
            GLint id = gl4es_gles_glGetUniformLocation(glprogram->id, name);
            if(id!=-1) {
        ...
        DBG(else SHUT_LOGD("MobileGlues-gl4es: LIBGL: Warning, getting Uniform #%d info failed with %s\n", i, PrintEnum(e2));)
    }
    free(name);
    // reset uniform cache
    ...
    // Grab all Attrib
  gl4es_gles_glGetProgramiv(glprogram->id, GL_ACTIVE_ATTRIBUTES, &n);
  gl4es_gles_glGetProgramiv(glprogram->id, GL_ACTIVE_ATTRIBUTE_MAX_LENGTH, &maxsize);
    name = (char*)malloc(maxsize);
    for (int i=0; i<n; i++) {
        DBG(e2=gl4es_gles_glGetError();)
        DBG(if(e2==GL_NO_ERROR))
        {
          gl4es_gles_glGetActiveAttrib(glprogram->id, i, maxsize, NULL, &size, &type, name);
            GLint id = gl4es_gles_glGetAttribLocation(glprogram->id, name);
            if(id!=-1) {
                ...
                DBG(SHUT_LOGD("MobileGlues-gl4es:  attrib #%d : \"%s\"%s type=%s size=%d\n", id, glattribloc->name, builtin?" (builtin) ":"", PrintEnum(glattribloc->type), glattribloc->size);)
            }
        }
        DBG(else SHUT_LOGD("MobileGlues-gl4es: LIBGL: Warning, getting Attrib #%d info failed with %s\n", i, PrintEnum(e2));)
    }
    free(name);
}
...
int gl4es_useProgramBinary(GLuint program, int length, GLenum format, const void* binary)
{
    if(hardext.prgbin_n==0)
        return 0;
    DBG(SHUT_LOGD("MobileGlues-gl4es: useProgramBinary(%d, %d, %d, %p)\n", program, length, format, binary);)
    ...
    {} //STUB of gl4es_gles_glProgramBinary(glprogram->id, format, binary, length);

  gl4es_gles_glGetProgramiv(glprogram->id, GL_LINK_STATUS, &glprogram->linked);
    DBG(SHUT_LOGD("MobileGlues-gl4es:  link status = %d\n", glprogram->linked);)
    if(glprogram->linked) {
        fill_program(glprogram);
    } else {
        // should DBG the linker error?
        DBG(SHUT_LOGD("MobileGlues-gl4es:  Load failed!\n");)
        glprogram->linked = 0;
        errorGL();
    }
...
int gl4es_getProgramBinary(GLuint program, int *length, GLenum *format, void** binary)
{
    if(hardext.prgbin_n==0)
        return 0;
    DBG(SHUT_LOGD("MobileGlues-gl4es: getProgramBinary(%d, %p, %p, %p)\n", program, length, format, binary);)
    CHECK_PROGRAM(int, program)
    noerrorShim();
    ...
    int l;
  gl4es_gles_glGetProgramiv(glprogram->id, GL_PROGRAM_BINARY_LENGTH_OES, &l);
    if(!l)
        return 0;

    *binary = malloc(l);
  gl4es_gles_glGetProgramBinary(glprogram->id, l, length, format, *binary);
...
void APIENTRY_GL4ES gl4es_glGetProgramBinary(GLuint program, GLsizei bufSize, GLsizei *length, GLenum *binaryFormat, void *binary)
{
    ...
    DBG(SHUT_LOGD("MobileGlues-gl4es: glGetProgramBinary(%d, %d, %p, %p, %p)\n", program, bufSize, length, binaryFormat, binary);)
    CHECK_PROGRAM(void, program)
    LOAD_GLES_OES(glGetProgramBinary);
  gl4es_gles_glGetProgramBinary(glprogram->id, bufSize, length, binaryFormat, binary);
    errorGL();
}
...
void APIENTRY_GL4ES gl4es_glProgramBinary(GLuint program, GLenum binaryFormat, const void *binary, GLsizei length)
{
    ...
    DBG(SHUT_LOGD("MobileGlues-gl4es: glProgramBinary(%d, %d, %p, %d)\n", program, binaryFormat, binary, length);)
...
void APIENTRY_GL4ES gl4es_glLinkProgram(GLuint program) {
    DBG(SHUT_LOGD("MobileGlues-gl4es: glLinkProgram(%d)\n", program);)
    FLUSH_BEGINEND;
    CHECK_PROGRAM(void, program)
    noerrorShim();
    ...
    if(!compatible) {
        DBG(SHUT_LOGD("MobileGlues-gl4es: Need to redo some shaders...\n");)
        for (int i=0; i<glprogram->attach_size; i++) {
    ...
    if(gl4es_gles_glLinkProgram) {
      ...
      gl4es_gles_glGetProgramiv(glprogram->id, GL_LINK_STATUS, &glprogram->linked);
        ...
        if(glprogram->linked) {
            ...
            // should DBG the linker error?
            DBG(SHUT_LOGD("MobileGlues-gl4es:  Link failled!\n");)
            glprogram->linked = 0;
            errorShim(err);
            return;
        }
...
void APIENTRY_GL4ES gl4es_glUseProgram(GLuint program) {
    ...
    CHECK_PROGRAM(void, program)
    noerrorShim();
    DBG(SHUT_LOGD("MobileGlues-gl4es: program id=%d\n", glprogram->id);)
...
void APIENTRY_GL4ES gl4es_glValidateProgram(GLuint program) {
    ...
    if(gl4es_gles_glValidateProgram) {
        LOAD_GLES(glGetError);
        LOAD_GLES2(glGetProgramiv);
        {} //STUB of gl4es_gles_glValidateProgram(glprogram->id);
        GLenum err = gl4es_gles_glGetError();
      gl4es_gles_glGetProgramiv(glprogram->id, GL_VALIDATE_STATUS, (GLint *) &glprogram->valid_result);
        errorShim(err);

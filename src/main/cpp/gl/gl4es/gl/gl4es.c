...
void APIENTRY_GL4ES gl4es_glVertexPointer(GLint size, GLenum type,
    ...
    DBG(SHUT_LOGD("MobileGlues-gl4es: glVertexPointer(%d, %s, %d, %p)\n", size, PrintEnum(type), stride, pointer);)
    if(size<1 || size>4) {
...
void APIENTRY_GL4ES gl4es_glColorPointer(GLint size, GLenum type,
                     GLsizei stride, const GLvoid *pointer) {
    DBG(if (size>4) {
        SHUT_LOGD("MobileGlues-gl4es: glColorPointer(%d, %s, %d, %p)\n", size, PrintEnum(type), stride, pointer)
    } else {
        SHUT_LOGD("MobileGlues-gl4es: glColorPointer(%s, %s, %d, %p)\n", PrintEnum(size), PrintEnum(type), stride, pointer);
    })
...
void APIENTRY_GL4ES gl4es_glNormalPointer(GLenum type, GLsizei stride, const GLvoid *pointer) {
    DBG(SHUT_LOGD("MobileGlues-gl4es: glNormalPointer(%s, %d, %p)\n", PrintEnum(type), stride, pointer);)
    noerrorShimNoPurge();
...
void APIENTRY_GL4ES gl4es_glTexCoordPointer(GLint size, GLenum type,
                     GLsizei stride, const GLvoid *pointer) {
    DBG(SHUT_LOGD("MobileGlues-gl4es: glTexCoordPointer(%d, %s, %d, %p), texture.client=%d\n", size, PrintEnum(type), stride, pointer, glstate->texture.client);)
    if(size<1 || size>4) {
        errorShim(GL_INVALID_VALUE);
		return;
    }
...
void APIENTRY_GL4ES gl4es_glSecondaryColorPointer(GLint size, GLenum type, 
					GLsizei stride, const GLvoid *pointer) {
    DBG(if (size>4) {
        SHUT_LOGD("MobileGlues-gl4es: glSecondaryColorPointer(%d, %s, %d, %p)\n", size, PrintEnum(type), stride, pointer) 
    } else {
        SHUT_LOGD("MobileGlues-gl4es: glSecondaryColorPointer(%s, %s, %d, %p)\n", PrintEnum(size), PrintEnum(type), stride, pointer);
    })
...
void APIENTRY_GL4ES gl4es_glFogCoordPointer(GLenum type, GLsizei stride, const GLvoid *pointer) {
    DBG(SHUT_LOGD("MobileGlues-gl4es: glFogCoordPointer(%s, %d, %p)\n", PrintEnum(type), stride, pointer);)
...
void APIENTRY_GL4ES gl4es_glInterleavedArrays(GLenum format, GLsizei stride, const GLvoid *pointer) {
    DBG(SHUT_LOGD("MobileGlues-gl4es: glInterleavedArrays(%s, %d, %p)\n", PrintEnum(format), stride, pointer);)
...
void APIENTRY_GL4ES gl4es_glNormal3fv(GLfloat* v) {
    if (glstate->list.active) {
        ...
        LOAD_GLES_FPE(glNormal3f);
        errorGL();
        gl4es_gles_glNormal3f(v[0], v[1], v[2]);
    }
...
void APIENTRY_GL4ES gl4es_glColor4fv(GLfloat* v) {
    if (glstate->list.active) {
        ...
        LOAD_GLES_FPE(glColor4f);
        errorGL();
        gl4es_gles_glColor4f(v[0], v[1], v[2], v[3]);
    }
    // change the state last thing
...
void APIENTRY_GL4ES gl4es_glArrayElement(GLint i) {
    ...
    if (p->enabled) {
        ...
        if(p->type==GL_FLOAT) {
            ...
            GLfloat scale = 1.0f/GL4ES_gl_max_value(p->type);
...
void ToBuffer(int first, int count) {
    ...
    for (int i=0; i<NB_VA; i++)
        if(glstate->vao->vertexattrib[i].enabled && (!GL4ES_valid_vertex_type(glstate->vao->vertexattrib[i].type) || glstate->vao->vertexattrib[i].real_buffer)) {
            return;
        }
    ...
    if(!stride) stride = GL4ES_gl_sizeof(glstate->vao->vertexattrib[ATT_VERTEX].type)*glstate->vao->vertexattrib[ATT_VERTEX].size;
    ...
    if(!glstate->scratch_vertex)
        gl4es_gles_glGenBuffers(1, &glstate->scratch_vertex);
    glstate->scratch_vertex_size = stride*count;
    bindBuffer(GL_ARRAY_BUFFER, glstate->scratch_vertex);
    gl4es_gles_glBufferData(GL_ARRAY_BUFFER, stride*count, (void*)(master+first*stride), GL_STREAM_DRAW);
    #else
    LOAD_GLES(glBufferSubData);
    gl4es_scratch_vertex(total);    // alloc if needed and bind scratch vertex buffer
    gl4es_gles_glBufferSubData(GL_ARRAY_BUFFER, ptr, stride*count, (void*)(master+first*stride));
    #endif
    for (int i=0; i<NB_VA; i++) {
        if(glstate->vao->vertexattrib[i].enabled) {
            uintptr_t p = (uintptr_t)glstate->vao->vertexattrib[i].pointer;
...
void APIENTRY_GL4ES gl4es_glIndexPointer(GLenum type, GLsizei stride, const GLvoid * pointer) {
    ...
    if(!warning) {
        LOGD("MobileGlues-gl4es: Warning, stubbed glIndexPointer\n");
        warning = true;
    }
}
...
void APIENTRY_GL4ES gl4es_glEdgeFlagPointer(GLsizei stride, const GLvoid * pointer) {
    static bool warning = false;
    if(!warning) {
        LOGD("MobileGlues-gl4es: Warning, stubbed glEdgeFlagPointer\n");
        warning = true;
    }
}
...
#endif
        bindBuffer_real(GL_ARRAY_BUFFER, glstate->scratch_vertex);
        gl4es_gles_glBufferData(GL_ARRAY_BUFFER, alloc, NULL, GL_STREAM_DRAW);
...
void gl4es_use_scratch_vertex(int use) {
    bindBuffer_real(GL_ARRAY_BUFFER, use?glstate->scratch_vertex:0);
}
...
void gl4es_scratch_indices(int alloc) {
    ...
    if(!glstate->scratch_indices) {
        gl4es_gles_glGenBuffers(1, &glstate->scratch_indices);
    }
    bindBuffer_real(GL_ELEMENT_ARRAY_BUFFER, glstate->scratch_indices);
    if(glstate->scratch_indices_size < alloc) {
        gl4es_gles_glBufferData(GL_ELEMENT_ARRAY_BUFFER, alloc, NULL, GL_DYNAMIC_DRAW);
        glstate->scratch_indices_size = alloc;
    }
}

void gl4es_use_scratch_indices(int use) {
    bindBuffer_real(GL_ELEMENT_ARRAY_BUFFER, use?glstate->scratch_indices:0);
}
    ...
    if (globals4es.showfps) 
    {
        ...
        if (frame == 1) {
            ...
            if (last_frame < now) {
                ...
                SHUT_LOGD("MobileGlues-gl4es: LIBGL: fps: %.2f, avg: %.2f\n", fps, avg);
            }
        }
        last_frame = now;
    }

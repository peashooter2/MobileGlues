...
void APIENTRY_GL4ES gl4es_glPushAttrib(GLbitfield mask) {
    DBG(SHUT_LOGD("MobileGlues-gl4es: glPushAttrib(0x%04X)\n", mask);)
	...
	if (mask & GL_PIXEL_MODE_BIT) {
		...
		int i;
		for (i=0; i<8; i++) 
			gl4es_glGetFloatv(pixel_name[i], &cur->GL4ES_pixel_scale_bias[i]);
...
void APIENTRY_GL4ES gl4es_glPushClientAttrib(GLbitfield mask) {
    DBG(SHUT_LOGD("MobileGlues-gl4es: glPushClientAttrib(0x%04X)\n", mask);)
    noerrorShim();
...
DBG(SHUT_LOGD("MobileGlues-gl4es: glPopAttrib()\n");)
    noerrorShim();
    if (glstate->list.active)
        if (glstate->list.compiling) {
	...
	if (cur->mask & GL_PIXEL_MODE_BIT) {
		...
		int i;
		for (i=0; i<8; i++) 
			gl4es_glPixelTransferf(pixel_name[i], cur->GL4ES_pixel_scale_bias[i]);
        //TODO: GL_DEPTH_BIAS & GL_DEPTH_SCALE (probably difficult)
        //TODO: GL_INDEX_OFFEST & GL_INDEX_SHIFT
        //TODO: GL_MAP_COLOR & GL_MAP_STENCIL (probably difficult too)
...
void APIENTRY_GL4ES gl4es_glPopClientAttrib(void) {
    DBG(SHUT_LOGD("MobileGlues-gl4es: glPopClientAttrib()\n");)
    noerrorShim();

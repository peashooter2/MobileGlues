...
GLvoid *copy_gl_array(const GLvoid *src,
    ...
    if (! stride)
        stride = width * GL4ES_gl_sizeof(from);

    const char *unknown_str = "LIBGL: copy_gl_array -> unknown type: %x\n";
    if(!dst) {
        dst = malloc((count-skip) * to_width * GL4ES_gl_sizeof(to));
    }
    GLsizei from_size = GL4ES_gl_sizeof(from) * width;
    if (to_width < width) {
/*        SHUT_LOGD("MobileGlues-gl4es: Warning: copy_gl_array: %i < %i\n", to_width, width);
        return NULL;*/
        width = to_width;
    }
    // quick path if just copying same to same and no specific stride involved
    if(stride == (to_width*GL4ES_gl_sizeof(to)) && width==to_width && from==to) {
        memcpy(dst, (char*)src + stride*skip, (count-skip) * to_width * GL4ES_gl_sizeof(to));
        return dst;
    }
...
GLvoid *copy_gl_array_texcoord(const GLvoid *src,
    ...
    if(!dst)
        dst = malloc((count-skip) * to_width * GL4ES_gl_sizeof(GL_FLOAT));
    GLsizei from_size = GL4ES_gl_sizeof(from) * width;
    GLsizei to_elem = GL4ES_gl_sizeof(GL_FLOAT);
    uintptr_t in = (uintptr_t)src;
    in += stride*skip;
...
GLvoid *copy_gl_array_quickconvert(const GLvoid *src,
    ...
    if (! stride)
        stride = 4 * GL4ES_gl_sizeof(from);
    const char *unknown_str = "LIBGL: copy_gl_array_quickconvert -> unknown type: %x\n";
    GLvoid *dst = (dest)?dest:malloc((count-skip) * 4 * GL4ES_gl_sizeof(GL_FLOAT));
    ...
    GL_TYPE_SWITCH2(input, in, from,
        const GLfloat maxf = 1.0f/GL4ES_gl_max_value(from);
        for (int i = skip; i < count; i++)
        ,
                for (j = 0; j < 4; j++) {
...
GLvoid *copy_gl_array_convert(const GLvoid *src,
    ...
    if(to==from && width==to_width && stride==(to_width * GL4ES_gl_sizeof(to))) {
    ...
    if(!dst) dst = malloc((count-skip) * to_width * GL4ES_gl_sizeof(to));
    if (to_width < width) {
        /*SHUT_LOGD("MobileGlues-gl4es: Warning: copy_gl_array: %i < %i\n", to_width, width);
        return NULL;*/
        width = to_width;
    ...
    if (from == to && to_width >= width) {
        GL_TYPE_SWITCH(out, dst, to,
            for (int i = skip; i < count; i++) {
                ...
                for (; j < to_width; j++) {
					memcpy(&out[j], filler, GL4ES_gl_sizeof(to));
                }
                out += to_width;
                in += stride;
            },
        ...
        GL_TYPE_SWITCH_MAX(out, dst, to,
            GL_TYPE_SWITCH2(input, in, from,
                const GLuint maxf = GL4ES_gl_max_value(from);
                for (int i = skip; i < count; i++)
                ,
                    for (j = 0; j < width; j++) {
...
GLfloat *gl_pointer_index(vertexattrib_t *p, GLint index) {
    ...
    GLsizei size = GL4ES_gl_sizeof(p->type);
    ...
    GL_TYPE_SWITCH(src, ptr, p->type,
        ...
        default:
            SHUT_LOGD("MobileGlues-gl4es: LIBGL: unknown pointer type: 0x%x\n", p->type);
    )
    return buf;
}

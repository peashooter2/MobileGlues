...
static void free_framebuffer(glframebuffer_t *fb)
{
    ...
    if(!fb || !gl4es_gles_glDeleteFramebuffers)
        return;
    if(fb->id)
        {} //STUB of gl4es_gles_glDeleteFramebuffers(1, &fb->id);
    // the texture will be free by the free of the texture list, as it's referenced there...
    free(fb);
}
...
static void free_texture(gltexture_t *tex)
{
    ...
    if(!tex || !gl4es_gles_glDeleteTextures)
        return;
    if(tex->glname)
...
void* NewGLState(void* shared_glstate, int es2only) {
  ...
  gl4es_gles_glGetIntegerv(GL_VIEWPORT, (GLint*)&glstate->raster.viewport);
  gl4es_gles_glGetIntegerv(GL_SCISSOR_BOX, (GLint*)&glstate->raster.scissor);
#if defined(AMIGAOS4) || defined(__EMSCRIPTEN__)
    }
#endif
    // FBO
    ...
    LOAD_GLES(glGetIntegerv);
#endif
  gl4es_gles_glGetIntegerv(GL_IMPLEMENTATION_COLOR_READ_FORMAT_OES, (GLint *) &glstate->readf);
  gl4es_gles_glGetIntegerv(GL_IMPLEMENTATION_COLOR_READ_TYPE_OES, (GLint *) &glstate->readt);
#if defined(AMIGAOS4) || defined(__EMSCRIPTEN__)
    }
#endif
    //SHUT_LOGD("MobileGlues-gl4es: LIBGL: Implementation Read is %s/%s\n", PrintEnum(glstate->readf), PrintEnum(glstate->readt));
...
void DeleteGLState(void* oldstate) {
    ...
    if(!state->shared_cnt && state->fbo.old) {
        LOAD_GLES2_OR_OES(glDeleteFramebuffers);
        {} //STUB of gl4es_gles_glDeleteFramebuffers(state->fbo.old->nbr, state->fbo.old->fbos);
...
void ActivateGLState(void* new_glstate) {
    ...
    if(new_glstate && (newstate->raster.viewport.width==0 || newstate->raster.viewport.height==0)) {
        LOAD_GLES(glGetIntegerv);
      gl4es_gles_glGetIntegerv(GL_VIEWPORT, (GLint*)&newstate->raster.viewport);
      gl4es_gles_glGetIntegerv(GL_SCISSOR_BOX, (GLint*)&newstate->raster.scissor);
    }
    glstate = newstate;
}

...
DBG(SHUT_LOGD("MobileGlues-gl4es: init_matrix(%p)\n", glstate);)
...
DBG(SHUT_LOGD("MobileGlues-gl4es: glMatrixMode(%s), list=%p\n", PrintEnum(mode), glstate->list.active);)
	noerrorShim();
...
DBG(SHUT_LOGD("MobileGlues-gl4es: glPushMatrix(), list=%p\n", glstate->list.active);)
	...
	switch(matrix_mode) {
		...
		default:
			if(glstate->matrix_mode>=GL_MATRIX0_ARB && glstate->matrix_mode<GL_MATRIX0_ARB+MAX_ARB_MATRIX) {
				...
				//LOGE("MobileGlues-gl4es: PushMatrix with Unrecognise matrix mode (0x%04X)\n", matrix_mode);
...
DBG(SHUT_LOGD("MobileGlues-gl4es: glPopMatrix(), list=%p\n", glstate->list.active);)
	...
	switch(matrix_mode) {
		#define P(A) if(glstate->A->top) { \
			...
			if (send_to_hardware()) {LOAD_GLES(glLoadMatrixf); gl4es_gles_glLoadMatrixf(update_current_mat()); } \
		...
		default:
			if(glstate->matrix_mode>=GL_MATRIX0_ARB && glstate->matrix_mode<GL_MATRIX0_ARB+MAX_ARB_MATRIX) {
				...
				//Warning?
				errorShim(GL_INVALID_OPERATION);
				//LOGE("MobileGlues-gl4es: PopMatrix with Unrecognise matrix mode (0x%04X)\n", matrix_mode);
...
DBG(SHUT_LOGD("MobileGlues-gl4es: glLoadMatrix(%f, %f, %f, %f, %f, %f, %f...), list=%p\n", m[0], m[1], m[2], m[3], m[4], m[5], m[6], glstate->list.active);)
	if (glstate->list.active) {
    ...
    if(send_to_hardware()) {
		...
		if(id) gl4es_gles_glLoadIdentity();	// in case the driver as some special optimisations
		else gl4es_gles_glLoadMatrixf(m);
	}
}

void APIENTRY_GL4ES gl4es_glMultMatrixf(const GLfloat * m) {
DBG(SHUT_LOGD("MobileGlues-gl4es: glMultMatrix(%f, %f, %f, %f, %f, %f, %f...), list=%p\n", m[0], m[1], m[2], m[3], m[4], m[5], m[6], glstate->list.active);)
	if (glstate->list.active) {
		if(glstate->list.pending) gl4es_flush();
		else {
...
DBG(SHUT_LOGD("MobileGlues-gl4es: glTranslatef(%f, %f, %f), list=%p\n", x, y, z, glstate->list.active);)
...
DBG(SHUT_LOGD("MobileGlues-gl4es: glScalef(%f, %f, %f), list=%p\n", x, y, z, glstate->list.active);)
...
DBG(SHUT_LOGD("MobileGlues-gl4es: glRotatef(%f, %f, %f, %f), list=%p\n", angle, x, y, z, glstate->list.active);)
...
DBG(SHUT_LOGD("MobileGlues-gl4es: glOrthof(%f, %f, %f, %f, %f, %f), list=%p\n", left, right, top, bottom, nearVal, farVal, glstate->list.active);)
	GLfloat tmp[16];
	memset(tmp, 0, 16*sizeof(GLfloat));
...
DBG(SHUT_LOGD("MobileGlues-gl4es: glFrustumf(%f, %f, %f, %f, %f, %f) list=%p\n", left, right, top, bottom, nearVal, farVal, glstate->list.active);)
	GLfloat tmp[16];
	memset(tmp, 0, 16*sizeof(GLfloat));

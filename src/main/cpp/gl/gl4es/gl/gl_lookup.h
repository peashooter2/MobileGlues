...
#define MAP(func_name, func) \
    if (strcmp(name, func_name) == 0) {SHUT_LOGD("MobileGlues-gl4es: %p (%s)\n", (void*)func, #func) ;return (void *)func;}
#else
#define MAP(func_name, func) \
...
#define STUB(func_name)                       \
    if (strcmp(name, #func_name) == 0) {      \
        SHUT_LOGD("MobileGlues-gl4es: => STUB\n");                  \
        if(!globals4es.silentstub) LOGD("MobileGlues-gl4es: GL4ES stub: %s\n", #func_name); \

...
KHASH_MAP_IMPL_INT(shaderlist, GL4ES_shader_t *);

GLuint APIENTRY_GL4ES gl4es_glCreateShader(GLenum shaderType) {
    DBG(SHUT_LOGD("MobileGlues-gl4es: glCreateShader(%s)\n", PrintEnum(shaderType));)
    // sanity check
    if (shaderType!=GL_VERTEX_SHADER && shaderType!=GL_FRAGMENT_SHADER) {
        DBG(SHUT_LOGD("MobileGlues-gl4es: Invalid shader type\n");)
    ...
    if(gl4es_gles_glCreateShader) {
        shader = gl4es_gles_glCreateShader(shaderType);
        if(!shader) {
            DBG(SHUT_LOGD("MobileGlues-gl4es: Failed to create shader\n");)
            errorGL();
            return 0;
        }
    } else {
    ...
    if (k == kh_end(shaders)){
        ...
        glshader = kh_value(shaders, k) = (GL4ES_shader_t*)calloc(1, sizeof(GL4ES_shader_t));
    } else {
...
void actually_deleteshader(GLuint shader) {
    ...
    k = kh_get(shaderlist, shaders, shader);
    if (k != kh_end(shaders)) {
        GL4ES_shader_t *glshader = kh_value(shaders, k);
...
void APIENTRY_GL4ES gl4es_glDeleteShader(GLuint shader) {
    DBG(SHUT_LOGD("MobileGlues-gl4es: glDeleteShader(%d)\n", shader);)
    // sanity check...
...
void APIENTRY_GL4ES gl4es_glCompileShader(GLuint shader) {
    DBG(SHUT_LOGD("MobileGlues-gl4es: glCompileShader(%d)\n", shader);)
    // look for the shader
    CHECK_SHADER(void, shader)
    ...
    if(gl4es_gles_glCompileShader) {
        ...
        if(globals4es.logshader) {
            ...
            GLint status = 0;
          gl4es_gles_glGetShaderiv(glshader->id, GL_COMPILE_STATUS, &status);
            if(status!=GL_TRUE) {
                SHUT_LOGD("MobileGlues-gl4es: LIBGL: Error while compiling shader %d. Original source is:\n%s\n=======\n", glshader->id, glshader->source);
                SHUT_LOGD("MobileGlues-gl4es: ShaderConv Source is:\n%s\n=======\n", glshader->converted);
                char tmp[500];
                GLint length;
              gl4es_gles_glGetShaderInfoLog(glshader->id, 500, &length, tmp);
                SHUT_LOGD("MobileGlues-gl4es: Compiler message is\n%s\nLIBGL: End of Error log\n", tmp);
            }
        }
    } else
        noerrorShim();
...
void APIENTRY_GL4ES gl4es_glShaderSource(GLuint shader, GLsizei count, const GLchar * const *string, const GLint *length) {
    DBG(SHUT_LOGD("MobileGlues-gl4es: glShaderSource(%d, %d, %p, %p)\n", shader, count, string, length);)
    // sanity check
    if(count<=0) {
...
void APIENTRY_GL4ES gl4es_glGetShaderSource(GLuint shader, GLsizei bufSize, GLsizei *length, GLchar *source) {
    DBG(SHUT_LOGD("MobileGlues-gl4es: glGetShaderSource(%d, %d, %p, %p)\n", shader, bufSize, length, source);)
    // find shader
    CHECK_SHADER(void, shader)
    if (bufSize<=0) {
...
GLboolean APIENTRY_GL4ES gl4es_glIsShader(GLuint shader) {
    DBG(SHUT_LOGD("MobileGlues-gl4es: glIsShader(%d)\n", shader);)
    // find shader
    GL4ES_shader_t *glshader = NULL;
    khint_t k;
    {
...
GL4ES_shader_t *getShader(GLuint shader) {
    khint_t k;
    {
        khash_t(shaderlist) *shaders = glstate->glsl->shaders;
        k = kh_get(shaderlist, shaders, shader);
...
void APIENTRY_GL4ES gl4es_glGetShaderInfoLog(GLuint shader, GLsizei maxLength, GLsizei *length, GLchar *infoLog) {
    DBG(SHUT_LOGD("MobileGlues-gl4es: glGetShaderInfoLog(%d, %d, %p, %p)\n", shader, maxLength, length, infoLog);)
    // find shader
    CHECK_SHADER(void, shader)
    if(maxLength<=0) {
        errorShim(GL_INVALID_OPERATION);
    ...
    LOAD_GLES2(glGetShaderInfoLog);
    if(gl4es_gles_glGetShaderInfoLog) {
      gl4es_gles_glGetShaderInfoLog(glshader->id, maxLength, length, infoLog);
        errorGL();
    } else {
...
void APIENTRY_GL4ES gl4es_glGetShaderiv(GLuint shader, GLenum pname, GLint *params) {
    DBG(SHUT_LOGD("MobileGlues-gl4es: glGetShaderiv(%d, %s, %p)\n", shader, PrintEnum(pname), params);)
    // find shader
    CHECK_SHADER(void, shader)
    LOAD_GLES2(glGetShaderiv);
    noerrorShim();
    switch (pname) {
        ...
        case GL_COMPILE_STATUS:
            if(gl4es_gles_glGetShaderiv) {
              gl4es_gles_glGetShaderiv(glshader->id, pname, params);
                errorGL();
            } else {
...
void APIENTRY_GL4ES gl4es_glShaderBinary(GLsizei count, const GLuint *shaders, GLenum binaryFormat, const void *binary, GLsizei length) {
    ...
    if (gl4es_gles_glShaderBinary) {
        {} //STUB of gl4es_gles_glShaderBinary(count, shaders, binaryFormat, binary, length);
        errorGL();
    } else {
        errorShim(GL_INVALID_ENUM);
    }
...
void APIENTRY_GL4ES gl4es_glReleaseShaderCompiler(void) {
    ...
    if(gl4es_gles_glReleaseShaderCompiler) {
        {} //STUB of gl4es_gles_glReleaseShaderCompiler();
        errorGL();
    } else
        noerrorShim();
}

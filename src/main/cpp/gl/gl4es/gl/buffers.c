...
void APIENTRY_GL4ES gl4es_glGenBuffers(GLsizei n, GLuint * buffers) {
    DBG(SHUT_LOGD("MobileGlues-gl4es: glGenBuffers(%i, %p)\n", n, buffers);)
	noerrorShim();
    if (n<1) {
...
void APIENTRY_GL4ES gl4es_glBindBuffer(GLenum target, GLuint buffer) {
    DBG(SHUT_LOGD("MobileGlues-gl4es: glBindBuffer(%s, %u)\n", PrintEnum(target), buffer);)
...
void APIENTRY_GL4ES gl4es_glBufferData(GLenum target, GLsizeiptr size, const GLvoid * data, GLenum usage, GLuint real_buffer) {
    DBG(SHUT_LOGD("MobileGlues-gl4es: glBufferData(%s, %zi, %p, %s)\n", PrintEnum(target), size, data, PrintEnum(usage));)
	if (!buffer_target(target)) {
		errorShim(GL_INVALID_ENUM);
		return;
	}
    ...
    if (buff==NULL) {
        ...
        LOGE("MobileGlues-gl4es: Warning, null buffer for target=0x%04X for glBufferData\n", target);
        return;
    }
    if(target==GL_ARRAY_BUFFER)
    ...
    if(go_real) {
        if(!buff->real_buffer) {
            ...
            {} //STUB of gl4es_gles_glG enBuffers(1, &buff->real_buffer);
            buff->real_buffer = real_buffer;
        }
        LOAD_GLES(glBufferData);
        LOAD_GLES(glBindBuffer);
        bindBuffer(target, buff->real_buffer);
        {} //STUB of gl4es_gles_glBufferData(target, size, data, usage);
        DBG(SHUT_LOGD("MobileGlues-gl4es:  => real VBO %d\n", buff->real_buffer);)
    }
    ...
    buff->size = size;
    buff->usage = usage;
    DBG(SHUT_LOGD("MobileGlues-gl4es: \t buff->data = %p (size=%zd)\n", buff->data, size);)
    buff->access = GL_READ_WRITE;
    if (data)
...
void APIENTRY_GL4ES gl4es_glNamedBufferData(GLuint buffer, GLsizeiptr size, const GLvoid * data, GLenum usage) {
    DBG(SHUT_LOGD("MobileGlues-gl4es: glNamedBufferData(%u, %zi, %p, %s)\n", buffer, size, data, PrintEnum(usage));)
    glbuffer_t *buff = getbuffer_id(buffer);
    if (buff==NULL) {
        DBG(SHUT_LOGD("MobileGlues-gl4es: Named Buffer not found\n");)
		errorShim(GL_INVALID_OPERATION);
        return;
    }
    if (buff->data) {
    ...
    if(go_real) {
        if(!buff->real_buffer) {
            LOAD_GLES(glGenBuffers);
            // TODO: No named buffer in GLES, need to impl in MobileGluse first
            {} //STUB of gl4es_gles_glG enBuffers(1, &buff->real_buffer);
        }
        LOAD_GLES(glBufferData);
        LOAD_GLES(glBindBuffer);
        bindBuffer(buff->type, buff->real_buffer);
        {} //STUB of gl4es_gles_glBufferData(buff->type, size, data, usage);
    }
...
void APIENTRY_GL4ES gl4es_glBufferSubData(GLenum target, GLintptr offset, GLsizeiptr size, const GLvoid * data) {
    DBG(SHUT_LOGD("MobileGlues-gl4es: glBufferSubData(%s, %p, %zi, %p)\n", PrintEnum(target), (void*)offset, size, data);)
	if (!buffer_target(target)) {
		errorShim(GL_INVALID_ENUM);
		return;
	}
    ...
    if (buff==NULL) {
		errorShim(GL_INVALID_OPERATION);
        DBG(SHUT_LOGD("MobileGlues-gl4es: LIBGL: Warning, null buffer for target=0x%04X for glBufferSubData\n", target);)
        return;
    }
    ...
    if((target==GL_ARRAY_BUFFER || target==GL_ELEMENT_ARRAY_BUFFER) && buff->real_buffer) {
        ...
        LOAD_GLES(glBindBuffer);
        bindBuffer(target, buff->real_buffer);
        {} //STUB of gl4es_gles_glBufferSubData(target, offset, size, data);
    }
...
void APIENTRY_GL4ES gl4es_glNamedBufferSubData(GLuint buffer, GLintptr offset, GLsizeiptr size, const GLvoid * data) {
    DBG(SHUT_LOGD("MobileGlues-gl4es: glNamedBufferSubData(%u, %p, %zi, %p)\n", buffer, (void*)offset, size, data);)
    glbuffer_t *buff = getbuffer_id(buffer);
    if (buff==NULL) {
		errorShim(GL_INVALID_OPERATION);
        return;
    ...
    if((buff->type==GL_ARRAY_BUFFER || buff->type==GL_ELEMENT_ARRAY_BUFFER) && buff->real_buffer) {
        ...
        LOAD_GLES(glBindBuffer);
        bindBuffer(buff->type, buff->real_buffer);
        {} //STUB of gl4es_gles_glBufferSubData(buff->type, offset, size, data);
    }
    memcpy((char*)buff->data + offset, data, size);
    noerrorShim();
}
...
void APIENTRY_GL4ES gl4es_glDeleteBuffers(GLsizei n, const GLuint * buffers) {
    DBG(SHUT_LOGD("MobileGlues-gl4es: glDeleteBuffers(%i, %p)\n", n, buffers);)
    if(!glstate) return;
    FLUSH_BEGINEND;
    ...
    if (list) {
        ...
        for (int i = 0; i < n; i++) {
            GLuint t = buffers[i];
            DBG(SHUT_LOGD("MobileGlues-gl4es: \t deleting %d\n", t);)
            if (t) {    // don't allow to remove default one
                ...
                if (k != kh_end(list)) {
                    ...
                    DBG(SHUT_LOGD("MobileGlues-gl4es: \t buff->data = %p\n", buff->data);)
    ...
    DBG(SHUT_LOGD("MobileGlues-gl4es: \t done\n");)
    noerrorShim();
}

GLboolean APIENTRY_GL4ES gl4es_glIsBuffer(GLuint buffer) {
    DBG(SHUT_LOGD("MobileGlues-gl4es: glIsBuffer(%u)\n", buffer);)
	khash_t(buff) *list = glstate->buffers;
	khint_t k;
	noerrorShim();
    if (list) {
...
void APIENTRY_GL4ES gl4es_glGetBufferParameteriv(GLenum target, GLenum value, GLint * data) {
    DBG(SHUT_LOGD("MobileGlues-gl4es: glGetBufferParameteriv(%s, %s, %p)\n", PrintEnum(target), PrintEnum(value), data);)
	if (!buffer_target(target)) {
		errorShim(GL_INVALID_ENUM);
		return;
	}
...
void APIENTRY_GL4ES gl4es_glGetNamedBufferParameteriv(GLuint buffer, GLenum value, GLint * data) {
    DBG(SHUT_LOGD("MobileGlues-gl4es: glGetNamedBufferParameteriv(%u, %s, %p)\n", buffer, PrintEnum(value), data);)
	glbuffer_t* buff = getbuffer_id(buffer);
	if (buff==NULL) {
		errorShim(GL_INVALID_OPERATION);
		return;		// Should generate an error!
...
void* APIENTRY_GL4ES gl4es_glMapBuffer(GLenum target, GLenum access) {
    DBG(SHUT_LOGD("MobileGlues-gl4es: glMapBuffer(%s, %s)\n", PrintEnum(target), PrintEnum(access));)
	if (!buffer_target(target)) {
		errorShim(GL_INVALID_ENUM);
		return (void*)NULL;
	}
...
void* APIENTRY_GL4ES gl4es_glMapNamedBuffer(GLuint buffer, GLenum access) {
    DBG(SHUT_LOGD("MobileGlues-gl4es: glMapNamedBuffer(%u, %s)\n", buffer, PrintEnum(access));)
...
GLboolean APIENTRY_GL4ES gl4es_glUnmapBuffer(GLenum target) {
    DBG(SHUT_LOGD("MobileGlues-gl4es: glUnmapBuffer(%s)\n", PrintEnum(target));)
    ...
    if(buff->real_buffer && (buff->type==GL_ARRAY_BUFFER || buff->type==GL_ELEMENT_ARRAY_BUFFER) && buff->mapped && !buff->ranged && (buff->access==GL_WRITE_ONLY || buff->access==GL_READ_WRITE)) {
        ...
        LOAD_GLES(glBindBuffer);
        bindBuffer(buff->type, buff->real_buffer);
        {} //STUB of gl4es_gles_glBufferSubData(buff->type, 0, buff->size, buff->data);
    }
    if(buff->real_buffer && (buff->type==GL_ARRAY_BUFFER || buff->type==GL_ELEMENT_ARRAY_BUFFER) && buff->mapped && buff->ranged && (buff->access&GL_MAP_WRITE_BIT_EXT) && !(buff->access&GL_MAP_FLUSH_EXPLICIT_BIT_EXT)) {
        LOAD_GLES(glBufferSubData);
        bindBuffer(buff->type, buff->real_buffer);
        {} //STUB of gl4es_gles_glBufferSubData(buff->type, buff->offset, buff->length, (void*)((uintptr_t)buff->data+buff->offset));
    }
    if (buff->mapped) {
		buff->mapped = 0;
        buff->ranged = 0;
...
GLboolean APIENTRY_GL4ES gl4es_glUnmapNamedBuffer(GLuint buffer) {
    DBG(SHUT_LOGD("MobileGlues-gl4es: glUnmapNamedBuffer(%u)\n", buffer);)
    if(glstate->list.compiling) {errorShim(GL_INVALID_OPERATION); return GL_FALSE;}
    FLUSH_BEGINEND;
...
void APIENTRY_GL4ES gl4es_glGetBufferSubData(GLenum target, GLintptr offset, GLsizeiptr size, GLvoid * data) {
    DBG(SHUT_LOGD("MobileGlues-gl4es: glGetBufferSubData(%s, %p, %zi, %p)\n", PrintEnum(target), (void*)offset, size, data);)
	if (!buffer_target(target)) {
		errorShim(GL_INVALID_ENUM);
		return;
	}
...
void APIENTRY_GL4ES gl4es_glGetNamedBufferSubData(GLuint buffer, GLintptr offset, GLsizeiptr size, GLvoid * data) {
    DBG(SHUT_LOGD("MobileGlues-gl4es: glGetNamedBufferSubData(%u, %p, %zi, %p)\n", buffer, (void*)offset, size, data);)
	glbuffer_t *buff = getbuffer_id(buffer);
...
void APIENTRY_GL4ES gl4es_glGetBufferPointerv(GLenum target, GLenum pname, GLvoid ** params) {
    DBG(SHUT_LOGD("MobileGlues-gl4es: glGetBufferPointerv(%s, %s, %p)\n", PrintEnum(target), PrintEnum(pname), params);)
	if (!buffer_target(target)) {
		errorShim(GL_INVALID_ENUM);
		return;
	}
...
void APIENTRY_GL4ES gl4es_glGetNamedBufferPointerv(GLuint buffer, GLenum pname, GLvoid ** params) {
    DBG(SHUT_LOGD("MobileGlues-gl4es: glGetNamedBufferPointerv(%u, %s, %p)\n", buffer, PrintEnum(pname), params);)
	glbuffer_t *buff = getbuffer_id(buffer);
	if (buff==NULL)
		return;		// Should generate an error!
	if (pname != GL_BUFFER_MAP_POINTER) {
...
{
    DBG(SHUT_LOGD("MobileGlues-gl4es: glMapBufferRange(%s, %p, %zd, 0x%x)\n", PrintEnum(target), (void*)offset, length, access);)
	if (!buffer_target(target)) {
		errorShim(GL_INVALID_ENUM);
		return NULL;
	}
...
{
    DBG(SHUT_LOGD("MobileGlues-gl4es: glFlushMappedBufferRange(%s, %p, %zd)\n", PrintEnum(target), (void*)offset, length);)
	if (!buffer_target(target)) {
		errorShim(GL_INVALID_ENUM);
		return;
	}
...
void bindBuffer(GLenum target, GLuint buffer)
{
    ...
    if(target==GL_ARRAY_BUFFER) {
        if(glstate->bind_buffer.array == buffer)
            return;
        DBG(SHUT_LOGD("MobileGlues-gl4es: Bind buffer %d to GL_ARRAY_BUFFER\n", buffer);)
...
void bindBuffer_real(GLenum target, GLuint buffer)
{
    LOAD_GLES(glBindBuffer);
    if(target==GL_ARRAY_BUFFER) {
        if(glstate->bind_buffer.array == buffer)
            return;
        DBG(SHUT_LOGD("MobileGlues-gl4es: Really Bind buffer %d to GL_ARRAY_BUFFER\n", buffer);)
        glstate->bind_buffer.array = buffer;
...
void realize_bufferIndex()
{
    ...
    if(glstate->bind_buffer.index != glstate->bind_buffer.want_index) {
        ...
        {} //STUB of gl4es_gles_glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, glstate->bind_buffer.index);
        DBG(SHUT_LOGD("MobileGlues-gl4es: Bind buffer %d to GL_ELEMENT_ARRAY_BUFFER\n", glstate->bind_buffer.index);)
        glstate->bind_buffer.used = (glstate->bind_buffer.index && glstate->bind_buffer.array)?1:0;
    }
}

void realize_bufferIndex_real()
{
    LOAD_GLES(glBindBuffer);
    if(glstate->bind_buffer.index != glstate->bind_buffer.want_index) {
        glstate->bind_buffer.index = glstate->bind_buffer.want_index;
        gl4es_gles_glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, glstate->bind_buffer.index);
        DBG(SHUT_LOGD("MobileGlues-gl4es: Really Bind buffer %d to GL_ELEMENT_ARRAY_BUFFER\n", glstate->bind_buffer.index);)
        glstate->bind_buffer.used = (glstate->bind_buffer.index && glstate->bind_buffer.array)?1:0;
    }
}
...
void deleteSingleBuffer_real(GLuint buffer) {
   LOAD_GLES(glDeleteBuffers);
   if(glstate->bind_buffer.index == buffer) glstate->bind_buffer.index = 0;
   else if(glstate->bind_buffer.want_index == buffer) glstate->bind_buffer.want_index = 0;
   else if(glstate->bind_buffer.array == buffer) glstate->bind_buffer.array = 0;
   gl4es_gles_glDeleteBuffers(1, &buffer);
}
...
void unboundBuffers()
{
    ...
    if(glstate->bind_buffer.array) {
        ...
        {} //STUB of gl4es_gles_glBindBuffer(GL_ARRAY_BUFFER, 0);
        DBG(SHUT_LOGD("MobileGlues-gl4es: Bind buffer %d to GL_ARRAY_BUFFER\n", 0);)
    }
...
void unboundBuffers_real()
{
    ...
    if(glstate->bind_buffer.array) {
        glstate->bind_buffer.array = 0;
        gl4es_gles_glBindBuffer(GL_ARRAY_BUFFER, 0);
        DBG(SHUT_LOGD("MobileGlues-gl4es: Really Bind buffer %d to GL_ARRAY_BUFFER\n", 0);)
    }
    if(glstate->bind_buffer.index) {
        glstate->bind_buffer.index = 0;
...
void APIENTRY_GL4ES gl4es_glGenVertexArrays(GLsizei n, GLuint *arrays) {
    DBG(SHUT_LOGD("MobileGlues-gl4es: glGenVertexArrays(%i, %p)\n", n, arrays);)
	noerrorShim();
    if (n<1) {
		errorShim(GL_INVALID_VALUE);
        return;
...
void APIENTRY_GL4ES gl4es_glBindVertexArray(GLuint array) {
    DBG(SHUT_LOGD("MobileGlues-gl4es: glBindVertexArray(%u)\n", array);)
    FLUSH_BEGINEND;
...
void APIENTRY_GL4ES gl4es_glDeleteVertexArrays(GLsizei n, const GLuint *arrays) {
    DBG(SHUT_LOGD("MobileGlues-gl4es: glDeleteVertexArrays(%i, %p)\n", n, arrays);)
    if(!glstate) return;
    FLUSH_BEGINEND;
...
GLboolean APIENTRY_GL4ES gl4es_glIsVertexArray(GLuint array) {
    DBG(SHUT_LOGD("MobileGlues-gl4es: glIsVertexArray(%u)\n", array);)
    if(!glstate)
        return GL_FALSE;
	khash_t(glvao) *list = glstate->vaos;
	khint_t k;

...
void *gl4es_open_lib(const char **names, const char *override) {
...
void *gl4es_gles = NULL, *gl4es_egl = NULL, *bcm_host = NULL, *vcos = NULL, *gbm = NULL, *drm = NULL;
#ifndef _WIN32
#ifndef NO_GBM
...
void *gl4es_open_lib(const char **names, const char *override) {
    ...
    if (override) {
        if ((lib = dlopen(override, flags))) {
            ...
            if(!globals4es.nobanner) LOGD("MobileGlues-gl4es: LIBGL:loaded: %s\n", path_name);
            return lib;
        } else {
            LOGE("MobileGlues-gl4es: LIBGL_GLES override failed: %s\n", dlerror());
        }
    }
        ...
        for (int i = 0; names[i]; i++) {
            for (int e = 0; lib_ext[e]; e++) {
                ...
                if ((lib = dlopen(path_name, flags))) {
                    if(!globals4es.nobanner) LOGD("MobileGlues-gl4es: loaded: %s\n", path_name);
                    return lib;
                }
            }
        }
...
#else  // _WIN32
void* gl4es_open_lib(const wchar_t* envvar, const wchar_t* dll)
{
...
/*#ifndef _WIN32
    const char *gles_override = GL4ES_GetEnvVar("LIBGL_GLES");
    if (!gles_override) {
        ...
        if (! gles_override) {
            ...
            bcm_host = gl4es_open_lib(bcm_host_name, NULL);
            vcos = gl4es_open_lib(vcos_name, NULL);
        }
#endif
    }
    gl4es_gles = gl4es_open_lib((globals4es.es==1)?gles_lib:gles2_lib, gles_override);
#else
    gles = gl4es_open_lib(L"LIBGL_GLES", L"libGLESv2.dll");
#endif
    WARN_NULL(gl4es_gles);

#ifdef NOEGL
    egl = gles;
#elif !defined(_WIN32)
    const char *egl_override = GL4ES_GetEnvVar("LIBGL_EGL");
    if (!egl_override) {
        egl_override = DEFAULT_EGL;
    }
    gl4es_egl = gl4es_open_lib(egl_lib, egl_override);
#else
    egl = gl4es_open_lib(L"LIBGL_EGL", L"libEGL.dll");
#endif
    WARN_NULL(gl4es_egl);

#ifndef NO_GBM
    const char *gbm_override = GL4ES_GetEnvVar("LIBGL_GBM");
    gbm = gl4es_open_lib(gbm_lib, gbm_override);
    const char *drm_override = GL4ES_GetEnvVar("LIBGL_DRM");
    drm = gl4es_open_lib(drm_lib, drm_override);
#endif*/
}
#endif

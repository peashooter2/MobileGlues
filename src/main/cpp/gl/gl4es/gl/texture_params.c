...
void tex_setup_texcoord(GLuint len, int changes, GLuint itarget, vertexattrib_t* ptr) {
    ...
    if (changes) {
        ...
        gl4es_gles_glTexCoordPointer(4, GL_FLOAT, 0, glstate->helper_tex[texunit]);
    } else {
        gl4es_gles_glTexCoordPointer(ptr->size, ptr->type, ptr->stride, ptr->pointer);
    }
}
...
gltexture_t* gl4es_getTexture(GLenum target, GLuint texture, bool fpe_action, GLuint real_tex_glname) {
    ...
    if (k == kh_end(list)){
        ...
        tex->texture = texture;
        if (texture) {
            //{} //STUB of gl4es_gles_glG enTextures(1, &tex->glname);
            //if (!fpe_action)
            //    tex->glname = real_tex_glname;
            //else
                gl4es_gles_glGenTextures(1, &tex->glname);
        } else
            tex->glname = 0;    // special case for texture n# 0
        DBG(SHUT_LOGD("MobileGlues-gl4es: getTexture(%s, %u), failed, creating texture %u\n", PrintEnum(target), texture, tex->glname);)
        tex->target = target;
...
GLenum get_texture_wrap(GLenum wrap, gltexture_t* texture)
{
    ...
    //SHUT_LOGD("MobileGlues-gl4es: Warning, REPEAT / MIRRORED_REPEAT on NPOT texture\n");
...
void APIENTRY_GL4ES gl4es_glTexParameterfv(GLenum target, GLenum pname, const GLfloat *params) {
    ...
    if(!samplerParameterfv(&texture->sampler, pname, params)) {
        ...
        FLUSH_BEGINEND;
        realize_bound(glstate->texture.active, target);
        {} //STUB of gl4es_gles_glTexParameterfv(rtarget, pname, params);
        errorGL();
    }
}
    ...
    if (list) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            if (k != kh_end(list)) {
                ...
                {} //STUB of gl4es_gles_glDeleteTextures(1, &tex->glname);
...
void APIENTRY_GL4ES gl4es_glGetTexLevelParameterfv(GLenum target, GLint level, GLenum pname, GLfloat *params) {
    DBG(SHUT_LOGD("MobileGlues-gl4es: glGetTexLevelParameteriv(%s, %d, %s, %p)\n", PrintEnum(target), level, PrintEnum(pname), params);)
    ...
    if(!getSamplerParameterfv(&bound->sampler, pname, params)) {
        switch (pname) {
            ...
            default:
                ...
                SHUT_LOGD("MobileGlues-gl4es: Stubbed glGetTexLevelParameteriv(%s, %i, %s, %p)\n", PrintEnum(target), level, PrintEnum(pname), params);
        }
    }
}

void APIENTRY_GL4ES gl4es_glActiveTexture( GLenum texture ) {
    DBG(SHUT_LOGD("MobileGlues-gl4es: glActiveTexture(%s)\n", PrintEnum(texture));)
...
void APIENTRY_GL4ES gl4es_glClientActiveTexture( GLenum texture ) {
    DBG(SHUT_LOGD("MobileGlues-gl4es: glClientActiveTexture(%s)\n", PrintEnum(texture));)
    int tmu = texture - GL_TEXTURE0;
    if ((tmu<0) || (tmu >= hardext.maxtex)) {
        errorShim(GL_INVALID_ENUM);
    return;
...
void APIENTRY_GL4ES gl4es_glPixelStorei(GLenum pname, GLint param) {
    ...
    errorGL();
    {} //STUB of gl4es_gles_glPixelStorei(pname, param);
}
...
void realize_1texture(GLenum target, int wantedTMU, gltexture_t* tex, glsampler_t* sampler)
{
    ...
    if(tex->actual.min_filter!=param) {
        ...
        DBG(SHUT_LOGD("MobileGlues-gl4es: Adjusting %s[%d]:Texture[%u].min_filter = %s (binded=%u)\n", PrintEnum(target), TMU, tex->glname, PrintEnum(param), glstate->actual_tex2d[TMU]);)
        if(glstate->gleshard->active!=TMU) {
            glstate->gleshard->active = TMU;
            {} //STUB of gl4es_gles_glActiveTexture(GL_TEXTURE0+TMU);
        }
        {} //STUB of gl4es_gles_glTexParameteri(target, GL_TEXTURE_MIN_FILTER, param);
    ...
    if(tex->actual.mag_filter!=param) {
        ...
        DBG(SHUT_LOGD("MobileGlues-gl4es: Adjusting %s[%d]:Texture[%u].mag_filter = %s (min=%s/%s)\n", PrintEnum(target), TMU, tex->glname, PrintEnum(param), PrintEnum(sampler->min_filter), PrintEnum(tex->actual.min_filter));)
        if(glstate->gleshard->active!=TMU) {
            glstate->gleshard->active = TMU;
            {} //STUB of gl4es_gles_glActiveTexture(GL_TEXTURE0+TMU);
        }
        {} //STUB of gl4es_gles_glTexParameteri(target, GL_TEXTURE_MAG_FILTER, param);
    ...
    if(tex->actual.wrap_s!=param) {
        ...
        DBG(SHUT_LOGD("MobileGlues-gl4es: Adjusting %s[%d]:Texture[%u].wrap_s = %s\n", PrintEnum(target), TMU, tex->glname, PrintEnum(param));)
        if(glstate->gleshard->active!=TMU) {
            glstate->gleshard->active = TMU;
            {} //STUB of gl4es_gles_glActiveTexture(GL_TEXTURE0+TMU);
        }
        {} //STUB of gl4es_gles_glTexParameteri(target, GL_TEXTURE_WRAP_S, param);
    ...
    if(tex->actual.wrap_t!=param) {
        ...
        DBG(SHUT_LOGD("MobileGlues-gl4es: Adjusting %s[%d]:Texture[%u].wrap_t = %s\n", PrintEnum(target), TMU, tex->glname, PrintEnum(param));)
        if(glstate->gleshard->active!=TMU) {
            glstate->gleshard->active = TMU;
            {} //STUB of gl4es_gles_glActiveTexture(GL_TEXTURE0+TMU);
        }
        {} //STUB of gl4es_gles_glTexParameteri(target, GL_TEXTURE_WRAP_T, param);
...
void realize_1texture_real(GLenum target, int wantedTMU, gltexture_t* tex, glsampler_t* sampler)
{
    DBG(SHUT_LOGD("MobileGlues-gl4es: realize_1texture(%s, %d, %p[%u], %p)\n", PrintEnum(target), wantedTMU, tex, tex->glname, sampler);)
    LOAD_GLES(glActiveTexture);
    void gl4es_gles_glTexParameteri(glTexParameteri_ARG_EXPAND); //LOAD_GLES(glTexParameteri);
    ...
    if(tex->actual.min_filter!=param) {
        ...
        if(glstate->gleshard->active!=TMU) {
            glstate->gleshard->active = TMU;
            gl4es_gles_glActiveTexture(GL_TEXTURE0+TMU);
        }
        gl4es_gles_glTexParameteri(target, GL_TEXTURE_MIN_FILTER, param);
        tex->actual.min_filter=param;
    }
    param = sampler->mag_filter;
    if(tex->actual.mag_filter!=param) {
        ...
        gl4es_gles_glTexParameteri(target, GL_TEXTURE_MAG_FILTER, param);
        tex->actual.mag_filter=param;
    }
    param = get_texture_wrap_s(tex, sampler);
    if(tex->actual.wrap_s!=param) {
        ...
        gl4es_gles_glTexParameteri(target, GL_TEXTURE_WRAP_S, param);
        tex->actual.wrap_s=param;
    }
    param = get_texture_wrap_t(tex, sampler);
    if(tex->actual.wrap_t!=param) {
        ...
        gl4es_gles_glTexParameteri(target, GL_TEXTURE_WRAP_T, param);
        tex->actual.wrap_t=param;
    }
    if(wantedTMU==-2) {
        if (oldtex!=tex->glname) gl4es_gles_glBindTexture(GL_TEXTURE_2D, oldtex);
...
void realize_textures_real(int drawing) {
    LOAD_GLES(glEnable);
    LOAD_GLES(glDisable);
    LOAD_GLES(glBindTexture);
    LOAD_GLES(glActiveTexture);
    void gl4es_gles_glTexParameteri(glTexParameteri_ARG_EXPAND); //LOAD_GLES(glTexParameteri);
#ifdef TEXSTREAM
    DBG(SHUT_LOGD("MobileGlues-gl4es: realize_textures(%d), glstate->bound_changed=%d, glstate->enable.texture[0]=%X glsate->actual_tex2d[0]=%u / glstate->bound_stream[0]=%u\n", drawing, glstate->bound_changed, glstate->enable.texture[0], glstate->actual_tex2d[0], glstate->bound_stream[0]);)
#else
    DBG(SHUT_LOGD("MobileGlues-gl4es: realize_textures(%d), glstate->bound_changed=%d, glstate->enable.texture[0]=%X glsate->actual_tex2d[0]=%u\n", drawing, glstate->bound_changed, glstate->enable.texture[0], glstate->actual_tex2d[0]);)
#endif
    for (int i=0; i<glstate->bound_changed; i++) {
        ...
        GLenum target = gl4es_map_tex_target(to_target(tgt));
        ...
        if(tgt!=ENABLED_CUBE_MAP) {// CUBE MAP are immediately bound
            ...
            if(glstate->bound_stream[i]) {
                realize_active();
                if(hardext.esversion<2) gl4es_gles_glDisable(GL_TEXTURE_STREAM_IMG);
                DeactivateStreaming();
            ...
            if (t!=glstate->actual_tex2d[i]
                ...
                if(glstate->gleshard->active!=i) {
                    ...
                    gl4es_gles_glActiveTexture(GL_TEXTURE0+i);
                }
#ifdef TEXSTREAM
                ...
                if(glstate->bound_stream[i]) {
                    if(hardext.esversion<2) gl4es_gles_glDisable(GL_TEXTURE_STREAM_IMG);
                    DeactivateStreaming();
                    if(hardext.esversion<2) gl4es_gles_glEnable(GL_TEXTURE_2D);
                    glstate->bound_stream[i] = 0;
                }
                if (globals4es.texstream && (streamingID>-1)) {
        ...
        if(drawing) {
            ...
            if(tex->mipmap_need && !tex->mipmap_done) {
                if(!tex->mipmap_auto) {
                    ...
                    LOAD_GLES2_OR_OES(glGenerateMipmap);
                    gl4es_gles_glGenerateMipmap(GL_TEXTURE_2D);
                }
                tex->mipmap_done = 1;
            }
        }
        realize_1texture_real(target, i, tex, glstate->samplers.sampler[i]);
    }
    ...
    for (int i=0; i<glstate->bound_changed; i++) {
        ...
        if(tgt!=ENABLED_CUBE_MAP) {// CUBE MAP are immediately bound
            ...
            if (t!=glstate->actual_tex2d[i]
                ...
                if(glstate->gleshard->active!=i) {
                    glstate->gleshard->active = i;
                    {} //STUB of gl4es_gles_glActiveTexture(GL_TEXTURE0+i);
                }
#ifdef TEXSTREAM
                int streamed = tex->streamed;
                int streamingID = tex->streamingID;
        ...
        if(drawing) {
            ...
            if(tex->mipmap_need && !tex->mipmap_done) {
                if(!tex->mipmap_auto) {
                    // should check if glGenerateMipmap exist, and fall back to no mipmap if not
                    LOAD_GLES2_OR_OES(glGenerateMipmap);
                    {} //STUB of gl4es_gles_glG enerateMipmap(GL_TEXTURE_2D);
                }
                tex->mipmap_done = 1;
            }
        }

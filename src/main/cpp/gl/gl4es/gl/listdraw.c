...
int list2VBO(renderlist_t* list)
{
    ...
    gl4es_gles_glGenBuffers(1, &list->vbo_array);
    bindBuffer_real(GL_ARRAY_BUFFER, list->vbo_array);
    gl4es_gles_glBufferData(GL_ARRAY_BUFFER, vbo_base, NULL, GL_STATIC_DRAW);
    for(int i=0; i<imax; ++i) {
        array2vbo_t *r = work+sorted[i];
        if(r->vbo_base==r->vbo_basebase)
            gl4es_gles_glBufferSubData(GL_ARRAY_BUFFER, r->vbo_basebase, r->real_size, (void*)r->real_base);
    }
    // work -> list
    imax = 0;
    if(list->vert) {
...
//SHUT_LOGD("MobileGlues-gl4es: draw_renderlist %p, size=%i, mode=%s(%s), ilen=%d, next=%p, color=%p, secondarycolor=%p fogcoord=%p\n", list, list->len, PrintEnum(list->mode), PrintEnum(list->mode_init), list->ilen, list->next, list->color, list->secondary, list->fogcoord);
    ...
    do {
        ...
        if (list->vert) {
            ...
            gl4es_gles_glVertexPointer(4, GL_FLOAT, list->vert_stride, list->vert);
        } else {
        ...
        if (list->normal) {
            ...
            gl4es_gles_glNormalPointer(GL_FLOAT, list->normal_stride, list->normal);
        } else {
        ...
        if(hardext.esversion==1) {
            for (int a=0; a<hardext.maxtex; a++) {
                ...
                if ((list->tex[a] || (use_texgen[a] && !needclean[a]))/* && glstate->enable.texture[a]*/) {
                    ...
                    gl4es_gles_glTexCoordPointer(4, GL_FLOAT, (use_texgen[a])?0:list->tex_stride[a], (use_texgen[a])?glstate->texgened[a]:list->tex[a]);
                } else {
...
//else if (!glstate->enable.texgen_s[a] && glstate->enable.texture[a]) SHUT_LOGD("MobileGlues-gl4es: LIBGL: texture[%i] without TexCoord, mode=0x%04X (init=0x%04X), listlen=%i\n", a, list->mode, list->mode_init, list->len);
            ...
            for (int a=0; a<hardext.maxtex; a++) {
                if(list->tex[a]) {
                    TEXTURE(a);
                    fpe_glEnableClientState(GL_TEXTURE_COORD_ARRAY);
                    gl4es_gles_glTexCoordPointer(4, GL_FLOAT, list->tex_stride[a], list->tex[a]);
                } else {
                    TEXTURE(a);
                    fpe_glDisableClientState(GL_TEXTURE_COORD_ARRAY);
                }
        ...
        if (indices) {
            if (glstate->render_mode == GL_SELECT) {
                ...
                if (glstate->polygon_mode == GL_LINE && list->mode_init>=GL_TRIANGLES) {
                    ...
                    gl4es_gles_glDrawElements(mode, list->ind_line, GL_UNSIGNED_SHORT, list->ind_lines);
                    use_vbo_indices = 1;
                } else {
                    int vbo_indices = 0;
                    if(!use_vbo_indices) {
                        ...
                        LOAD_GLES2(glGenBuffers);
                        LOAD_GLES2(glBufferData);
                        gl4es_gles_glGenBuffers(1, &list->vbo_indices);
                        bindBuffer_real(GL_ELEMENT_ARRAY_BUFFER, list->vbo_indices);
                        gl4es_gles_glBufferData(GL_ELEMENT_ARRAY_BUFFER, list->ilen*sizeof(GLushort), indices, GL_STATIC_DRAW);
                        use_vbo_indices = 2;
                        vbo_indices = 1;
            ...
            if (glstate->render_mode == GL_SELECT) {	
                ...
                if ((glstate->polygon_mode == GL_LINE) && (list->mode_init>=GL_TRIANGLES)) {
                    ...
                    if(list->instanceCount==1)
                        gl4es_gles_glDrawArrays(mode, 0, len);
                    else {
                        for (glstate->instanceID=0; glstate->instanceID<list->instanceCount; ++glstate->instanceID)
                            gl4es_gles_glDrawArrays(mode, 0, len);
                        glstate->instanceID = 0;

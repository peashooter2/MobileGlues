...
void APIENTRY_GL4ES gl4es_glCopyTexImage2D(GLenum target,  GLint level,  GLenum internalformat,  GLint x,  GLint y,  
    ...
    if (globals4es.skiptexcopies) {
        DBG(SHUT_LOGD("MobileGlues-gl4es: glCopyTexImage2D skipped.\n"));
        return;
    }
    ...
    if(glstate->fbo.current_fb->read_type==0) {
      ...
      gl4es_gles_glGetIntegerv(GL_IMPLEMENTATION_COLOR_READ_FORMAT_OES, (GLint *) &glstate->fbo.current_fb->read_format);
      gl4es_gles_glGetIntegerv(GL_IMPLEMENTATION_COLOR_READ_TYPE_OES, (GLint *) &glstate->fbo.current_fb->read_type);
    }
    ...
    if (copytex) {
        ...
        {} //STUB of gl4es_gles_glCopyTexImage2D(target, level, fmt, x, y, width, height, border);
    } else {
...
void APIENTRY_GL4ES gl4es_glCopyTexSubImage2D(GLenum target, GLint level, GLint xoffset, GLint yoffset,
...
#endif
    {
        ...
        if (copytex || !glstate->colormask[0] || !glstate->colormask[1] || !glstate->colormask[2] || !glstate->colormask[3]) {
            {} //STUB of gl4es_gles_glCopyTexSubImage2D(target, level, xoffset, yoffset, x, y, width, height);
            if(((((bound->max_level == level) && (level || bound->mipmap_need)) && (globals4es.automipmap!=3) && (bound->mipmap_need!=0))) && !(bound->max_level==bound->base_level && bound->base_level==0)) {
                ...
                if(gl4es_gles_glGenerateMipmap)
                    {} //STUB of gl4es_gles_glG enerateMipmap(to_target(itarget));
            }
        } else {
            void* tmp = malloc(width*height*4);
...
void APIENTRY_GL4ES gl4es_glReadPixels(GLint x, GLint y, GLsizei width, GLsizei height, GLenum format, GLenum type, GLvoid * data) {
    DBG(SHUT_LOGD("MobileGlues-gl4es: glReadPixels(%i, %i, %i, %i, %s, %s, 0x%p)\n", x, y, width, height, PrintEnum(format), PrintEnum(type), data);)
    FLUSH_BEGINEND;
    ...
    {
        // easy passthru
        {} //STUB of gl4es_gles_glReadPixels(x, y, width, height, format, type, dst);
        readfboEnd();
        return;
    }
    ...
    {} //STUB of gl4es_gles_glReadPixels(x, y, width, height, use_bgra?GL_BGRA:GL_RGBA, GL_UNSIGNED_BYTE, pixels);
    if (! GL4ES_pixel_convert(pixels, &dst, width, height,
                        use_bgra?GL_BGRA:GL_RGBA, GL_UNSIGNED_BYTE, format, type, 0,glstate->texture.pack_align)) {
        LOGE("MobileGlues-gl4es: ReadPixels error: (%s, UNSIGNED_BYTE -> %s, %s )\n",
...
void APIENTRY_GL4ES gl4es_glGetTexImage(GLenum target, GLint level, GLenum format, GLenum type, GLvoid * img) {
    ...
    if (level != 0) {
        //SHUT_LOGD("MobileGlues-gl4es: STUBBED glGetTexImage with level=%i\n", level);
        void* tmp = malloc(width*height*GL4ES_pixel_sizeof(format, type)); // tmp space...
        void* tmp2;
        gl4es_glGetTexImage(gl4es_map_tex_target(target), 0, format, type, tmp);
        for (int i=0; i<level; i++) {
            GL4ES_pixel_halfscale(tmp, &tmp2, width, height, format, type);
            free(tmp);
            tmp = tmp2;
            width = nlevel(width, 1);
            height = nlevel(height, 1);
        }
        memcpy(img, tmp, width*height*GL4ES_pixel_sizeof(format, type));
        free(tmp);
        return;
    }
    ...
    if (globals4es.texstream && bound->streamed) {
        noerrorShim();
        GL4ES_pixel_convert(GetStreamingBuffer(bound->streamingID), &dst, width, height, GL_RGB, GL_UNSIGNED_SHORT_5_6_5, format, type, 0, glstate->texture.unpack_align);
        readfboEnd();
        return;
    }
#endif
    if (globals4es.texcopydata && bound->data) {
        //SHUT_LOGD("MobileGlues-gl4es: texcopydata* glGetTexImage(0x%04X, %d, 0x%04x, 0x%04X, %p)\n", target, level, format, type, img);
        noerrorShim();
        if (!GL4ES_pixel_convert(bound->data, &dst, width, height, GL_RGBA, GL_UNSIGNED_BYTE, format, type, 0, glstate->texture.pack_align))
            SHUT_LOGD("MobileGlues-gl4es: LIBGL: Error on GL4ES_pixel_convert while glGetTexImage\n");
    } else {
        ...
        if((bound->format==GL_RGBA || bound->format==GL_RGB || (bound->format==GL_BGRA && hardext.bgra8888) || bound->format==GL_ALPHA) && (shrink==0)) {
            gl4es_glGenFramebuffers(1, &fbo, 0);
            ...
            gl4es_glBindFramebuffer(GL_FRAMEBUFFER_OES, fbo);
            GLuint temptex;
            gl4es_glGenTextures(1, &temptex, 0); // TODO: this may be non-fpe feature, so skip now

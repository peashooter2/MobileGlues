...
void readfboBegin() {
    ...
    {} //STUB of gl4es_gles_glBindFramebuffer(GL_FRAMEBUFFER, fbo);
}

void readfboEnd() {
    if (glstate->fbo.fbo_read->id == glstate->fbo.fbo_draw->id)
        return;
    DBG(SHUT_LOGD("MobileGlues-gl4es: readfboEnd, fbo status read=%p, draw=%p, main=%u, current=%p\n", glstate->fbo.fbo_read, glstate->fbo.fbo_draw, glstate->fbo.mainfbo_fbo, glstate->fbo.current_fb);)
...
void APIENTRY_GL4ES gl4es_glGenFramebuffers(GLsizei n, GLuint *ids, GLuint* real_fbos) {
    DBG(SHUT_LOGD("MobileGlues-gl4es: glGenFramebuffers(%i, %p)\n", n, ids);)
    LOAD_GLES2_OR_OES(glGenFramebuffers);
    GLsizei m = 0;
    while(glstate->fbo.old && (glstate->fbo.old->nbr>0) && (n-m>0)) {
        DBG(SHUT_LOGD("MobileGlues-gl4es: Recycled 1 FBO\n");)
        ids[m++] = glstate->fbo.old->fbos[--glstate->fbo.old->nbr];
    }
    noerrorShim();
    if(n-m) {
        errorGL();
        {} //STUB of gl4es_gles_glG enFramebuffers(n-m, ids+m);
        if (real_fbos) {
            for(int i=0; i<n-m; ++i)
                ids[i+m] = real_fbos[i];
        }
...
void APIENTRY_GL4ES gl4es_glDeleteFramebuffers(GLsizei n, GLuint *framebuffers) {
    DBG(SHUT_LOGD("MobileGlues-gl4es: glDeleteFramebuffers(%i, %p), framebuffers[0]=%u\n", n, framebuffers, framebuffers[0]);)
    // delete tracking
            ...
            for (int i = 0; i < n; i++) {
                ...
                if(t) {
                    ...
                    if (k != kh_end(glstate->fbo.framebufferlist)) {
                        ...
                        // detach texture...
                        for(int j=0; j<GL4ES_MAX_DRAW_BUFFERS; ++j) {
                            if(fb->color[j] && fb->t_color[j]!=GL_RENDERBUFFER) {
                                gltexture_t *tex = gl4es_getTexture(fb->t_color[j], fb->color[j] , 0, 0);
                                if(tex) {
                                    tex->binded_fbo = 0;
                        ...
                        if(fb->depth && fb->t_depth!=GL_RENDERBUFFER) {
                            gltexture_t *tex = gl4es_getTexture(fb->t_depth, fb->depth,0,0);
                            if(tex) {
                                tex->binded_fbo = 0;
                                tex->binded_attachment = 0;
                                tex->renderdepth = 0;
                        ...
                        if(fb->stencil && fb->t_stencil!=GL_RENDERBUFFER) {
                            gltexture_t *tex = gl4es_getTexture(fb->t_stencil, fb->stencil,0,0);
                            if(tex) {
                                tex->binded_fbo = 0;
                                tex->binded_attachment = 0;
                                tex->renderstencil = 0;
    ...
    if (globals4es.recyclefbo) {
        DBG(SHUT_LOGD("MobileGlues-gl4es: Recycling %i FBOs\n", n);)
        noerrorShim();
        ...
        errorGL();
        {} //STUB of gl4es_gles_glDeleteFramebuffers(n, framebuffers);
    }
}

GLboolean APIENTRY_GL4ES gl4es_glIsFramebuffer(GLuint framebuffer) {
    DBG(SHUT_LOGD("MobileGlues-gl4es: glIsFramebuffer(%u)\n", framebuffer);)
...
GLenum APIENTRY_GL4ES gl4es_glCheckFramebufferStatus(GLenum target) {
    ...
    if(glstate->fbo.internal) {
        ...
        result = gl4es_gles_glCheckFramebufferStatus(rtarget);
     }
    DBG(SHUT_LOGD("MobileGlues-gl4es: glCheckFramebufferStatus(0x%04X)=0x%04X\n", target, result);)
    return result;
}

void APIENTRY_GL4ES gl4es_glBindFramebuffer(GLenum target, GLuint framebuffer) {
    DBG(SHUT_LOGD("MobileGlues-gl4es: glBindFramebuffer(%s, %u), list=%s, glstate->fbo.current_fb=%d (draw=%d, read=%d)\n", PrintEnum(target), framebuffer, glstate->list.active?"active":"none", glstate->fbo.current_fb->id, glstate->fbo.fbo_draw->id, glstate->fbo.fbo_read->id);)
...
//    LOAD_GLES2_OR_OES(glCheckFramebufferStatus);
    ...
    {} //STUB of gl4es_gles_glBindFramebuffer(target, framebuffer);
    GLenum err=gl4es_gles_glGetError();
    errorShim(err);
    
//    glstate->fbo.fb_status = (framebuffer==0)?GL_FRAMEBUFFER_COMPLETE:gl4es_gles_glCheckFramebufferStatus(target);
}
...
GLenum ReadDraw_Push(GLenum target) {
    ...
    if(target==GL_DRAW_FRAMEBUFFER) {
        if(glstate->fbo.current_fb!=glstate->fbo.fbo_draw)
            {} //STUB of gl4es_gles_glBindFramebuffer(GL_FRAMEBUFFER, (glstate->fbo.fbo_draw->id)?glstate->fbo.fbo_draw->id:glstate->fbo.mainfbo_fbo);
        return GL_FRAMEBUFFER;
    }
    if(target==GL_READ_FRAMEBUFFER) {
        if(glstate->fbo.current_fb!=glstate->fbo.fbo_read)
            {} //STUB of gl4es_gles_glBindFramebuffer(GL_FRAMEBUFFER, (glstate->fbo.fbo_read->id)?glstate->fbo.fbo_read->id:glstate->fbo.mainfbo_fbo);
        return GL_FRAMEBUFFER;
    }
    return target;
}
void ReadDraw_Pop(GLenum target) {
    ...
    if(target==GL_DRAW_FRAMEBUFFER && glstate->fbo.current_fb!=glstate->fbo.fbo_draw) {
        {} //STUB of gl4es_gles_glBindFramebuffer(GL_FRAMEBUFFER, (glstate->fbo.current_fb->id)?glstate->fbo.current_fb->id:glstate->fbo.mainfbo_fbo);
    }
...
void APIENTRY_GL4ES gl4es_glFramebufferTexture2D(GLenum target, GLenum attachment, GLenum textarget, GLuint texture, GLint level) {
    ...
    if (texture) {
        tex = gl4es_getTexture(textarget, texture,0,0);

        if (!tex) {
            LOGE("MobileGlues-gl4es: texture for FBO not found, name=%u\n", texture);
        } else {
            texture = tex->glname;
            ...
            if (globals4es.fbtexscale > 0.0f || tex->shrink || tex->useratio || (tex->adjust && (hardext.npot==1 || hardext.npot==2) && !globals4es.potframebuffer)) {
                ...
                if(oldactive) gl4es_gles_glActiveTexture(GL_TEXTURE0);
                gltexture_t *bound = glstate->texture.bound[0/*glstate->texture.active*/][ENABLED_TEX2D];
                GLuint oldtex = bound->glname;
                if (oldtex!=tex->glname) gl4es_gles_glBindTexture(GL_TEXTURE_2D, tex->glname);
                {} //STUB of gl4es_gles_glTexImage2D(GL_TEXTURE_2D, 0, tex->format, tex->nwidth, tex->nheight, 0, tex->format, tex->type, NULL);
                if (oldtex!=tex->glname) gl4es_gles_glBindTexture(GL_TEXTURE_2D, oldtex);
                if(oldactive) gl4es_gles_glActiveTexture(GL_TEXTURE0+oldactive);
            }
            ...
            if(need_change) {
                ...
                LOGD("MobileGlues-gl4es: Recreate a texture for a FBO (%s%s%s)\n", (need_change&1)?"POT":"", (need_change==3)?" & ":"", (need_change&2)?"Format/Type":"");
                if(need_change&1) {
 ...
 /*           if ((tex->width<32) || (tex->height<32)) {
                SHUT_LOGD("MobileGlues-gl4es: LIBGL: enlarging too-small texture for FBO\n");
    ...
    if(old_attachment) {
        gltexture_t* old = gl4es_getTexture(old_attachment_type, old_attachment,0,0);
        if(old) {
            DBG(SHUT_LOGD("MobileGlues-gl4es: Detach Texture %d from FBO %d as Attachement %s\n", old->glname, old->binded_fbo, PrintEnum(old->binded_attachment));)
            old->binded_fbo = 0;
            old->binded_attachment = 0;
        } else {
            DBG(SHUT_LOGD("MobileGlues-gl4es: Old attachement %s for FBO %d not found in textures\n", PrintEnum(attachment), fb->id);)
        }
    }

    if(tex) {
        DBG(SHUT_LOGD("MobileGlues-gl4es: Attach Texture %d to FBO %d as Attachement %s\n", tex->glname, fb->id, PrintEnum(attachment));)
    ...
    if(attachment>=GL_COLOR_ATTACHMENT0 && attachment<(GL_COLOR_ATTACHMENT0+hardext.maxcolorattach) && tex) {
        ...
        realize_1texture(gl4es_map_tex_target(textarget), -1, tex, NULL);
    }
    ...
    if(attachment==GL_DEPTH_ATTACHMENT /*&& hardext.depthtex==0*/) {
        ...
        if(hardext.depthtex && (tex || !texture)) {
            ...
            if(tex && !(tex->format==GL_DEPTH_COMPONENT || tex->format==GL_DEPTH_STENCIL)) {
                ...
                GLuint oldtex = bound->glname;
                if (oldtex!=tex->glname) gl4es_gles_glBindTexture(GL_TEXTURE_2D, tex->glname);
                {} //STUB of gl4es_gles_glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
                {} //STUB of gl4es_gles_glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
                {} //STUB of gl4es_gles_glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
                {} //STUB of gl4es_gles_glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);
                {} //STUB of gl4es_gles_glTexImage2D(GL_TEXTURE_2D, 0, tex->format, tex->nwidth, tex->nheight, 0, tex->format, tex->type, NULL);
                if (oldtex!=tex->glname) gl4es_gles_glBindTexture(GL_TEXTURE_2D, oldtex);
                if(oldactive) gl4es_gles_glActiveTexture(GL_TEXTURE0+oldactive);
            }
            // bind the depth texture...
            {} //STUB of gl4es_gles_glFramebufferTexture2D(ntarget, attachment, GL_TEXTURE_2D, texture, 0);
        } else {
            // let's create a renderbuffer and attach it instead of the (presumably) depth texture
            if(tex && !tex->renderdepth) {
                gl4es_glGenRenderbuffers(1, &tex->renderdepth, NULL);
        ...
        if((tex || !texture) && (hardext.stenciltex || (hardext.depthtex && hardext.depthstencil))) {
            ...
            if(tex && !(tex->format==GL_STENCIL_INDEX8 || tex->format==GL_DEPTH_STENCIL)) {
                if(tex->format==GL_DEPTH_ATTACHMENT) {
                    ...
                    gl4es_glGenTextures(1, &texture, 0, 0); // TODO: this is non-fpe feature, so just skip it
                    realize_textures(0);
                    int oldactive = glstate->texture.active;
                    if(oldactive) gl4es_gles_glActiveTexture(GL_TEXTURE0);
                    gltexture_t *bound = glstate->texture.bound[0/*glstate->texture.active*/][ENABLED_TEX2D];
            ...
            if(tex && !tex->renderstencil) {
                gl4es_glGenRenderbuffers(1, &tex->renderstencil, NULL);
    ...
    if(attachment==GL_DEPTH_STENCIL_ATTACHMENT /*&& hardext.depthtex==0*/) {
        ...
        if(hardext.depthstencil /*&& !(hardext.depthtex && tex)*/) {
            if(hardext.depthtex && (tex || !texture)) {
                ...
                {} //STUB of gl4es_gles_glFramebufferTexture2D(ntarget, GL_DEPTH_ATTACHMENT, GL_TEXTURE_2D, texture, 0);
                {} //STUB of gl4es_gles_glFramebufferTexture2D(ntarget, GL_STENCIL_ATTACHMENT, GL_TEXTURE_2D, texture, 0);
            } else {
                if(tex && !tex->renderdepth) {
                    gl4es_glGenRenderbuffers(1, &tex->renderdepth, NULL);
                    gl4es_glBindRenderbuffer(GL_RENDERBUFFER, tex->renderdepth);
    ...
    if (level!=0) {
        ...
        if(!scrap_tex)
            gl4es_glGenTextures(1, &scrap_tex, 0, 0);  // TODO: this is non-fpe feature, so just skip it
        if ((scrap_width!=twidth) || (scrap_height!=theight)) {
                scrap_width = twidth;
                scrap_height = theight;
                gltexture_t *bound = glstate->texture.bound[glstate->texture.active][ENABLED_TEX2D];
                GLuint oldtex = bound->glname;
                if (oldtex!=scrap_tex) gl4es_gles_glBindTexture(GL_TEXTURE_2D, scrap_tex);
                {} //STUB of gl4es_gles_glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, scrap_width, scrap_height, 0, GL_RGBA, GL_UNSIGNED_BYTE, NULL);
                if (oldtex!=scrap_tex) gl4es_gles_glBindTexture(GL_TEXTURE_2D, oldtex);
        }
        texture = scrap_tex;
    }
    ...
    {} //STUB of gl4es_gles_glFramebufferTexture2D(ntarget, attachment, realtarget, texture, 0);
    DBG(CheckGLError(1);)
    ReadDraw_Pop(target);
}
...
void gl4es_glGenRenderbuffers(GLsizei n, GLuint *renderbuffers, GLuint *real_renderbuffers) {
    DBG(SHUT_LOGD("MobileGlues-gl4es: glGenRenderbuffers(%i, %p)\n", n, renderbuffers);)
    LOAD_GLES2_OR_OES(glGenRenderbuffers);
    errorGL();
    if (real_renderbuffers)
        for (int i=0; i<n; ++i) {
            renderbuffers[i] = real_renderbuffers[i];
        }
    {} //STUB of gl4es_gles_glG enRenderbuffers(n, renderbuffers);
...
void APIENTRY_GL4ES gl4es_glFramebufferRenderbuffer(GLenum target, GLenum attachment, GLenum renderbuffertarget, GLuint renderbuffer) {
    ...
    if (attachment >= GL_COLOR_ATTACHMENT0 && (attachment < (GL_COLOR_ATTACHMENT0+hardext.maxcolorattach)) && globals4es.fboforcetex) {
        if(rend->renderbuffer) {
            ...
            if(!rend->secondarytexture) {
                GLuint newtex;
                gl4es_glGenTextures(1, &newtex, 0, 0);  // TODO: this is non-fpe feature, so just skip it
    ...
    errorGL();
    {} //STUB of gl4es_gles_glFramebufferRenderbuffer(ntarget, attachment, renderbuffertarget, renderbuffer);
    DBG(CheckGLError(1);)
    ReadDraw_Pop(target);
}

void APIENTRY_GL4ES gl4es_glDeleteRenderbuffers(GLsizei n, GLuint *renderbuffers) {
    DBG(SHUT_LOGD("MobileGlues-gl4es: glDeleteRenderbuffer(%d, %p)\n", n, renderbuffers);)
                ...
                if(t) {
                    ...
                    if (k != kh_end(glstate->fbo.renderbufferlist)) {
                        ...
                        if(rend->secondarybuffer)
                            {} //STUB of gl4es_gles_glDeleteRenderbuffers(1, &rend->secondarybuffer);
                        if(rend->secondarytexture)
...
void APIENTRY_GL4ES gl4es_glRenderbufferStorage(GLenum target, GLenum internalformat, GLsizei width, GLsizei height) {
    ...
    if ((internalformat == GL_DEPTH24_STENCIL8 && (hardext.depthstencil==0 || ((hardext.vendor&VEND_IMGTEC)==VEND_IMGTEC)))) {
        ...
        if(!rend->secondarybuffer) {
            // TODO: impl glRenderbufferStorage in MobileGlues first
            {} //STUB of gl4es_gles_glG enRenderbuffers(1, &rend->secondarybuffer);
        }
    ...
    if(rend->secondarybuffer) {
        if(use_secondarybuffer) {
            ...
            {} //STUB of gl4es_gles_glBindRenderbuffer(GL_RENDERBUFFER, rend->secondarybuffer);
            {} //STUB of gl4es_gles_glRenderbufferStorage(target, GL_STENCIL_INDEX8, width, height);
            {} //STUB of gl4es_gles_glBindRenderbuffer(GL_RENDERBUFFER, current_rb);
        } else {
            LOAD_GLES2_OR_OES(glDeleteRenderbuffers);
            {} //STUB of gl4es_gles_glDeleteRenderbuffers(1, &rend->secondarybuffer);
    ...
    if(rend->secondarytexture) {
        ...
        gltexture_t *tex = gl4es_getTexture(GL_TEXTURE_2D, rend->secondarytexture,0,0);
        LOAD_GLES(glActiveTexture);
        LOAD_GLES(glBindTexture);
        LOAD_GLES(glTexImage2D);
        int oldactive = glstate->texture.active;
        ...
        gltexture_t *bound = glstate->texture.bound[0/*glstate->texture.active*/][ENABLED_TEX2D];
        GLuint oldtex = bound->glname;
        if (oldtex!=rend->secondarytexture) gl4es_gles_glBindTexture(GL_TEXTURE_2D, rend->secondarytexture);
    ...
    {} //STUB of gl4es_gles_glRenderbufferStorage(target, internalformat, width, height);
    DBG(CheckGLError(1);)
}
...
void APIENTRY_GL4ES gl4es_glBindRenderbuffer(GLenum target, GLuint renderbuffer) {
    DBG(SHUT_LOGD("MobileGlues-gl4es: glBindRenderbuffer(%s, %u), binded Fbo=%u\n", PrintEnum(target), renderbuffer, glstate->fbo.current_fb->id);)
    LOAD_GLES2_OR_OES(glBindRenderbuffer);
...
void APIENTRY_GL4ES gl4es_glGenerateMipmap(GLenum target) {
    ...
    if(globals4es.automipmap != 3) {
        {} //STUB of gl4es_gles_glG enerateMipmap(rtarget);
        bound->mipmap_auto = 1;
...
void APIENTRY_GL4ES gl4es_glGetFramebufferAttachmentParameteriv(GLenum target, GLenum attachment, GLenum pname, GLint *params) {
    DBG(SHUT_LOGD("MobileGlues-gl4es: glGetFramebufferAttachmentParameteriv(%s, %s, %s, %p)\n", PrintEnum(target), PrintEnum(attachment), PrintEnum(pname), params);)
    LOAD_GLES2_OR_OES(glGetFramebufferAttachmentParameteriv);
    ...
    */
    ...
    if(attachment==GL_DEPTH_ATTACHMENT && pname==GL_FRAMEBUFFER_ATTACHMENT_DEPTH_SIZE) {
        ...
        int depth, stencil;
        GLenum ntarget = ReadDraw_Push(target);
      gl4es_gles_glGetFramebufferAttachmentParameteriv(ntarget, GL_STENCIL_ATTACHMENT, GL_FRAMEBUFFER_ATTACHMENT_STENCIL_SIZE, &stencil);
      gl4es_gles_glGetFramebufferAttachmentParameteriv(ntarget, GL_DEPTH_ATTACHMENT, GL_FRAMEBUFFER_ATTACHMENT_DEPTH_SIZE, &depth);
        errorGL();
        ReadDraw_Pop(target);
    ...
    errorGL();
  gl4es_gles_glGetFramebufferAttachmentParameteriv(ntarget, attachment, pname, params);
    ReadDraw_Pop(target);
}

void APIENTRY_GL4ES gl4es_glGetRenderbufferParameteriv(GLenum target, GLenum pname, GLint * params) {
    DBG(SHUT_LOGD("MobileGlues-gl4es: glGetRenderbufferParameteriv(%s, %s, %p)\n", PrintEnum(target), PrintEnum(pname), params);)
...
void createMainFBO(int width, int height) {
    ...
    DBG(SHUT_LOGD("MobileGlues-gl4es: LIBGL: Create FBO of %ix%i 32bits\n", width, height);)
    // switch to texture unit 0 if needed
    if (glstate->texture.active != 0)
        {} //STUB of gl4es_gles_glActiveTexture(GL_TEXTURE0);
    if (glstate->texture.client != 0 && gl4es_gles_glClientActiveTexture)
    ...
    // create the texture
    if(createIt)
        {} //STUB of gl4es_gles_glGenTextures(1, &glstate->fbo.mainfbo_tex);
    {} //STUB of gl4es_gles_glBindTexture(GL_TEXTURE_2D, glstate->fbo.mainfbo_tex);
    if(createIt) {
        {} //STUB of gl4es_gles_glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
        {} //STUB of gl4es_gles_glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
        {} //STUB of gl4es_gles_glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
    ...
    {} //STUB of gl4es_gles_glTexImage2D(GL_TEXTURE_2D, 0, globals4es.fbo_noalpha?GL_RGB:GL_RGBA, width, height, 0, globals4es.fbo_noalpha?GL_RGB:GL_RGBA, GL_UNSIGNED_BYTE, NULL);
    {} //STUB of gl4es_gles_glBindTexture(GL_TEXTURE_2D, 0);
    // create the render buffers
    if(createIt) {
        {} //STUB of gl4es_gles_glGenRenderbuffers(1, &glstate->fbo.mainfbo_dep);
        {} //STUB of gl4es_gles_glGenRenderbuffers(1, &glstate->fbo.mainfbo_ste);
    }
    {} //STUB of gl4es_gles_glBindRenderbuffer(GL_RENDERBUFFER, glstate->fbo.mainfbo_ste);
    {} //STUB of gl4es_gles_glRenderbufferStorage(GL_RENDERBUFFER, GL_STENCIL_INDEX8, width, height);
    {} //STUB of gl4es_gles_glBindRenderbuffer(GL_RENDERBUFFER, glstate->fbo.mainfbo_dep);
    {} //STUB of gl4es_gles_glRenderbufferStorage(GL_RENDERBUFFER, GL_DEPTH_COMPONENT24, width, height);
    {} //STUB of gl4es_gles_glBindRenderbuffer(GL_RENDERBUFFER, 0);
    // create a fbo
    if(createIt)
        {} //STUB of gl4es_gles_glGenFramebuffers(1, &glstate->fbo.mainfbo_fbo);
    {} //STUB of gl4es_gles_glBindFramebuffer(GL_FRAMEBUFFER, glstate->fbo.mainfbo_fbo);
    
    // re-attach, even if not creating the fbo...
    {} //STUB of gl4es_gles_glFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_STENCIL_ATTACHMENT, GL_RENDERBUFFER, glstate->fbo.mainfbo_ste);
    {} //STUB of gl4es_gles_glFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, GL_RENDERBUFFER, glstate->fbo.mainfbo_dep);
    
    {} //STUB of gl4es_gles_glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, glstate->fbo.mainfbo_tex, 0);

    GLenum status = gl4es_gles_glCheckFramebufferStatus(GL_FRAMEBUFFER);

    {} //STUB of gl4es_gles_glBindFramebuffer(GL_FRAMEBUFFER, 0);

    // Put everything back
    {} //STUB of gl4es_gles_glBindTexture(GL_TEXTURE_2D, glstate->texture.bound[0][ENABLED_TEX2D]->glname);
    if (glstate->texture.active != 0)
        {} //STUB of gl4es_gles_glActiveTexture(GL_TEXTURE0 + glstate->texture.active);
    if (glstate->texture.client != 0 && gl4es_gles_glClientActiveTexture)
        {} //STUB of gl4es_gles_glClientActiveTexture(GL_TEXTURE0 + glstate->texture.client);
    GLuint current_rb = glstate->fbo.current_rb->renderbuffer;
    {} //STUB of gl4es_gles_glBindRenderbuffer(GL_RENDERBUFFER, current_rb);
    // Final check, and bind the fbo for future use
    if (status != GL_FRAMEBUFFER_COMPLETE) {
        SHUT_LOGD("MobileGlues-gl4es: LIBGL: Error while creating main fbo (0x%04X)\n", status);
        deleteMainFBO(glstate);
        {} //STUB of gl4es_gles_glBindFramebuffer(GL_FRAMEBUFFER, glstate->fbo.current_fb->id);
        
    } else {
        {} //STUB of gl4es_gles_glBindFramebuffer(GL_FRAMEBUFFER, (glstate->fbo.current_fb->id)?glstate->fbo.current_fb->id:glstate->fbo.mainfbo_fbo);
        // clear color, depth and stencil...
        if (glstate->fbo.current_fb->id==0)
            {} //STUB of gl4es_gles_glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT | GL_STENCIL_BUFFER_BIT);
    }
...
void bindMainFBO() {
    ...
    if (glstate->fbo.current_fb->id==0) {
        {} //STUB of gl4es_gles_glBindFramebuffer(GL_FRAMEBUFFER, glstate->fbo.mainfbo_fbo);
        //gl4es_gles_glCheckFramebufferStatus(GL_FRAMEBUFFER);
    }
}
...
void deleteMainFBO(void *state) {
    ...
    if (glstate->fbo.mainfbo_dep) {
        {} //STUB of gl4es_gles_glDeleteRenderbuffers(1, &glstate->fbo.mainfbo_dep);
        glstate->fbo.mainfbo_dep = 0;
    }
    if (glstate->fbo.mainfbo_ste) {
        {} //STUB of gl4es_gles_glDeleteRenderbuffers(1, &glstate->fbo.mainfbo_ste);
        glstate->fbo.mainfbo_ste = 0;
    }
    if (glstate->fbo.mainfbo_tex) {
        {} //STUB of gl4es_gles_glDeleteTextures(1, &glstate->fbo.mainfbo_tex);
        glstate->fbo.mainfbo_tex = 0;
    }
    if (glstate->fbo.mainfbo_fbo) {
        {} //STUB of gl4es_gles_glDeleteFramebuffers(1, &glstate->fbo.mainfbo_fbo);
...
void APIENTRY_GL4ES gl4es_glBlitFramebuffer(GLint srcX0, GLint srcY0, GLint srcX1, GLint srcY1, GLint dstX0, GLint dstY0, GLint dstX1, GLint dstY1, GLbitfield mask, GLenum filter) {
    ...
    DBG(SHUT_LOGD("MobileGlues-gl4es: glBlitFramebuffer(%d, %d, %d, %d,  %d, %d, %d, %d,  0x%04X, %s) fbo_read=%d, fbo_draw=%d\n",
    ...
    DBG(SHUT_LOGD("MobileGlues-gl4es:    blit: created=%d, texture=%u, oldtex=%d\n", created, texture, oldtex);)
    if (oldtex)
    ...
    if(texture==glstate->fbo.mainfbo_tex) {
        ...
        gltexture_t *tex = gl4es_getTexture(GL_TEXTURE_2D, texture,0,0);
        if(tex) {
            nwidth = tex->nwidth;
            nheight = tex->nheight;
            glname = tex->glname;
...
void APIENTRY_GL4ES gl4es_glDrawBuffers(GLsizei n, const GLenum *bufs) {
    DBG(SHUT_LOGD("MobileGlues-gl4es: glDrawBuffers(%d, %p) [0]=%s\n", n, bufs, n?PrintEnum(bufs[0]):"nil");)
    if(hardext.drawbuffers) {
        LOAD_GLES_EXT(glDrawBuffers);
        {} //STUB of gl4es_gles_glDrawBuffers(n, bufs);
        errorGL();
    } else {
    ...
    switch(buffer) {
        case GL_COLOR:
            ...
            if(!(attch>=GL_COLOR_ATTACHMENT0 && attch<GL_COLOR_ATTACHMENT0+hardext.maxdrawbuffers)) {
                ...
                // select the buffer...
                if(hardext.drawbuffers)
                    {} //STUB of gl4es_gles_glDrawBuffers(1, (const GLenum *) &drawbuffer);
                ...
                if(hardext.drawbuffers)
                    {} //STUB of gl4es_gles_glDrawBuffers(glstate->fbo.fbo_draw->n_draw, glstate->fbo.fbo_draw->drawbuff);
                return;
            }
            break;
        case GL_STENCIL:
...
void gl4es_restoreCurrentFBO()
{
    ...
    if(framebuffer) {
        LOAD_GLES2_OR_OES(glBindFramebuffer);
        {} //STUB of gl4es_gles_glBindFramebuffer(GL_FRAMEBUFFER, framebuffer);
    }
}

...
/*    if (!norm) {
        SHUT_LOGD("MobileGlues-gl4es: LIBGL: GL_SPHERE_MAP without Normals\n");
        return;
    }*/
...
void reflection_loop(const GLfloat *verts, const GLfloat *norm, GLfloat *out, GLint count, GLushort *indices) {
    // based on https://www.opengl.org/wiki/Mathematics_of_glTexGen
/*    if (!norm) {
        SHUT_LOGD("MobileGlues-gl4es: LIBGL: GL_REFLECTION_MAP without Normals\n");
        return;
    }*/
    GLfloat InvModelview[16];
...
static inline void tex_coord_loop(GLfloat *verts, GLfloat *norm, GLfloat *out, GLint count, GLenum type, GLfloat *param_o, GLfloat *param_e, GLushort *indices) {
    switch (type) {
        ...
        case GL_EYE_LINEAR:
            ...
            break;
        case GL_SPHERE_MAP:
            //SHUT_LOGD("MobileGlues-gl4es: LIBGL: GL_SPHERE_MAP with only 1 TexGen available");  //Broken here
            break;
    }
}
...
//SHUT_LOGD("MobileGlues-gl4es: gen_tex_coords(%p, %p, %p, %d, %p, %d, %p, %d) texgen = S:%s T:%s R:%s Q:%s, enabled:%c%c%c%c, tex=%02X\n", verts, norm, *coords, count, needclean, texture, indices, ilen, (glstate->enable.texgen_s[texture])?PrintEnum(glstate->texgen[texture].S):"-", (glstate->enable.texgen_t[texture])?PrintEnum(glstate->texgen[texture].T):"-", (glstate->enable.texgen_r[texture])?PrintEnum(glstate->texgen[texture].R):"-", (glstate->enable.texgen_q[texture])?PrintEnum(glstate->texgen[texture].Q):"-", (glstate->enable.texgen_s[texture])?'S':'-', (glstate->enable.texgen_t[texture])?'T':'-', (glstate->enable.texgen_r[texture])?'R':'-', (glstate->enable.texgen_q[texture])?'Q':'-', glstate->enable.texture[texture]);
     ...
     && (glstate->enable.texgen_r[texture] && (glstate->texgen[texture].R==GL_REFLECTION_MAP)))
    {
        if(hardext.cubemap) {
            *needclean=1;
            ...
            LOAD_GLES(glEnable);
            // setup cube map mode
            gl4es_gles_glTexGeni(GL_TEXTURE_GEN_STR, GL_TEXTURE_GEN_MODE, GL_REFLECTION_MAP);
            // enable texgen
            gl4es_gles_glEnable(GL_TEXTURE_GEN_STR);
            ...
            if (!(globals4es.texmat || glstate->texture_matrix[texture]->identity)) {
                ...
                gl4es_gles_glLoadMatrixf(getTexMat(texture));
     ...
     && (glstate->enable.texgen_r[texture] && (glstate->texgen[texture].R==GL_NORMAL_MAP)))
    {
        *needclean=1;
        ...
        LOAD_GLES(glEnable);
        // setup cube map mode
        gl4es_gles_glTexGeni(GL_TEXTURE_GEN_STR, GL_TEXTURE_GEN_MODE, GL_NORMAL_MAP);
        // enable texgen
        gl4es_gles_glEnable(GL_TEXTURE_GEN_STR);
        // check Texture Matrix
        if (!(globals4es.texmat || glstate->texture_matrix[texture]->identity)) {
...
void gen_tex_clean(GLint cleancode, int texture) {
	...
	if (cleancode == 1) {
		GLuint old_tex=glstate->texture.active;
		LOAD_GLES(glDisable);
		gl4es_gles_glDisable(GL_TEXTURE_GEN_STR);
        // check Texture Matrix

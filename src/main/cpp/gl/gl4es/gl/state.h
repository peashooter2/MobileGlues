...
typedef struct {
    GLuint id;
    GLenum target;
    GLuint color[GL4ES_MAX_DRAW_BUFFERS];   // attachement_color0..9
    GLuint depth;
    GLuint stencil;
    GLuint t_color[GL4ES_MAX_DRAW_BUFFERS]; // type for attachement_0 (GL_NONE, GL_TEXTUREXX, GL_RENDERBUFFER)
    GLuint t_depth;
    GLuint t_stencil;
    int    l_color[GL4ES_MAX_DRAW_BUFFERS]; // level of attachment
    int    l_depth;
    int    l_stencil;
    int    width;
    int    height;
    ...
    GLenum read_type;
    int    n_draw;
    GLenum drawbuff[GL4ES_MAX_DRAW_BUFFERS];    //TODO: define a MAX_DRAWBUFF?
} glframebuffer_t;

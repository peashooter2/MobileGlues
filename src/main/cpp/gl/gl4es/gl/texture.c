    ...
    switch(internalformat) {
        ...
        case GL_RGB32F:
            ...
            *type = (hardext.floattex)?GL_FLOAT:GL_UNSIGNED_BYTE;
            break;
        default:
            SHUT_LOGD("MobileGlues-gl4es: LIBGL: Warning, unknown Internalformat (%s)\n", PrintEnum(internalformat));
            *format = GL_RGBA;
            *type = GL_UNSIGNED_BYTE;
            break;
    }
...
static void *swizzle_texture(GLsizei width, GLsizei height,
    ...
    if (data) {
        if (convert) {
            ...
            if (! GL4ES_pixel_convert(data, &pixels, width, height,
                                *format, *type, dest_format, dest_type, 0, glstate->texture.unpack_align)) {
                SHUT_LOGD("MobileGlues-gl4es: LIBGL: swizzle error: (%s, %s -> %s, %s)\n",
            ...
            *format = dest_format;
            if(dest_format!=internalformat) {
                ...
                bound->format = dest_format;
                bound->type = dest_type;
                if (! GL4ES_pixel_convert(pixels, &pix2, width, height,
                                *format, *type, dest_format, dest_type, 0, glstate->texture.unpack_align)) {
                    SHUT_LOGD("MobileGlues-gl4es: LIBGL: swizzle error: (%s, %s -> %s, %s)\n",
            ...
            if (raster_need_transform())
                if (!GL4ES_pixel_transform(data, &pixels, width, height,
                                *format, *type, glstate->raster.raster_scale, glstate->raster.raster_bias)) {
                    SHUT_LOGD("MobileGlues-gl4es: LIBGL: swizzle/convert error: (%s, %s -> %s, %s)\n",
                        PrintEnum(*format), PrintEnum(*type), PrintEnum(dest_format), PrintEnum(dest_type));
                    pix2 = pixels;
                }
...
void APIENTRY_GL4ES gl4es_glTexImage2D(GLenum target, GLint level, GLint internalformat,
    ...
    if(bound->binded_fbo && (bound->binded_attachment==GL_DEPTH_ATTACHMENT || bound->binded_attachment==GL_STENCIL_ATTACHMENT || bound->binded_attachment==GL_DEPTH_STENCIL_ATTACHMENT))
    {
        ...
        if(data!=NULL) 
            SHUT_LOGD("MobileGlues-gl4es: LIBGL: Warning, Depth/stencil texture resized and with data\n");
        // get new size...
        ...
        if(bound->binded_attachment==GL_DEPTH_ATTACHMENT || bound->binded_attachment==GL_DEPTH_STENCIL_ATTACHMENT)
        {
            if(bound->renderdepth) {
                ...
                {} //STUB of gl4es_gles_glTexImage2D(GL_TEXTURE_2D, 0, bound->format, bound->nwidth, bound->nheight, 0, bound->format, bound->type, NULL);
            }
        }
    ...
    bound->alpha = GL4ES_pixel_hasalpha(format);
    ...
    if (datab) {
        ...
        || glstate->texture.unpack_skip_pixels || glstate->texture.unpack_skip_rows) {
            ...
            pixelSize = GL4ES_pixel_sizeof(format, type);
            ...
            case 11:
                if ((mipwidth > 1) && (mipheight > 1)) {
                    ...
                    if(ratiox==0.5f && ratioy==0.5f && npot(width)==width && npot(height)==height) {
                        ...
                        GL4ES_pixel_halfscale(pixels, &out, width, height, format, type);
                    } else {
                        bound->ratiox = ratiox;
                        bound->ratioy = ratioy;
                        bound->useratio = 1;
                        GL4ES_pixel_scale(pixels, &out, width, height, newwidth, newheight, format, type);
                    }
            ...
            default:
                ...
                while(shrink) {
                    ...
                    if(toshrink==1) {
                        ...
                        GL4ES_pixel_quarterscale(pixels, &out, width, height, format, type);
                        if(!bound->valid)
        ...
        if (globals4es.texdump) {
            GL4ES_pixel_to_ppm(pixels, width, height, format, type, bound->texture, glstate->texture.pack_align);
        }
    } else {
#ifdef TEXSTREAM
...
#define NO_1x1
        ...
        if(bound->npot) {
            if(limitednpot && rtarget==GL_TEXTURE_2D) {
                ...
                GLvoid *out = pixels;
                if(pixels)
                    GL4ES_pixel_scale(pixels, &out, width, height, nwidth, nheight, format, type);
                if (out != pixels && pixels!=datab)
                    free(pixels);
                pixels = out;
                width = nwidth;
        ...
        if (!(globals4es.texstream && bound->streamed)) {
            if ((target!=GL_TEXTURE_RECTANGLE_ARB) && (globals4es.automipmap!=3) && (bound->mipmap_need || bound->mipmap_auto) && !(bound->npot && hardext.npot<2) && (bound->max_level==-1)) {
                if(hardext.esversion<2)
                    {} //STUB of gl4es_gles_glTexParameteri( rtarget, GL_GENERATE_MIPMAP, GL_TRUE );
                else
                    callgeneratemipmap = 1;
            } else {
                //if(target!=GL_TEXTURE_RECTANGLE_ARB) gl4es_gles_glTexParameteri( rtarget, GL_GENERATE_MIPMAP, GL_FALSE );
            ...
            if (height != nheight || width != nwidth) {
                errorGL();
                {} //STUB of gl4es_gles_glTexImage2D(rtarget, level, format, nwidth, nheight, border, format, type, NULL);
                DBG(CheckGLError(1);)
                if (pixels) {
                    {} //STUB of gl4es_gles_glTexSubImage2D(rtarget, level, 0, 0, width, height, format, type, pixels);// DBG(CheckGLError(1);)
                }
#ifdef NO_1x1
                if(level==0 && (width==1 && height==1 && pixels)) {
                    ...
                    if(width==1) {gl4es_gles_glTexSubImage2D(rtarget, level, 1, 0, width, height, format, type, pixels); nwidth=1;}
                    if(height==1) {gl4es_gles_glTexSubImage2D(rtarget, level, 0, 1, width, height, format, type, pixels); nheight=1;}
                    if(width==1 && height==1) {   // create a manual mipmap just in case_state
                        {} //STUB of gl4es_gles_glTexSubImage2D(rtarget, level, 1, 1, width, height, format, type, pixels);
                        {} //STUB of gl4es_gles_glTexImage2D(rtarget, 1, format, 1, 1, 0, format, type, pixels);
                    }
                }
#endif
            } else {
                errorGL();
                {} //STUB of gl4es_gles_glTexImage2D(rtarget, level, format, width, height, border, format, type, pixels);
                DBG(CheckGLError(1);)
            }
            ...
            if(bound->base_level == level && !(bound->max_level==level && level==0)) {
                ...
                while(leveln) {
                    if(pixels) {
                        GLvoid *out = ndata;
                        GL4ES_pixel_doublescale(ndata, &out, nw, nh, format, type);
                    ...
                    nhh <<= 1;
                    --leveln;
                    {} //STUB of gl4es_gles_glTexImage2D(rtarget, leveln, format, nww, nhh, border, format, type, (pot)?ndata:NULL);
                    if(!pot && pixels) gl4es_gles_glTexSubImage2D(rtarget, leveln, 0, 0, nw, nh,
                                        format, type, ndata);
                }
                if (ndata!=pixels)
                    free(ndata);
            ...
            if(((bound->max_level == level && (level || bound->mipmap_need)) || (callgeneratemipmap && level==0) || (globals4es.automipmap==5 && level && !bound->mipmap_done)) && !(bound->max_level==bound->base_level && bound->max_level==0)) {
                ...
                while(nw!=1 || nh!=1) {
                    if(pixels) {
                        GLvoid *out = ndata;
                        GL4ES_pixel_halfscale(ndata, &out, nww, nhh, format, type);
                        if (out != ndata && ndata!=pixels)
                            free(ndata);
                        ndata = out;
                    }
                    ...
                    nhh = nlevel(nhh, 1);
                    ++leveln;
                    {} //STUB of gl4es_gles_glTexImage2D(rtarget, leveln, format, nw, nh, border, format, type, (pot)?ndata:NULL);
                    if(!pot && pixels) gl4es_gles_glTexSubImage2D(rtarget, leveln, 0, 0, nww, nhh,
                                        format, type, ndata);
                }
                if (ndata!=pixels)
                    free(ndata);
        ...
        /*if (bound && bound->mipmap_need && !bound->mipmap_auto && (globals4es.automipmap!=3))
            {} //STUB of gl4es_gles_glTexParameteri( rtarget, GL_GENERATE_MIPMAP, GL_FALSE );*/
        } else {
            if (pixels)
    ...
    if ((target==GL_TEXTURE_2D) && globals4es.texcopydata && ((globals4es.texstream && !bound->streamed) || !globals4es.texstream)) {
        ...
        if (datab) {
            if (!GL4ES_pixel_convert(pixels, &bound->data, width, height, format, type, GL_RGBA, GL_UNSIGNED_BYTE, 0, glstate->texture.unpack_align))
                SHUT_LOGD("MobileGlues-gl4es: LIBGL: Error on GL4ES_pixel_convert when TEXCOPY in glTexImage2D\n");
        } else {
    ...
    if(bound->binded_fbo && (bound->binded_attachment==GL_DEPTH_ATTACHMENT || bound->binded_attachment==GL_STENCIL_ATTACHMENT || bound->binded_attachment==GL_DEPTH_STENCIL_ATTACHMENT))
    {
        ...
        if(bound->binded_attachment==GL_DEPTH_ATTACHMENT || bound->binded_attachment==GL_DEPTH_STENCIL_ATTACHMENT)
        {
            if(bound->renderdepth) {
                ...
                gl4es_gles_glTexImage2D(GL_TEXTURE_2D, 0, bound->format, bound->nwidth, bound->nheight, 0, bound->format, bound->type, NULL);
            }
        }
        if((bound->binded_attachment==GL_STENCIL_ATTACHMENT || bound->binded_attachment==GL_DEPTH_STENCIL_ATTACHMENT) && bound->renderstencil)
        {
...
#define NO_1x1
        ...
        if (!(globals4es.texstream && bound->streamed)) {
            if ((target!=GL_TEXTURE_RECTANGLE_ARB) && (globals4es.automipmap!=3) && (bound->mipmap_need || bound->mipmap_auto) && !(bound->npot && hardext.npot<2) && (bound->max_level==-1)) {
                if(hardext.esversion<2)
                    gl4es_gles_glTexParameteri( rtarget, GL_GENERATE_MIPMAP, GL_TRUE );
                else
                    callgeneratemipmap = 1;
            } else {
                //if(target!=GL_TEXTURE_RECTANGLE_ARB) gl4es_gles_glTexParameteri( rtarget, GL_GENERATE_MIPMAP, GL_FALSE );
            ...
            if (height != nheight || width != nwidth) {
                errorGL();
                gl4es_gles_glTexImage2D(rtarget, level, format, nwidth, nheight, border, format, type, NULL);
                DBG(CheckGLError(1);)
                if (pixels) {
                    gl4es_gles_glTexSubImage2D(rtarget, level, 0, 0, width, height, format, type, pixels);// DBG(CheckGLError(1);)
                }
#ifdef NO_1x1
                if(level==0 && (width==1 && height==1 && pixels)) {
                    // complete the texture, juste in case it use GL_REPEAT
                    ...
                    if(height==1) {gl4es_gles_glTexSubImage2D(rtarget, level, 0, 1, width, height, format, type, pixels); nheight=1;}
                    if(width==1 && height==1) {   // create a manual mipmap just in case_state
                        gl4es_gles_glTexSubImage2D(rtarget, level, 1, 1, width, height, format, type, pixels);
                        gl4es_gles_glTexImage2D(rtarget, 1, format, 1, 1, 0, format, type, pixels);
                    }
                }
#endif
            } else {
                errorGL();
                gl4es_gles_glTexImage2D(rtarget, level, format, width, height, border, format, type, pixels);
                DBG(CheckGLError(1);)
            }
            // check if base_level is set... and calculate lower level mipmap
            if(bound->base_level == level && !(bound->max_level==level && level==0)) {
                ...
                while(leveln) {
                    ...
                    nhh <<= 1;
                    --leveln;
                    gl4es_gles_glTexImage2D(rtarget, leveln, format, nww, nhh, border, format, type, (pot)?ndata:NULL);
                    if(!pot && pixels) gl4es_gles_glTexSubImage2D(rtarget, leveln, 0, 0, nw, nh,
                                        format, type, ndata);
                }
                if (ndata!=pixels)
            ...
            if(((bound->max_level == level && (level || bound->mipmap_need)) || (callgeneratemipmap && level==0) || (globals4es.automipmap==5 && level && !bound->mipmap_done)) && !(bound->max_level==bound->base_level && bound->max_level==0)) {
                ...
                while(nw!=1 || nh!=1) {
                    ...
                    nhh = nlevel(nhh, 1);
                    ++leveln;
                    gl4es_gles_glTexImage2D(rtarget, leveln, format, nw, nh, border, format, type, (pot)?ndata:NULL);
                    if(!pot && pixels) gl4es_gles_glTexSubImage2D(rtarget, leveln, 0, 0, nww, nhh,
                                        format, type, ndata);
                }
                if (ndata!=pixels)
        ...
        /*if (bound && bound->mipmap_need && !bound->mipmap_auto && (globals4es.automipmap!=3))
            gl4es_gles_glTexParameteri( rtarget, GL_GENERATE_MIPMAP, GL_FALSE );*/
        } else {
            if (pixels)
                gl4es_glTexSubImage2D(rtarget, level, 0, 0, width, height, format, type, pixels);    // (should never happens) updload the 1st data...
        }
...
void APIENTRY_GL4ES gl4es_glTexSubImage2D(GLenum target, GLint level, GLint xoffset, GLint yoffset,
    ...
    if (globals4es.texstream && (bound->streamed)) {
        ...
        if (! GL4ES_pixel_convert(old, &tmp, width, height,
                        format, type, GL_RGB, GL_UNSIGNED_SHORT_5_6_5, bound->width, glstate->texture.unpack_align)) {
            SHUT_LOGD("MobileGlues-gl4es: LIBGL: swizzle error: (%#4x, %#4x -> GL_RGB, UNSIGNED_SHORT_5_6_5)\n",
                        format, type);
        }
        format = GL_RGB;
        type = GL_UNSIGNED_SHORT_5_6_5;
...
#endif
    {
        if (!GL4ES_pixel_convert(old, &pixels, width, height, format, type, bound->inter_format, bound->inter_type, 0, glstate->texture.unpack_align)) {
            SHUT_LOGD("MobileGlues-gl4es: LIBGL: Error in GL4ES_pixel_convert while glTexSubImage2D\n");
        } else {
            format = bound->inter_format;
            type = bound->inter_type;
            if(bound->inter_format!=bound->format || bound->inter_type!=bound->type) {
                GLvoid* pix2 = pixels;
                if (!GL4ES_pixel_convert(pixels, &pix2, width, height, format, type, bound->format, bound->type, 0, glstate->texture.unpack_align)) {
                    SHUT_LOGD("MobileGlues-gl4es: LIBGL: Error in GL4ES_pixel_convert while glTexSubImage2D\n");
                }
    ...
    if (bound->shrink || bound->useratio) {
        ...
        if(bound->useratio) {
            ...
            int newwidth = width * bound->ratiox;
            int newheight = height * bound->ratioy;
            GL4ES_pixel_scale(pixels, &old, width, height, newwidth, newheight, format, type);
            width = newwidth;
            height = newheight;
            ...
            while(shrink) {
                ...
                GLvoid *out = pixels;
                if(toshrink==1)
                    GL4ES_pixel_halfscale(pixels, &old, width, height, format, type);
                else
                    GL4ES_pixel_quarterscale(pixels, &old, width, height, format, type);
                if (old != pixels && pixels!=datab)
                    free(pixels);
                pixels = old;
                width = nlevel(width, toshrink);
...
/*    // copy the texture to the buffer
    ...
    for (int yy=0; yy<height; yy++) {
        ...
        if(bound->base_level == level && !(bound->max_level==level && level==0)) {
            ...
            while(leveln) {
                ...
                yy <<= 1;
                --leveln;
                {} //STUB of gl4es_gles_glTexSubImage2D(rtarget, leveln, xx, yy, nw, nh, format, type, ndata);
            }
            if (ndata!=pixels)
                free(ndata);
        }
        ...
        if(genmipmap && (globals4es.automipmap!=3)) {
            ...
            while(nw!=1 || nh!=1) {
                if(pixels) {
                    GLvoid *out = ndata;
                    GL4ES_pixel_halfscale(ndata, &out, nw, nh, format, type);
                    if (out != ndata && ndata!=pixels)
                        free(ndata);
                    ndata = out;
                }
    ...
    if ((target==GL_TEXTURE_2D) && globals4es.texcopydata && ((globals4es.texstream && !bound->streamed) || !globals4es.texstream)) {
    //SHUT_LOGD("MobileGlues-gl4es: *texcopy* glTexSubImage2D, xy=%i,%i, size=%i,%i=>%i,%i, format=%s, type=%s, tex=%u\n", xoffset, yoffset, width, height, bound->width, bound->height, PrintEnum(format), PrintEnum(type), bound->glname);
        GLvoid * tmp = (char*)bound->data + (yoffset*bound->width + xoffset)*4;
        if (!GL4ES_pixel_convert(pixels, &tmp, width, height, format, type, GL_RGBA, GL_UNSIGNED_BYTE, bound->width, glstate->texture.unpack_align))
            SHUT_LOGD("MobileGlues-gl4es: LIBGL: Error on GL4ES_pixel_convert while TEXCOPY in glTexSubImage2D\n");
    }
    ...
    for (int yy=0; yy<height; yy++) {
        ...
        if(bound->base_level == level && !(bound->max_level==level && level==0)) {
            ...
            while(leveln) {
                ...
                yy <<= 1;
                --leveln;
                gl4es_gles_glTexSubImage2D(rtarget, leveln, xx, yy, nw, nh, format, type, ndata);
            }
            if (ndata!=pixels)
                free(ndata);
        }
...
GLboolean APIENTRY_GL4ES gl4es_glIsTexture(GLuint texture) {
    DBG(SHUT_LOGD("MobileGlues-gl4es: glIsTexture(%d):", texture);)
    if(!glstate) {DBG(SHUT_LOGD("MobileGlues-gl4es: GL_FALSE\n");) return GL_FALSE;}
    noerrorShim();
    if (!texture) {
        DBG(SHUT_LOGD("MobileGlues-gl4es: %s\n", glstate->texture.zero->valid?"GL_TRUE":"GL_FALSE");)
        return glstate->texture.zero->valid;
    }
    khint_t k;
    khash_t(tex) *list = glstate->texture.list;
    if (! list) {
        DBG(SHUT_LOGD("MobileGlues-gl4es: GL_FALSE\n");)
        return GL_FALSE;
    }

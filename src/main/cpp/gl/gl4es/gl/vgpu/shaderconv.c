...
int testGenericShader(struct shader_s* shader_source) {
    ...
    GLuint shad = gl4es_gles_glCreateShader(shader_source->type);
    gl4es_gles_glShaderSource(shad, 1, (const GLchar *const *)(&shader_source->converted), NULL);
...
char* process_uniform_declarations_for_c(char* glslCode);
...
char * ConvertShaderConditionally(struct shader_s * shader_source){
    ...
    shader_source->converted = process_uniform_declarations_for_c(shader_source->converted);

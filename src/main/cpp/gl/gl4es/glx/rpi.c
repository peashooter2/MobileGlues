...
void rpi_init() {
    ...
    if(!bcm_host_init || !bcm_host_deinit) {
        SHUT_LOGD("MobileGlues-gl4es: LIBGL: Warning, bcm_host function missing (init=%p, deinit=%p)\n", bcm_host_init, bcm_host_deinit);
        return;
    }
    bcm_host_init();

    #define GO(A) A=dlsym(bcm_host, #A); if(A==NULL) A=dlsym(vcos, #A); if(A==NULL) SHUT_LOGD("MobileGlues-gl4es: LIBGL: Warning, " #A " is null")

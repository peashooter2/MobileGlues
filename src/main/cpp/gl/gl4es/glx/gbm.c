...
static int init_drm_and_gbm()
{
    ...
    if(!resources) {
        SHUT_LOGD("MobileGlues-gl4es: LIBGL: Error initializing drm resources\n");
        return 0;
    }
    ...
    if (!connector) {
         ...
         * a connector..
         */
        SHUT_LOGD("MobileGlues-gl4es: LIBGL: DRM no connected connector!\n");
        return 0;
    }
    ...
    if (!drm_mode) {
        SHUT_LOGD("MobileGlues-gl4es: LIBGL: DRM could not find mode!\n");
        return 0;
    }
    /* find encoder: */
    ...
    if (encoder) {
        ...
        if (crtc_id == 0) {
            SHUT_LOGD("MobileGlues-gl4es: LIBGL: DRM no crtc found!\n");
            return 0;
        }
    ...
    if(!gbmdev) {
        SHUT_LOGD("MobileGlues-gl4es: LIBGL: Error initializing gbm device\n");
        return 0;
    }
    ...
    if(!gbmsurf) {
        SHUT_LOGD("MobileGlues-gl4es: LIBGL: Error initializing gbm surface\n");
        return 0;
    }
...
static struct drm_fb * drm_fb_get_from_bo(struct gbm_bo *bo)
{
    ...
    if (ret) {
        SHUT_LOGD("MobileGlues-gl4es: failed to create fb: %s\n", strerror(errno));
        free(fb);
        return NULL;
    }
...
static int OpenGBM()
{
    ...
    SHUT(SHUT_LOGD("MobileGlues-gl4es: LIBGL: GBM on card %s is %s\n", globals4es.drmcard, available?"Available":"Not available"));
...
void LoadGBMFunctions()
{
    ...
    #define GBMFUNC(ret, name, args) \
        ...
        if(!gbmdrm_##name) { \
            SHUT_LOGD("MobileGlues-gl4es: LIBGL: libgbm function %s missing, no gbm surface enabled\n", #name); \
            CancelGBM(); \
            return; \
        }
    #include "gbmfunc.h"
    ...
    #define DRMFUNC(ret, name, args) \
        ...
        if(!gbmdrm_##name) { \
            SHUT_LOGD("MobileGlues-gl4es: LIBGL: libdrm function %s missing, no gbm surface enabled\n", #name); \
            CancelGBM(); \
            return; \
        }
    #include "drmfunc.h"
...
void* OpenGBMDisplay(void* display)
{
    ...
    if(gl4es_egl_eglGetPlatformDisplay) {
        return gl4es_egl_eglGetPlatformDisplay(EGL_PLATFORM_GBM_KHR, gbmdev, NULL);
    } else {
        LOAD_EGL(eglGetDisplay);
        return gl4es_egl_eglGetDisplay((EGLNativeDisplayType)gbmdev);
    }
}
...
void* CreateGBMWindow(int w, int h)
{
    void* ret = gbmsurf;
    if(!ret)
        SHUT_LOGD("MobileGlues-gl4es: LIBGL: Warning, cannot create gbm surface %dx%d\n", w, h);
}
...
int FindGBMConfig(EGLDisplay eglDisp, EGLConfig *configs, int numFounds)
{
    ...
    {
        EGLint gbm_format;
        if (gl4es_egl_eglGetConfigAttrib(eglDisp, configs[idx], EGL_NATIVE_VISUAL_ID, &gbm_format)) {
    ...
    SHUT_LOGD("MobileGlues-gl4es: LIBGL: Warning, no EGLConfig matching GBM Format found\n");
    return 0;   // not found...
}
EGLBoolean GBMMakeCurrent(EGLDisplay eglDisp, EGLSurface draw, EGLSurface read, EGLContext ctx)
{
    ...
    EGLBoolean res = gl4es_egl_eglMakeCurrent(eglDisp, draw, read, ctx);
    ...
    gl4es_egl_eglSwapBuffers(eglDisp, draw);
    struct gbm_bo *bo = gbmdrm_gbm_surface_lock_front_buffer(gbmsurf);
    if(!bo) {
        // probably a PBuffer
        //SHUT_LOGD("MobileGlues-gl4es: LIBGL: gbm BO is NULL\n");
        return EGL_TRUE;
    }
    ...
    if (r) {
        SHUT_LOGD("MobileGlues-gl4es: LIBGL: GBM failed to set mode: %s\n", strerror(errno));
        return EGL_FALSE;
    }

...
static int testGLSL(const char* version, int uniformLoc) {
    ...
    GLuint shad = gl4es_gles_glCreateShader(GL_VERTEX_SHADER);
    ...
    gl4es_gles_glShaderSource(shad, 4, shadTest, NULL);
    gl4es_gles_glCompileShader(shad);
    GLint compiled;
    gl4es_gles_glGetShaderiv(shad, GL_COMPILE_STATUS, &compiled);
    /*
    if(!compiled) {
        LOAD_GLES2(glGetShaderInfoLog)
        char buff[500];
        gl4es_gles_glGetShaderInfoLog(shad, 500, NULL, buff);
        SHUT_LOGD("MobileGlues-gl4es: LIBGL: \"%s\" failed, message:\n%s\n", version, buff);
    }
    */
    gl4es_gles_glDeleteShader(shad);
    gl4es_gles_glGetError();	// reset GL Error
...
static int testTextureCubeLod() {
    ...
    GLuint shad = gl4es_gles_glCreateShader(GL_FRAGMENT_SHADER);
    ...
    gl4es_gles_glShaderSource(shad, 3, shadTest, NULL);
    gl4es_gles_glCompileShader(shad);
    GLint compiled;
    gl4es_gles_glGetShaderiv(shad, GL_COMPILE_STATUS, &compiled);
    gl4es_gles_glDeleteShader(shad);
...
void GetHardwareExtensions(int notest)
{
    ...
    {
#ifndef AMIGAOS4
        SHUT_LOGD("MobileGlues-gl4es: Hardware test disabled, nothing activated...\n");
#endif
...
#endif
#ifdef NOEGL
    SHUT_LOGD("MobileGlues-gl4es: Hardware test on current Context...\n");
#else
    ...
    if (gl4es_egl_eglInitialize(eglDisplay, NULL, NULL) != EGL_TRUE) {
        LOGE("MobileGlues-gl4es: Error while gathering supported extension (eglInitialize: %s), default to none\n", PrintEGLError(0));
        gl4es_egl_eglTerminate(eglDisplay);
        return;
    }

    gl4es_egl_eglChooseConfig(eglDisplay, configAttribs, pbufConfigs, 1, &configsFound);
#ifndef NO_GBM
    const char* eglExts = gl4es_egl_eglQueryString(EGL_NO_DISPLAY, EGL_EXTENSIONS);
    if(eglExts && strstr(eglExts, "EGL_KHR_platform_gbm ")) {
        SHUT_LOGD("MobileGlues-gl4es: GBM Surfaces supported%s\n", globals4es.usegbm?" and used":"");
        hardext.gbm = 1;
    }
#endif
#ifndef PANDORA
    if(!configsFound) {
        ...
        gl4es_egl_eglChooseConfig(eglDisplay, configAttribs, pbufConfigs, 1, &configsFound);
        if(configsFound) {
            SHUT_LOGD("MobileGlues-gl4es: No alpha channel in PBuffer context, disabling EGL Alpha channel...\n");
            hardext.eglnoalpha = 1;
        }
    }
#endif
    if(!configsFound) {
        SHUT_LOGE("MobileGlues-gl4es: Error while gathering supported extension (eglChooseConfig: %s), default to none\n", PrintEGLError(0));
        gl4es_egl_eglTerminate(eglDisplay);
        return;
    }
    eglContext = gl4es_egl_eglCreateContext(eglDisplay, pbufConfigs[0], EGL_NO_CONTEXT, (hardext.esversion==1)?egl_context_attrib:egl_context_attrib_es2);
    if(!eglContext) {
        SHUT_LOGE("MobileGlues-gl4es: Error while gathering supported extension (eglCreateContext: %s), default to none\n", PrintEGLError(0));
        return;
    }
    eglSurface = gl4es_egl_eglCreatePbufferSurface(eglDisplay, pbufConfigs[0], egl_attribs);
    if(!eglSurface) {
        SHUT_LOGE("MobileGlues-gl4es: Error while gathering supported extension (eglCreatePBufferSurface: %s), default to none\n", PrintEGLError(0));
        gl4es_egl_eglDestroyContext(eglDisplay, eglContext);
        gl4es_egl_eglTerminate(eglDisplay);
        return;
    }
    gl4es_egl_eglMakeCurrent(eglDisplay, eglSurface, eglSurface, eglContext);
#endif
    tested = 1;
    ...
    LOAD_GLES(glGetError);
    // Now get extensions
    const char *Exts = (const char *) gl4es_gles_glGetString(GL_EXTENSIONS);
    // Parse them!
    #define S(A, B, C) if(strstr(Exts, A)) { hardext.B = 1; SHUT_LOGD("MobileGlues-gl4es: Extension %s detected%s",A, C?" and used\n":"\n"); } 
    ...
    if(hardext.npot>0) {
        SHUT_LOGD("MobileGlues-gl4es: Hardware %s NPOT detected and used\n", hardext.npot==3?"Full":(hardext.npot==2?"Limited+Mipmap":"Limited"));
    }
    S("GL_EXT_blend_minmax ", blendminmax, 1);
    if (hardext.esversion>2) {
        SHUT_LOGD("MobileGlues-gl4es: Extension GL_EXT_draw_buffers is in core ES3, and so used\n");
    ...
    /*if(hardext.blendcolor==0) {
        ...
        if(gl4es_gles_glBlendColor != NULL) {
            hardext.blendcolor = 1;
	        SHUT_LOGD("MobileGlues-gl4es: Extension glBlendColor found and used\n");
	    }
    ...
    if(hardext.esversion<2) {
        ...
        hardext.fbo = 1; 
        SHUT_LOGD("MobileGlues-gl4es: FBO are in core, and so used\n");
        hardext.pointsprite = 1;
        SHUT_LOGD("MobileGlues-gl4es: PointSprite are in core, and so used\n");
        hardext.pointsize = 1;
        SHUT_LOGD("MobileGlues-gl4es: CubeMap are in core, and so used\n");
        hardext.cubemap = 1;
        SHUT_LOGD("MobileGlues-gl4es: BlendColor is in core, and so used\n");
        hardext.blendcolor = 1;
        SHUT_LOGD("MobileGlues-gl4es: Blend Subtract is in core, and so used\n");
        hardext.blendsub = 1;
        SHUT_LOGD("MobileGlues-gl4es: Blend Function and Equation Separation is in core, and so used\n");
        hardext.blendfunc = 1;
        hardext.blendeq = 1;
        SHUT_LOGD("MobileGlues-gl4es: Texture Mirrored Repeat is in core, and so used\n");
        hardext.mirrored = 1;
    }
    ...
    if (hardext.esversion>1) {
        if(!globals4es.nohighp) {
            ...
            if(!hardext.highp) {
                ...
                if(gl4es_gles_glGetShaderPrecisionFormat) {
                    GLint range[2] = {0};
                    GLint precision=0;
                    gl4es_gles_glGetShaderPrecisionFormat(GL_FRAGMENT_SHADER, GL_HIGH_FLOAT, range, &precision);
                    if(!(range[0]==0 && range[1]==0 && precision==0)) {
                        hardext.highp = 2;  // no need to declare #entension here
                        SHUT_LOGD("MobileGlues-gl4es: high precision float in fragment shader available and used\n");
                    }
                }
            }
        }
        ...
        if(hardext.shaderlod) {
            ...
            if(testTextureCubeLod()) {
                hardext.cubelod = 1;
                SHUT_LOGD("MobileGlues-gl4es: textureCubeLod in fragment doesn't need trailing EXT\n");
            }
        }
        S("GL_EXT_frag_depth ", fragdepth, 1);
        gl4es_gles_glGetIntegerv(GL_MAX_VERTEX_ATTRIBS, &hardext.maxvattrib);
        SHUT_LOGD("MobileGlues-gl4es: Max vertex attrib: %d\n", hardext.maxvattrib);
        ...
        if(hardext.prgbinary) {
            gl4es_gles_glGetIntegerv(GL_NUM_PROGRAM_BINARY_FORMATS_OES, &hardext.prgbin_n);
            SHUT_LOGD("MobileGlues-gl4es: Number of supported Program Binary Format: %d\n", hardext.prgbin_n);
        }
    }
    // Now get some max stuffs
    gl4es_gles_glGetIntegerv(GL_MAX_TEXTURE_SIZE, &hardext.maxsize);
    SHUT_LOGD("MobileGlues-gl4es: Max texture size: %d\n", hardext.maxsize);
    gl4es_gles_glGetIntegerv((hardext.esversion==1)?GL_MAX_TEXTURE_UNITS:GL_MAX_TEXTURE_IMAGE_UNITS, &hardext.maxtex);
    if (hardext.esversion==1) {
        gl4es_gles_glGetIntegerv(GL_MAX_LIGHTS, &hardext.maxlights);
        gl4es_gles_glGetIntegerv(GL_MAX_CLIP_PLANES, &hardext.maxplanes);
        hardext.maxteximage=hardext.maxtex;
    } else {
        // simulated stuff using the FPE
        hardext.maxlights = 8;
        hardext.maxplanes = 6;
        gl4es_gles_glGetIntegerv(GL_MAX_TEXTURE_IMAGE_UNITS, &hardext.maxteximage);
        gl4es_gles_glGetIntegerv(GL_MAX_VARYING_VECTORS, &hardext.maxvarying);
        SHUT_LOGD("MobileGlues-gl4es: Max Varying Vector: %d\n", hardext.maxvarying);
    ...
    SHUT_LOGD("MobileGlues-gl4es: Texture Units: %d/%d (hardware: %d), Max lights: %d, Max planes: %d\n", hardext.maxtex, hardext.maxteximage, hardmaxtex, hardext.maxlights, hardext.maxplanes);
    S("GL_EXT_texture_filter_anisotropic ", aniso, 1);
    if(hardext.aniso) {
        gl4es_gles_glGetIntegerv(GL_MAX_TEXTURE_MAX_ANISOTROPY, &hardext.aniso);
        if(gl4es_gles_glGetError()!=GL_NO_ERROR)
            hardext.aniso = 0;
        if(hardext.aniso)
            SHUT_LOGD("MobileGlues-gl4es: Max Anisotropic filtering: %d\n", hardext.aniso);
    }
    if(hardext.drawbuffers) {
        gl4es_gles_glGetIntegerv(GL_MAX_COLOR_ATTACHMENTS_EXT,&hardext.maxcolorattach);
        gl4es_gles_glGetIntegerv(GL_MAX_DRAW_BUFFERS_ARB, &hardext.maxdrawbuffers);
    }
    if(hardext.maxcolorattach<1)
        hardext.maxcolorattach = 1;
    if(hardext.maxcolorattach>GL4ES_MAX_DRAW_BUFFERS)
        hardext.maxcolorattach=GL4ES_MAX_DRAW_BUFFERS;
    if(hardext.maxdrawbuffers<1)
        hardext.maxdrawbuffers = 1;
    if(hardext.maxdrawbuffers>GL4ES_MAX_DRAW_BUFFERS)
        hardext.maxdrawbuffers=GL4ES_MAX_DRAW_BUFFERS;
    SHUT_LOGD("MobileGlues-gl4es: Max Color Attachments: %d / Draw buffers: %d\n", hardext.maxdrawbuffers, hardext.maxcolorattach);
    // get GLES driver signatures...
    const char *vendor = (const char *) gl4es_gles_glGetString(GL_VENDOR);
    SHUT_LOGD("MobileGlues-gl4es: Hardware vendor is %s\n", vendor);
    ...
    if(strstr(gl4es_egl_eglQueryString(eglDisplay, EGL_EXTENSIONS), "EGL_KHR_gl_colorspace")) {
        SHUT_LOGD("MobileGlues-gl4es: sRGB surface supported\n");
        hardext.srgb = 1;
    }
    if(strstr(gl4es_egl_eglQueryString(eglDisplay, EGL_EXTENSIONS), "EGL_KHR_image_pixmap")) {
        SHUT_LOGD("MobileGlues-gl4es: EGLImage from Pixmap supported\n");
        hardext.khr_pixmap = 1;
    }
    if(strstr(gl4es_egl_eglQueryString(eglDisplay, EGL_EXTENSIONS), "EGL_KHR_gl_texture_2D_image")) {
        SHUT_LOGD("MobileGlues-gl4es: EGLImage to Texture2D supported\n");
        hardext.khr_texture_2d = 1;
    }
    if(strstr(gl4es_egl_eglQueryString(eglDisplay, EGL_EXTENSIONS), "EGL_KHR_gl_renderbuffer_image")) {
        SHUT_LOGD("MobileGlues-gl4es: EGLImage to RenderBuffer supported\n");
        hardext.khr_renderbuffer = 1;
    }

    // End, cleanup
    gl4es_egl_eglMakeCurrent(eglDisplay, 0, 0, EGL_NO_CONTEXT);
    gl4es_egl_eglDestroySurface(eglDisplay, eglSurface);
    gl4es_egl_eglDestroyContext(eglDisplay, eglContext);

...
int8_t CheckEGLErrors() {
    ...
    if (errortext) {
        LOGE("MobileGlues-gl4es: ERROR: EGL Error detected: %s\n", errortext);
        return 1;
    }
#endif
    return 0;
...
void glx_getMainFBSize(GLint* width, GLint* height) {
    ...
    DBG(SHUT_LOGD("MobileGlues-gl4es: gl4es_getMainFBSize() %dx%d -> ", *width, *height);)
    LOAD_EGL(eglQuerySurface);
    gl4es_egl_eglQuerySurface(eglDisplay, glxContext->eglSurface, EGL_WIDTH, width);
    gl4es_egl_eglQuerySurface(eglDisplay, glxContext->eglSurface, EGL_HEIGHT, height);
    DBG(SHUT_LOGD("MobileGlues-gl4es: %dx%d (%s)\n", *width, *height, PrintEGLError(0));)
#endif
}
...
static void RecycleAddSurface(GLXDrawable drawable, SharedEGLSurface_t* surf) {
    ...
    DBG(SHUT_LOGD("MobileGlues-gl4es: LIBGL: EGLSurface for drawable %p Added\n", (void*)drawable);)
}
...
static SharedEGLSurface_t* RecycleGetSurface(GLXDrawable drawable) {
    ...
    if (k != kh_end(eglsurfaces)){
        DBG(SHUT_LOGD("MobileGlues-gl4es: LIBGL: EGLSurface for drawable %p found\n", (void*)drawable);)
...
static void RecycleDelSurface(GLXDrawable drawable) {
    ...
    if (k != kh_end(eglsurfaces)){
        ...
        gl4es_egl_eglDestroySurface(eglDisplay, kh_value(eglsurfaces, k));*/
        free(kh_value(eglsurfaces, k));
        kh_del(eglsurfacelist_t, eglsurfaces, k);
        DBG(SHUT_LOGD("MobileGlues-gl4es: LIBGL: EGLSurface for drawable %p removed\n", (void*)drawable);)    
    }
    return;
}
#endif
    ...
    switch (attribute) {
        ...
        case GLX_VISUAL_ID:
            {
                ...
                if (!n) {
                    LOGD("MobileGlues-gl4es: Warning, get_config_default: XGetVisualInfo gives 0 VisualInfo for %d depth and TrueColor class\n", glx_default_depth);
                    *value = 0;
                } else {
        ...
        case GLX_FRAMEBUFFER_SRGB_CAPABLE_ARB:
            *value = hardext.srgb;
            break;
        default:
            DBG(SHUT_LOGD("MobileGlues-gl4es:  => Unknown attrib\n");)
            LOGD("MobileGlues-gl4es: unknown attrib %i\n", attribute);
            *value = 0;
            return 1;
    }
    DBG(SHUT_LOGD("MobileGlues-gl4es:  -> 0x%04X\n", *value);)
    return 0;
}
...
static void init_display(Display *display) {
    ...
    if(!eglDisplay) {
        if (globals4es.usefb || globals4es.usepbuffer) {
            eglDisplay = gl4es_egl_eglGetDisplay(EGL_DEFAULT_DISPLAY);
        } else {
            eglDisplay = gl4es_egl_eglGetDisplay(display);
        }
    }
}
...
static void fill1GLXFBConfig(Display *display, EGLConfig eglConfig, int DB, GLXFBConfig fbConfig) {
    ...
    EGLint tmp;
    gl4es_egl_eglGetConfigAttrib(eglDisplay, eglConfig, EGL_RED_SIZE, &fbConfig->redBits);
    gl4es_egl_eglGetConfigAttrib(eglDisplay, eglConfig, EGL_GREEN_SIZE, &fbConfig->greenBits);
    gl4es_egl_eglGetConfigAttrib(eglDisplay, eglConfig, EGL_BLUE_SIZE, &fbConfig->blueBits);
    gl4es_egl_eglGetConfigAttrib(eglDisplay, eglConfig, EGL_ALPHA_SIZE, &fbConfig->alphaBits);
    gl4es_egl_eglGetConfigAttrib(eglDisplay, eglConfig, EGL_DEPTH_SIZE, &fbConfig->depthBits);
    gl4es_egl_eglGetConfigAttrib(eglDisplay, eglConfig, EGL_STENCIL_SIZE, &fbConfig->stencilBits);
    gl4es_egl_eglGetConfigAttrib(eglDisplay, eglConfig, EGL_SAMPLES, &fbConfig->multiSampleSize);
    gl4es_egl_eglGetConfigAttrib(eglDisplay, eglConfig, EGL_SAMPLE_BUFFERS, &fbConfig->nMultiSampleBuffers);
    gl4es_egl_eglGetConfigAttrib(eglDisplay, eglConfig, EGL_SURFACE_TYPE, &tmp);
    ...
    gl4es_egl_eglGetConfigAttrib(eglDisplay, eglConfig, EGL_MAX_PBUFFER_WIDTH, &fbConfig->maxPbufferWidth);
    gl4es_egl_eglGetConfigAttrib(eglDisplay, eglConfig, EGL_MAX_PBUFFER_HEIGHT, &fbConfig->maxPbufferHeight);
    gl4es_egl_eglGetConfigAttrib(eglDisplay, eglConfig, EGL_MAX_PBUFFER_PIXELS, &fbConfig->maxPbufferPixels);
    gl4es_egl_eglGetConfigAttrib(eglDisplay, eglConfig, EGL_NATIVE_VISUAL_ID, &fbConfig->associatedVisualId);
    if(!fbConfig->associatedVisualId || globals4es.usefb || globals4es.usefbo || globals4es.usepbuffer) {
        ...
        XVisualInfo *visuals = XGetVisualInfo(display, VisualDepthMask|VisualClassMask, &xvinfo, &n);
        if (!n) {
            LOGD("MobileGlues-gl4es: Warning, fillGLXFBConfig: XGetVisualInfo gives 0 VisualInfo for %d depth and TrueColor class\n", glx_default_depth);
...
static void init_eglconfig(Display *display) {
    ...
    gl4es_egl_eglChooseConfig(eglDisplay, configAttribs, NULL, 0, &configsFound);
    EGLConfig allConfigs[configsFound];
    gl4es_egl_eglChooseConfig(eglDisplay, configAttribs, allConfigs, configsFound, &configsFound);
    maxEGLConfig = -1;
    EGLint confID;
    for(int i=0; i<configsFound; ++i) {
        gl4es_egl_eglGetConfigAttrib(eglDisplay, allConfigs[i], EGL_CONFIG_ID, &confID);
...
static int InitEGL(Display *display) {
    ...
    gl4es_egl_eglBindAPI(EGL_OPENGL_ES_API);
    EGLint result = gl4es_egl_eglInitialize(eglDisplay, NULL, NULL);
    if (result != EGL_TRUE) {
        CheckEGLErrors();
        LOGE("MobileGlues-gl4es: Unable to initialize EGL display.\n");
        return 0;
    }
    eglInitialized = true;
    return 1;
...
static void init_vsync() {
    ...
    if (fbdev < 0) {
        LOGE("MobileGlues-gl4es: Could not open /dev/fb0 for vsync.\n");
    }
#endif
}
    ...
    if (globals4es.stacktrace) {
        switch (sig) {
            ...
            case SIGSEGV: {
                ...
                if (! size) {
                    LOGD("MobileGlues-gl4es: No stacktrace. Compile with -funwind-tables.\n");
                } else {
                    LOGD("MobileGlues-gl4es: Stacktrace: %zd\n", size);
...
void glx_init() {
    ...
    init_liveinfo();
    if (sock>-1) {
        SHUT_LOGD("MobileGlues-gl4es: LiveInfo detected, fps will be shown\n");
    }
#endif
}
...
GLXContext gl4es_glXCreateContext(Display *display,
                            ...
                            Bool isDirect) {
    DBG(SHUT_LOGD("MobileGlues-gl4es: glXCreateContext(%p, %p, %p, %i) fbcontext_count=%d ", display, visual, shareList, isDirect, fbcontext_count);)
    ...
    if (eglDisplay == NULL || eglDisplay == EGL_NO_DISPLAY) {
        init_display(display);
        if (eglDisplay == EGL_NO_DISPLAY) {
            DBG(SHUT_LOGD("MobileGlues-gl4es:  => %p\n", NULL);)
            CheckEGLErrors();
            LOGE("MobileGlues-gl4es: Unable to create EGL display.\n");
            free(fake);
            return 0;
        }
    }
    ...
    if (eglInitialized == false) {
        if(!InitEGL(display)) {
            DBG(SHUT_LOGD("MobileGlues-gl4es:  => %p\n", NULL);)
            CheckEGLErrors();
            LOGE("MobileGlues-gl4es: Unable to init EGL.\n");
            free(fake);
            return 0;
        }
    }

	result = gl4es_egl_eglChooseConfig(eglDisplay, configAttribs, fake->eglConfigs, 64, &fake->eglConfigsCount);
    ...
    if (result != EGL_TRUE || fake->eglConfigsCount == 0) {
        DBG(SHUT_LOGD("MobileGlues-gl4es:  => %p\n", NULL);)
        LOGE("MobileGlues-gl4es: No EGL configs found (depth=%d, stencil=%d).\n", depthBits, glxfbconfig->stencilBits);
        CheckEGLErrors();
        free(fake);
        return 0;
    }
    EGLContext shared = (shareList)?shareList->eglContext:EGL_NO_CONTEXT;
	fake->eglContext = gl4es_egl_eglCreateContext(eglDisplay, fake->eglConfigs[fake->eglconfigIdx], shared, (hardext.esversion==1)?egl_context_attrib:egl_context_attrib_es2);
...
GLXContext createPBufferContext(Display *display, GLXContext shareList, GLXFBConfig config) {
    ...
    result = gl4es_egl_eglChooseConfig(eglDisplay, configAttribs, pbufConfigs, 1, &configsFound);

    CheckEGLErrors();
    if (result != EGL_TRUE || configsFound == 0) {
        LOGE("MobileGlues-gl4es: No EGL configs found.\n");
        return 0;
    }
	...
	fake->eglContext = gl4es_egl_eglCreateContext(eglDisplay, fake->eglConfigs[0], shared, (hardext.esversion==1)?egl_context_attrib:egl_context_attrib_es2);
    ...
    gl4es_egl_eglGetConfigAttrib(eglDisplay, fake->eglConfigs[0], EGL_RED_SIZE, &fake->rbits);
    gl4es_egl_eglGetConfigAttrib(eglDisplay, fake->eglConfigs[0], EGL_GREEN_SIZE, &fake->gbits);
    gl4es_egl_eglGetConfigAttrib(eglDisplay, fake->eglConfigs[0], EGL_BLUE_SIZE, &fake->bbits);
    gl4es_egl_eglGetConfigAttrib(eglDisplay, fake->eglConfigs[0], EGL_ALPHA_SIZE, &fake->abits);
    gl4es_egl_eglGetConfigAttrib(eglDisplay, fake->eglConfigs[0], EGL_DEPTH_SIZE, &fake->depth);
    gl4es_egl_eglGetConfigAttrib(eglDisplay, fake->eglConfigs[0], EGL_STENCIL_SIZE, &fake->stencil);
    gl4es_egl_eglGetConfigAttrib(eglDisplay, fake->eglConfigs[0], EGL_SAMPLES, &fake->samples);
    gl4es_egl_eglGetConfigAttrib(eglDisplay, fake->eglConfigs[0], EGL_SAMPLE_BUFFERS, &fake->samplebuffers);

    DBG(SHUT_LOGD("MobileGlues-gl4es:  => return PBufferContext %p (context->shared=%p)\n", fake, fake->shared);)
    return fake;
}
...
GLXContext gl4es_glXCreateContextAttribsARB(Display *display, GLXFBConfig config,
    ...
    DBG(SHUT_LOGD("MobileGlues-gl4es: glXCreateContextAttribsARB(%p, %p, %p, %d, %p) ", display, config, share_context, direct, attrib_list);
        if(config)
            SHUT_LOGD("MobileGlues-gl4es: config is RGBA:%d%d%d%d, depth=%d, stencil=%d, multisample=%d/%d doublebuff=%d, drawable=%d\n", config->redBits, config->greenBits, config->blueBits, config->alphaBits, config->depthBits, config->stencilBits, config->multiSampleSize, config->nMultiSampleBuffers, config->doubleBufferMode, config->drawableType); 
        else SHUT_LOGD("MobileGlues-gl4es: \n");
    )
    if(config && config->drawableType==GLX_PBUFFER_BIT) {
        ...
        gl4es_egl_eglGetConfigAttrib(eglDisplay, fake->eglConfigs[fake->eglconfigIdx], EGL_RED_SIZE, &fake->rbits);
        gl4es_egl_eglGetConfigAttrib(eglDisplay, fake->eglConfigs[fake->eglconfigIdx], EGL_GREEN_SIZE, &fake->gbits);
        gl4es_egl_eglGetConfigAttrib(eglDisplay, fake->eglConfigs[fake->eglconfigIdx], EGL_BLUE_SIZE, &fake->bbits);
        gl4es_egl_eglGetConfigAttrib(eglDisplay, fake->eglConfigs[fake->eglconfigIdx], EGL_ALPHA_SIZE, &fake->abits);
        gl4es_egl_eglGetConfigAttrib(eglDisplay, fake->eglConfigs[fake->eglconfigIdx], EGL_DEPTH_SIZE, &fake->depth);
        gl4es_egl_eglGetConfigAttrib(eglDisplay, fake->eglConfigs[fake->eglconfigIdx], EGL_STENCIL_SIZE, &fake->stencil);
        gl4es_egl_eglGetConfigAttrib(eglDisplay, fake->eglConfigs[fake->eglconfigIdx], EGL_SAMPLES, &fake->samples);
        gl4es_egl_eglGetConfigAttrib(eglDisplay, fake->eglConfigs[fake->eglconfigIdx], EGL_SAMPLE_BUFFERS, &fake->samplebuffers);
        gl4es_egl_eglGetConfigAttrib(eglDisplay, fake->eglConfigs[fake->eglconfigIdx], EGL_MIN_SWAP_INTERVAL, &minswap);
        gl4es_egl_eglGetConfigAttrib(eglDisplay, fake->eglConfigs[fake->eglconfigIdx], EGL_MAX_SWAP_INTERVAL, &maxswap);
        DBG(SHUT_LOGD("MobileGlues-gl4es:  => return %p (eglContext=%p, context->shared=%p)\n", fake, fake->eglContext, fake->shared);)
        return fake;
    }
}

void gl4es_glXDestroyContext(Display *display, GLXContext ctx) {
    DBG(SHUT_LOGD("MobileGlues-gl4es: glXDestroyContext(%p, %p), fbcontext_count=%d, ctx_type=%d\n", display, ctx, fbcontext_count, (ctx)?ctx->contextType:0);)
    if(globals4es.usefb)
        --fbcontext_count;
    if (ctx->eglContext) {
        ...
        if(eglSurface!=ctx->eglSurface || eglContext!=ctx->eglContext) {
            DBG(SHUT_LOGD("MobileGlues-gl4es:   temporary switch to eglSurface:%p(from %p), eglContext:%p(from %p)\n", ctx->eglSurface, eglSurface, ctx->eglContext, eglContext);)
#ifndef NO_GBM
            if(globals4es.usegbm)
                GBMMakeCurrent(eglDisplay, ctx->eglSurface, ctx->eglSurface, ctx->eglContext);
            else
#endif
                gl4es_egl_eglMakeCurrent(eglDisplay, ctx->eglSurface, ctx->eglSurface, ctx->eglContext);
        }
        ...
        // bind context back
        if(eglSurface!=ctx->eglSurface || eglContext!=ctx->eglContext) {
            DBG(SHUT_LOGD("MobileGlues-gl4es:   switch back to eglSurface:%p, eglContext:%p\n", eglSurface, eglContext);)
#ifndef NO_GBM
            if(globals4es.usegbm)
                GBMMakeCurrent(eglDisplay, eglContext?eglSurface:NULL, eglContext?eglSurface:NULL, eglContext);
            else
#endif
            gl4es_egl_eglMakeCurrent(eglDisplay, eglContext?eglSurface:NULL, eglContext?eglSurface:NULL, eglContext);
        }

        LOAD_EGL(eglDestroyContext);
        LOAD_EGL(eglDestroySurface);
        
		EGLBoolean result = gl4es_egl_eglDestroyContext(eglDisplay, ctx->eglContext);
        ctx->eglContext = 0;
        if (ctx->eglSurface != 0) {
            if(globals4es.usefb!=1 || !fbcontext_count) { // (This may cause troble on Pandora, has some driver doesn't seems to like to many Creation of the surface)
                ...
                if(destroySurf) {
					if(!globals4es.glxrecycle) {
						DBG(SHUT_LOGD("MobileGlues-gl4es:   egDestroySurface(%p, %p), drawable=%p\n", eglDisplay, ctx->eglSurface, (void*)ctx->drawable);)
						gl4es_egl_eglDestroySurface(eglDisplay, ctx->eglSurface);
        ...
        if (result != EGL_TRUE) {
            CheckEGLErrors();
            LOGE("MobileGlues-gl4es: Failed to destroy EGL context.\n");
        }
        /*if (fbdev >= 0) {
            close(fbdev);
            fbdev = -1;
...
Display *gl4es_glXGetCurrentDisplay() {
    DBG(SHUT_LOGD("MobileGlues-gl4es: glXGetCurrentDisplay()\n");)
...
XVisualInfo *gl4es_glXChooseVisual(Display *display,
                             int screen,
                             int *attributes) {
    DBG(SHUT_LOGD("MobileGlues-gl4es: glXChooseVisual(%p, %d, %p[", display, screen, attributes);)
    DBG(if(attributes) {for(int* a=attributes; *a!=0; ++a)SHUT_LOGD("MobileGlues-gl4es: %x,", *a);SHUT_LOGD("MobileGlues-gl4es: 0");})
    DBG(SHUT_LOGD("MobileGlues-gl4es: ])\n");)
    ...
    if (glx_default_depth != 16 && glx_default_depth != 24  && glx_default_depth != 32)
        LOGD("MobileGlues-gl4es: unusual desktop color depth %d\n", glx_default_depth);
...
*/  // this makes window of TokiTory transparent...
    ...
    XVisualInfo *visuals = XGetVisualInfo(display, VisualDepthMask|VisualClassMask, &xvinfo, &n);
    if (!n) {
        LOGD("MobileGlues-gl4es: Warning, gl4es_glXChooseVisual: XGetVisualInfo gives 0 VisualInfo for %d depth and TrueColor class\n", glx_default_depth);
        return NULL;
    }
    ...
    if(!count) {
        DBG(SHUT_LOGD("MobileGlues-gl4es: glXChooseVisual return %p (because no Config found)\n", NULL);)
        return NULL;
    }
    AddFBVisual(visuals, confs);

    DBG(SHUT_LOGD("MobileGlues-gl4es: glXChooseVisual return %p\n", visuals);)
    return visuals;
}
...
Bool gl4es_glXMakeCurrent(Display *display,
    ...
    if(context && glxContext==context && context->drawable==drawable) {
        DBG(SHUT_LOGD("MobileGlues-gl4es:  => True\n");)
        //same context, all is done bye
        DBG(SHUT_LOGD("MobileGlues-gl4es: Same context and drawable, doing nothing\n");)
        return true;
    }
    if (glxContext && glxContext->drawable==drawable && glxContext->eglSurface && !context->eglSurface && 
        ...
        DBG(SHUT_LOGD("MobileGlues-gl4es: Same drawable and compatible context: sharing everything...\n");)
    ...
    if(context && glxContext && context->drawable==drawable && context->eglSurface==eglSurface) {
        ...
        DBG(SHUT_LOGD("MobileGlues-gl4es: Same Surface and Drawable, doing (almost) nothing\n");)
        return true;
    }
        ...
        if(context->drawable==drawable && context->eglSurface) {
            ...
            if(created) {
                ...
                if(globals4es.usepbuffer) {
                    ...
                    DBG(SHUT_LOGD("MobileGlues-gl4es: XGetGeometry gives %dx%d for drawable %p\n", width, height, (void*)drawable);)
                } else if((globals4es.usefb) || globals4es.usegbm) {
                    // Get size of desktop
                    Screen *screen = DefaultScreenOfDisplay(display);
                    width = WidthOfScreen(screen);
                    height = HeightOfScreen(screen);
                    DBG(SHUT_LOGD("MobileGlues-gl4es: X11 gives a size of desktop %dx%d for drawable %p\n", width, height, (void*)drawable);)
                }
                if(globals4es.usepbuffer) {
                    ...
                    if(createPBuffer(display, egl_attribs, &eglSurf, &eglCtx, &eglConfig, (depth>16)?8:5, (depth==15)?5:(depth>16)?8:6, (depth>16)?8:5, (depth==32)?8:0, context->samplebuffers, context->samples)==0) {
                        // fail too, abort
                        SHUT_LOGE("MobileGlues-gl4es: PBuffer creation failed too\n");
                        return 0;
                    }
                    int Width, Height;

                    gl4es_egl_eglQuerySurface(eglDisplay,eglSurf,EGL_WIDTH,&Width);
                    gl4es_egl_eglQuerySurface(eglDisplay,eglSurf,EGL_HEIGHT,&Height);
                    DBG(SHUT_LOGD("MobileGlues-gl4es: New surface %p is %dx%d\n", eglSurf, Width, Height);)
                    ...
                    if(context->eglContext && context->eglContext!=eglCtx) {
                        ...
                        LOAD_EGL(eglDestroyContext);
                        DBG(SHUT_LOGD("MobileGlues-gl4es: Remove old Cotnext %p, new is %p\n", context->eglContext, eglCtx);)
                        gl4es_egl_eglDestroyContext(eglDisplay, context->eglContext);
                    }
                    context->eglContext = eglCtx;
...
#endif
                {
                    if(globals4es.usefb || globals4es.usefbo || globals4es.usegbm) {
                        if(eglSurface) // cannot create multiple eglSurface for the same Framebuffer?
                            ...
                            if(eglSurf == EGL_NO_SURFACE) {
                                ...
                                if(globals4es.usegbm) {
                                    ...
                                    eglSurf = gl4es_egl_eglCreatePlatformWindowSurface(eglDisplay, context->eglConfigs[context->eglconfigIdx], context->nativewin, attrib_list);
                                } else
#endif
                                eglSurf = gl4es_egl_eglCreateWindowSurface(eglDisplay, context->eglConfigs[context->eglconfigIdx], (EGLNativeWindowType)context->nativewin, attrib_list);
                            } else {
                                DBG(SHUT_LOGD("MobileGlues-gl4es: LIBGL: EglSurf Recycled\n");)
                            }
                            eglSurface = context->eglSurface = eglSurf;
                            if(!eglSurf) {
                                DBG(SHUT_LOGD("MobileGlues-gl4es: LIBGL: Warning, EglSurf is null\n");)
                                CheckEGLErrors();
                            }
                        }
                    } else {
                        if(context->eglSurface) {
                            ...
                            if(destroySurf) {
                                if(!globals4es.glxrecycle) {
									gl4es_egl_eglDestroySurface(eglDisplay, context->eglSurface);
                        ...
                        if(eglSurf == EGL_NO_SURFACE) {
                            eglSurf = context->eglSurface = gl4es_egl_eglCreateWindowSurface(eglDisplay, context->eglConfigs[0], drawable, attrib_list);
                        } else {
                            DBG(SHUT_LOGD("MobileGlues-gl4es: LIBGL: eglSurf Recycled\n");)
                            context->eglSurface = eglSurf;
                        }
...
#endif
        result = gl4es_egl_eglMakeCurrent(eglDisplay, eglSurf, eglSurf, eglCtx);
    DBG(SHUT_LOGD("MobileGlues-gl4es: LIBGL: eglMakeCurrent(%p, %p, %p, %p)\n", eglDisplay, eglSurf, eglSurf, eglCtx);)
    CheckEGLErrors();
    glxContext = context;
    if(!result) {
        // error switching context, don't change glstate and abort...
        DBG(SHUT_LOGD("MobileGlues-gl4es:  => False\n");)
        return false;
    } else {
        eglSurface = eglSurf;
        eglContext = eglCtx;
    ...
    if (context) {
        ...
        if (result) {
            if (globals4es.usefbo && !created) {
                ...
                gl4es_egl_eglQuerySurface(eglDisplay,eglSurf,EGL_WIDTH,&g_width);
                gl4es_egl_eglQuerySurface(eglDisplay,eglSurf,EGL_HEIGHT,&g_height);
                int fbo_width, fbo_height;
                if(GL4ES_GetEnvVarFmt("LIBGL_FBO","%dx%d",&fbo_width, &fbo_height)==2) {
                    SHUT_LOGD("MobileGlues-gl4es: Forcing FBO size %dx%d (%dx%d)\n", fbo_width, fbo_height, g_width, g_height);
                    g_width = fbo_width; 
                    g_height = fbo_height;
                }
                // create the main_fbo...
            ...
            gl4es_restoreCurrentFBO();
             ...
             // finished
            DBG(SHUT_LOGD("MobileGlues-gl4es:  => True (glstate=%p)\n", context?context->glstate:NULL);)
            return true;
        }
        DBG(SHUT_LOGD("MobileGlues-gl4es:  => False\n");)
        return false;
...
Bool gl4es_glXMakeContextCurrent(Display *display, int drawable,
    ...
    DBG(SHUT_LOGD("MobileGlues-gl4es: glXMakeContextCurrent(%p, %X, %X, %p)\n", display, drawable, readable, context);)
...
void gl4es_glXSwapBuffers(Display *display,
                    GLXDrawable drawable) {
    static int frames = 0;
    DBG(SHUT_LOGD("MobileGlues-gl4es: \rglXSwapBuffers(%p, %p) ", display, (void*)drawable);)
...
#define TARGET_FPS 1000000000LL/61
...
#endif
        DBG(SHUT_LOGD("MobileGlues-gl4es: blitMainFBO(%d, %d, %u, %u)\n", x, y, width, height);)
    ...
    if(PBuffer && glstate->emulatedPixmap) {
        ...
        gl4es_egl_eglSwapBuffers(eglDisplay, surface);
...
#endif
    {
        ...
        if (frame == 1) {
            ...
            if (last_frame < now) {
                ...
                if (globals4es.showfps) LOGD("MobileGlues-gl4es: fps: %.2f, avg: %.2f\n", fps, avg);
#ifdef PANDORA
                if (sock>-1) {
                    char tmp[60];
...
int gl4es_glXGetConfig(Display *display,
                 ...
                 int attribute,
                 int *value) {
    DBG(SHUT_LOGD("MobileGlues-gl4es: glXGetConfig(%p, %p, 0x%x, %p)\n", display, visual, attribute, value);)
...
int gl4es_glXQueryContext( Display *dpy, GLXContext ctx, int attribute, int *value ) {
    DBG(SHUT_LOGD("MobileGlues-gl4es: glXQueryContext(%p, %p, %d, %p)\n", dpy, ctx, attribute, value);)
	*value=0;
...
GLXContext gl4es_glXGetCurrentContext() {
    DBG(SHUT_LOGD("MobileGlues-gl4es: glXGetCurrentContext()\n");)
...
GLXFBConfig * fillGLXFBConfig(EGLConfig *eglConfigs, int count, int withDB, Display *display) {
    ...
    for (int j=0; j<count; ++j) {
        ...
        gl4es_egl_eglGetConfigAttrib(eglDisplay, eglConfigs[i], EGL_CONFIG_ID, &confID);
...
GLXFBConfig *gl4es_glXChooseFBConfig(Display *display, int screen,
    ...
    DBG(SHUT_LOGD("MobileGlues-gl4es: glXChooseFBConfig(%p, %d, %p, %p)\n", display, screen, attrib_list, count);)
    ...
    *count = 1;
		...
		while(attrib_list[i]!=0) {
			switch(attrib_list[i++]) {
				case GLX_RED_SIZE:
                    ...
                    cr = cur;
                    attr[cur++] = tmp;
                    DBG(SHUT_LOGD("MobileGlues-gl4es: FBConfig redBits=%d\n", tmp);)
					break;
				case GLX_GREEN_SIZE:
					tmp = attrib_list[i++];
                    ...
                    cg = cur;
                    attr[cur++] = tmp;
                    DBG(SHUT_LOGD("MobileGlues-gl4es: FBConfig greenBits=%d\n", tmp);)
					break;
				case GLX_BLUE_SIZE:
					tmp = attrib_list[i++];
                    ...
                    cb = cur;
                    attr[cur++] = tmp;
                    DBG(SHUT_LOGD("MobileGlues-gl4es: FBConfig blueBits=%d\n", tmp);)
					break;
				case GLX_ALPHA_SIZE:
					tmp = attrib_list[i++];
                    ...
                    ca = cur;
                    attr[cur++] = tmp;
                    DBG(SHUT_LOGD("MobileGlues-gl4es: FBConfig alphaBits=%d\n", tmp);)
					break;
                case GLX_DEPTH_SIZE:
					tmp = attrib_list[i++];
                    attr[cur++] = EGL_DEPTH_SIZE;
                    attr[cur++] = tmp;
                    DBG(SHUT_LOGD("MobileGlues-gl4es: FBConfig depthBits=%d\n", tmp);)
					break;
                case GLX_STENCIL_SIZE:
					tmp = attrib_list[i++];
                    attr[cur++] = EGL_STENCIL_SIZE;
                    attr[cur++] = tmp;
                    DBG(SHUT_LOGD("MobileGlues-gl4es: FBConfig stencilBits=%d\n", tmp);)
					break;
                case GLX_DRAWABLE_TYPE:
                    tmp = attrib_list[i++];
                    ...
                    if(tmp&GLX_PBUFFER_BIT)
                        attr[1] |= EGL_PIXMAP_BIT;
                    drawable_set = 1;
                    DBG(SHUT_LOGD("MobileGlues-gl4es: FBConfig drawableType=0x%X\n", tmp);)
                    break;
                case GLX_SAMPLE_BUFFERS:
                    tmp = attrib_list[i++];
                    attr[cur++] = EGL_SAMPLE_BUFFERS;
                    attr[cur++] = tmp;
                    DBG(SHUT_LOGD("MobileGlues-gl4es: FBConfig multisampleBuffers=%d\n", tmp);)
                    break;
                case GLX_SAMPLES:
                    tmp = attrib_list[i++];
                    attr[cur++] = EGL_SAMPLES;
                    attr[cur++] = (tmp<0)?0:tmp;
                    DBG(SHUT_LOGD("MobileGlues-gl4es: FBConfig multiSampleSize=%d\n", tmp);)
                    break;
                case GLX_DOUBLEBUFFER:
                    if(attrib_list[i]==0 || attrib_list[i]==1)
                        ...
                        doublebuffer = 1;
                    DBG(SHUT_LOGD("MobileGlues-gl4es: FBConfig doubleBufferMode=%d\n", doublebuffer);)
                    break;
                case GLX_X_RENDERABLE:
                    ++i; //value ignored
                    DBG(SHUT_LOGD("MobileGlues-gl4es: FBConfig renderable=%d\n", attrib_list[i-1]);)
                    break;
                case GLX_LEVEL:
                    tmp = attrib_list[i++];
                    attr[cur++] = EGL_LEVEL;
                    attr[cur++] = tmp;
                    DBG(SHUT_LOGD("MobileGlues-gl4es: FBConfig level=%d\n", tmp);)
                    break;
                case GLX_VISUAL_ID:
                    tmp = attrib_list[i++];
                    attr[cur++] = EGL_NATIVE_VISUAL_ID;
                    attr[cur++] = tmp;
                    DBG(SHUT_LOGD("MobileGlues-gl4es: FBConfig visual id=%d\n", tmp);)
                    break;
                case GLX_FBCONFIG_ID:
                    glxconfig = attrib_list[i++];
                    DBG(SHUT_LOGD("MobileGlues-gl4es: GLXFBConfigID=%d\n", glxconfig);)
                    break;
                case GLX_X_VISUAL_TYPE:
                    tmp = attrib_list[i++];
                    ...
                    DBG(SHUT_LOGD("MobileGlues-gl4es: FBConfig visual type=%d\n", tmp);)
                    break;
                default:
                    ++i;
    ...
    if (eglInitialized == false) {
        if(!InitEGL((globals4es.usefb || globals4es.usepbuffer)?g_display:display)) {
            CheckEGLErrors();
            LOGE("MobileGlues-gl4es: Unable to initialize EGL.\n");
            return NULL;
            *count = 0;
        }
    }
    LOAD_EGL(eglChooseConfig);
    if(glxconfig==-1) {
        gl4es_egl_eglChooseConfig(eglDisplay, attr, NULL, 0, count);
        if((*count==0) && (globals4es.usepbuffer)) {
                DBG(SHUT_LOGD("MobileGlues-gl4es: glXChooseFBConfig found 0 config with PixMap, trying again with PBuffer\n");)
        ...
        if((*count==0) && (!globals4es.usepbuffer) && ca && attr[ca+1]) {
                DBG(SHUT_LOGD("MobileGlues-gl4es: glXChooseFBConfig found 0 config with an Alpha channel, trying without\n");)
                attr[ca] = 0;
                gl4es_egl_eglChooseConfig(eglDisplay, attr, NULL, 0, count);
        }
        if((*count==0) && (!globals4es.usepbuffer) && ca && attr[ca+1] && (attr[cr]>5 || attr[cg]>5 || attr[cb]>5)) {
                DBG(SHUT_LOGD("MobileGlues-gl4es: glXChooseFBConfig found 0 config without an Alpha channel, but 8bits rgb, trying lowering bitness\n");)
                attr[cr] = attr[cg] = attr[cb] = 5;
                gl4es_egl_eglChooseConfig(eglDisplay, attr, NULL, 0, count);
        }
#ifdef PANDORA
        if((*count==0) && (!globals4es.usepbuffer) && (attr[cr]>5 || attr[cg]>5 || attr[cb]>5)) {
                DBG(SHUT_LOGD("MobileGlues-gl4es: glXChooseFBConfig found 0 config with 8bits rgb, trying lowering bitness\n");)
                attr[cr] = attr[cg] = attr[cb] = 5;
                gl4es_egl_eglChooseConfig(eglDisplay, attr, NULL, 0, count);
        }
#endif
        /*if((*count==0) && (vt) && (attr[vt]!=-1)) {
            DBG(SHUT_LOGD("MobileGlues-gl4es: glXChooseFBConfig found 0 config with VisualType, trying without\n");)
            attr[vt] = -1;  //EGL_DONT_CARE
            gl4es_egl_eglChooseConfig(eglDisplay, attr, NULL, 0, count);
        }*/
        if(*count==0) {  // NO Config found....
            DBG(SHUT_LOGD("MobileGlues-gl4es: glXChooseFBConfig found 0 config\n");)
            return NULL;
        }
    } else {
        *count = 1;
    ...
    if(glxconfig==-1)
        gl4es_egl_eglChooseConfig(eglDisplay, attr, eglConfigs, *count, count);
    else {
    ...
    free(eglConfigs);
    DBG(
        SHUT_LOGD("MobileGlues-gl4es: glXChooseFBConfig found %d config\n", *count);
        for(int i=0; i<*count; ++i)
            SHUT_LOGD("MobileGlues-gl4es:  config[%d] = %p\n", i, configs[i]);
    )
...
GLXFBConfig *gl4es_glXGetFBConfigs(Display *display, int screen, int *count) {
    DBG(SHUT_LOGD("MobileGlues-gl4es: glXGetFBConfigs(%p, %d, %p)\n", display, screen, count);)
#ifdef NO_EGL
    ...
    *count *= 2;
    free(eglConfigs);
    DBG(SHUT_LOGD("MobileGlues-gl4es: glXGetFBConfig found %d config\n", *count);)
#endif
    return configs;
}

int gl4es_glXGetFBConfigAttrib(Display *display, GLXFBConfig config, int attribute, int *value) {
    DBG(SHUT_LOGD("MobileGlues-gl4es: glXGetFBConfigAttrib(%p, %p, 0x%04X, %p)", display, config, attribute, value);)
    if(!config) {
        return get_config_default(display, attribute, value);
    }
...
XVisualInfo *gl4es_glXGetVisualFromFBConfig(Display *display, GLXFBConfig config) {
    DBG(SHUT_LOGD("MobileGlues-gl4es: glXGetVisualFromFBConfig(%p, %p)\n", display, config);)
    /*if (g_display == NULL) {
    ...
    XVisualInfo *visuals = XGetVisualInfo(display, VisualDepthMask|VisualClassMask, &xvinfo, &n);
    if (!n) {
        LOGD("MobileGlues-gl4es: Warning, gl4es_glXGetVisualFromFBConfig: XGetVisualInfo gives 0 VisualInfo for %d depth and TrueColor class\n", glx_default_depth);
        return NULL;
    }
    return visuals;
}
...
GLXContext gl4es_glXCreateNewContext(Display *display, GLXFBConfig config,
                               ...
                               Bool is_direct) {
    DBG(SHUT_LOGD("MobileGlues-gl4es: glXCreateNewContext(%p, %p, %d, %p, %i), drawableType=0x%02X\n", display, config, render_type, share_list, is_direct, (config)?config->drawableType:0);)
...
void gl4es_glXSwapInterval(int interval) {
    DBG(SHUT_LOGD("MobileGlues-gl4es: glXSwapInterval(%i)\n", interval);)
#ifdef NOEGL
    // nothing
#elif defined(NOX11)
    LOAD_EGL(eglSwapInterval);
    gl4es_egl_eglSwapInterval(eglDisplay, swapinterval);
#elif defined(USE_FBIO)
    if (! globals4es.vsync) {
        static int warned = 0;
        if(!warned) {
            LOGD("MobileGlues-gl4es: Enable LIBGL_VSYNC=1 if you want to use vsync.\n");
            warned++;
        }
    }
    ...
    if(glxContext) {
        if(globals4es.usepbuffer) {
            DBG(SHUT_LOGD("MobileGlues-gl4es: Ignoring glXSwapInterval(%d) on PBuffer surface\n", interval);)
            swapinterval = 0;
        } else {
            LOAD_EGL(eglSwapInterval);
            gl4es_egl_eglSwapInterval(eglDisplay, swapinterval);
            ...
            if(interval<minswap || interval>maxswap) {
                SHUT_LOGE("MobileGlues-gl4es: Warning, Swap Interval %d is out of possible values %d, %d\n", interval, minswap, maxswap);
            } else
                swapinterval = interval;
        }
    } else {
        DBG(SHUT_LOGD("MobileGlues-gl4es: LIBGL: glXSwapInterval called before Context is current.\n");)
        swapinterval = interval;
    }
#endif
}
...
void gl4es_glXCopyContext(Display *display, GLXContext src, GLXContext dst, GLuint mask) {
    DBG(SHUT_LOGD("MobileGlues-gl4es: glXCopyContext(%p, %p, %p, %04X)\n", display, src, dst, mask);)
...
Window gl4es_glXCreateWindow(Display *display, GLXFBConfig config, Window win, int *attrib_list) {
    ...
    DBG(SHUT_LOGD("MobileGlues-gl4es: glXCreateWindow(%p, %p, %p, %p)\n", display, config, (void*)win, attrib_list);)
    return win;
}
void gl4es_glXDestroyWindow(Display *display, void *win) {
    // really wants a GLXWindow
    DBG(SHUT_LOGD("MobileGlues-gl4es: glXDestroyWindow(%p, %p)\n", display, win);)
} 

GLXDrawable gl4es_glXGetCurrentDrawable() {
    DBG(SHUT_LOGD("MobileGlues-gl4es: glXGetCurrentDrawable()\n");)
	if (glxContext) 
...
Bool gl4es_glXIsDirect(Display * display, GLXContext ctx) {
    DBG(SHUT_LOGD("MobileGlues-gl4es: glXIsDirect(%p, %p)\n", display, ctx);)
    return true;
}

void gl4es_glXUseXFont(Font font, int first, int count, int listBase) {
    DBG(SHUT_LOGD("MobileGlues-gl4es: glXUseXFont(%p, %d, %d, %d)\n", (void*)font, first, count, listBase);)
    ...
    if (!fs) {
      LOGE("MobileGlues-gl4es: error, no font set before call to glXUseFont\n");
      return;
    }
...
int gl4es_glXQueryDrawable(Display *dpy, GLXDrawable draw, int attribute, unsigned int *value) {
    DBG(SHUT_LOGD("MobileGlues-gl4es: glXQueryDrawable(%p, %p", dpy, (void*)draw);)
    ...
    *value = 0;
    ...
    if((attribute==GLX_WIDTH || attribute==GLX_HEIGHT)) {
        ...
        if(surf!=EGL_NO_SURFACE) {
            ...
            if(attribute==GLX_WIDTH)
                if(gl4es_egl_eglQuerySurface(eglDisplay, surf, EGL_WIDTH, &tmp)==EGL_TRUE)
                    width = tmp;
            if(attribute==GLX_HEIGHT)
                if(gl4es_egl_eglQuerySurface(eglDisplay, surf, EGL_HEIGHT, &tmp)==EGL_TRUE)
                    height = tmp;
        } 
#ifndef NO_EGL
        else {
    ...
    switch(attribute) {
        case GLX_WIDTH:
            *value = width;
            DBG(SHUT_LOGD("MobileGlues-gl4es: (%d), GLX_WIDTH, %p = %d)\n", pbuf, value, *value);)
            return 1;
        case GLX_HEIGHT:
            *value = height;
            DBG(SHUT_LOGD("MobileGlues-gl4es: (%d), GLX_HEIGHT, %p = %d)\n", pbuf, value, *value);)
            return 1;
        case GLX_PRESERVED_CONTENTS:
            if(pbuf) *value = 1;
            DBG(SHUT_LOGD("MobileGlues-gl4es: (%d), GLX_PRESERVED_CONTENTS, %p = %d)\n", pbuf, value, *value);)
            return 1;
        case GLX_LARGEST_PBUFFER:
            if(pbuf) *value = 0;
            DBG(SHUT_LOGD("MobileGlues-gl4es: (%d), GLX_LARGEST_PBUFFER, %p = %d)\n", pbuf, value, *value);)
            return 1;
        case GLX_FBCONFIG_ID:
            *value = 0;
            DBG(SHUT_LOGD("MobileGlues-gl4es: (%d), GLX_FBCONFIG_ID, %p = %d)\n", pbuf, value, *value);)
            return 1;
        case GLX_SWAP_INTERVAL_EXT:
            *value = swapinterval;
            DBG(SHUT_LOGD("MobileGlues-gl4es: (%d), GLX_SWAP_INTERVAL_EXT, %p = %d)\n", pbuf, value, *value);)
            return 1;
        case GLX_MAX_SWAP_INTERVAL_EXT:
            *value = maxswap; // fake, should eglQuery the Config for EGL_MAX_SWAP_INTERVAL (and EGL_MIN_SWAP_INTERVAL)
            DBG(SHUT_LOGD("MobileGlues-gl4es: (%d), GLX_MAX_SWAP_INTERVAL_EXT, %p = %d)\n", pbuf, value, *value);)
            return 1;
    }
    DBG(SHUT_LOGD("MobileGlues-gl4es: (%d), %04x, %p)\n", pbuf, attribute, value);)
    return 0;
}
...
{
    LOAD_EGL(eglDestroyContext);
    gl4es_egl_eglDestroyContext(eglDisplay, pbuffersize[j].Context);
    CheckEGLErrors();
    // should pack, but I think it's useless for common use 
}

void gl4es_glXDestroyPbuffer(Display * dpy, GLXPbuffer pbuf) {
    DBG(SHUT_LOGD("MobileGlues-gl4es: glxDestroyPBuffer(%p, %p)\n", dpy, (void*)pbuf);)
    LOAD_EGL(eglDestroySurface);
    int j=0;
    while(j<pbufferlist_size && pbufferlist[j]!=pbuf) j++;
    if(j==pbufferlist_size) {
        DBG(SHUT_LOGD("MobileGlues-gl4es: PBuff not found in pbufferlist\n");)
        return;
    }
        // delete de Surface
    EGLSurface surface = (EGLSurface)pbufferlist[j];
    gl4es_egl_eglDestroySurface(eglDisplay, surface);
    CheckEGLErrors();
...
int createPBuffer(Display * dpy, const EGLint * egl_attribs, EGLSurface* Surface, EGLContext* Context, EGLConfig *Config, int redBits, int greenBits, int blueBits, int alphaBits, int samplebuffers, int samples) {
    ...
    result = gl4es_egl_eglChooseConfig(eglDisplay, configAttribs, Config, 1, &configsFound);

    CheckEGLErrors();
    if (result != EGL_TRUE || configsFound == 0) {
        LOGD("MobileGlues-gl4es: No EGL configs found.\n");
        return 0;
    }

	// now, create the PBufferSurface
    (*Surface) = gl4es_egl_eglCreatePbufferSurface(eglDisplay, Config[0], egl_attribs);

    if((*Surface)==EGL_NO_SURFACE) {
        CheckEGLErrors();
        LOGD("MobileGlues-gl4es: Error creating PBuffer\n");
        return 0;
    }
    (*Context) = gl4es_egl_eglCreateContext(eglDisplay, Config[0], EGL_NO_CONTEXT, (hardext.esversion==1)?egl_context_attrib:egl_context_attrib_es2);
    CheckEGLErrors();
...
GLXPbuffer gl4es_glXCreatePbuffer(Display * dpy, GLXFBConfig config, const int * attrib_list) {
    DBG(SHUT_LOGD("MobileGlues-gl4es: glXCreatePbuffer(%p, %p, %p)\n", dpy, config, attrib_list);)
    LOAD_EGL(eglQuerySurface);
    ...
    gl4es_egl_eglQuerySurface(eglDisplay,Surface,EGL_WIDTH,&Width);
    gl4es_egl_eglQuerySurface(eglDisplay,Surface,EGL_HEIGHT,&Height);
...
int createPixBuffer(Display * dpy, int bpp, const EGLint * egl_attribs, NativePixmapType nativepixmap, EGLSurface* Surface, EGLContext* Context) {
    ...
    result = gl4es_egl_eglChooseConfig(eglDisplay, configAttribs, pixbufConfigs, 1, &configsFound);
    ...
    (*Surface) = gl4es_egl_eglCreatePixmapSurface(eglDisplay, pixbufConfigs[0], nativepixmap,egl_attribs);

    if((*Surface)==EGL_NO_SURFACE) {
        CheckEGLErrors();
        LOGE("MobileGlues-gl4es: Error creating PixmapSurface\n");
        return 0;
    }

    (*Context) = gl4es_egl_eglCreateContext(eglDisplay, pixbufConfigs[0], EGL_NO_CONTEXT, (hardext.esversion==1)?egl_context_attrib:egl_context_attrib_es2);
    CheckEGLErrors();
...
GLXPixmap gl4es_glXCreateGLXPixmap(Display *display, XVisualInfo * visual, Pixmap pixmap) {
    DBG(SHUT_LOGD("MobileGlues-gl4es: glXCreateGLXPixmap(%p, %p, %p)\n", display, visual, (void*)pixmap);)
    LOAD_EGL(eglQuerySurface);
    ...
    if(globals4es.usefb || createPixBuffer(display, depth, NULL, (NativePixmapType)pixmap, &Surface, &Context)==0) {
        ...
        SHUT_LOGE("MobileGlues-gl4es: Pixmap creation failed, trying PBuffer instead\n");
...
GLXPixmap gl4es_glXCreatePixmap(Display * dpy, GLXFBConfig config, Pixmap pixmap, const int * attrib_list) {
    DBG(SHUT_LOGD("MobileGlues-gl4es: glXCreatePixmap(%p, %p, %p, %p)\n", dpy, config, (void*)pixmap, attrib_list);)
    // Check that the config is for PBuffer
...
void gl4es_glXDestroyGLXPixmap(Display *display, void *pixmap) {
    DBG(SHUT_LOGD("MobileGlues-gl4es: glXDestroyGLXPixmap(%p, %p)\n", display, pixmap);)
    LOAD_EGL(eglDestroySurface);
    int j=0;
...
void gl4es_glXDestroyPixmap(Display *display, void *pixmap) {
    DBG(SHUT_LOGD("MobileGlues-gl4es: glXDestroyPixmap(%p, %p)\n", display, pixmap);)
...
const int sbuf = Width * Height * (Depth==16?2:4);
    ...
    if (tmp) {
        if(reverse) {
            ...
            GL4ES_pixel_convert(tmp, (void**)&pix, Width, Height, GL_BGRA, GL_UNSIGNED_BYTE, GL_RGB, GL_UNSIGNED_SHORT_5_6_5, 0, glstate->texture.unpack_align);
    } else
#endif
        if(reverse) {
...
void BlitEmulatedPixmap(int win) {
    ...
    if(reverse) {
        ...
        if(width!=Width || height!=Height /*|| depth!=Depth*/) {
            ...
            DBG(SHUT_LOGD("MobileGlues-gl4es: LIBGL: Recreate PBuffer %dx%dx%d => %dx%dx%d\n", Width, Height, Depth, width, height, depth);)
...
#endif
                gl4es_egl_eglMakeCurrent(eglDisplay, NULL, NULL, EGL_NO_CONTEXT);
            gl4es_egl_eglDestroySurface(eglDisplay, buff->Surface);
            CheckEGLErrors();

            EGLSurface Surface = NULL;
            Surface = gl4es_egl_eglCreatePbufferSurface(eglDisplay, buff->Config, egl_attribs);
            if(Surface==EGL_NO_SURFACE)
                LOGE("MobileGlues-gl4es: Warning, Recration of pbuffer failed (from %dx%dx%d => %dx%dx%d)\n", buff->Width, buff->Height, buff->Depth, width, height, depth);
            CheckEGLErrors();
#ifndef NO_GBM
            if(globals4es.usegbm)
...
#endif
                gl4es_egl_eglMakeCurrent(eglDisplay, Surface, Surface, buff->Context);
            CheckEGLErrors();
    ...
    if(direct_copy) {
        ...
        if(!gl4es_egl_eglCopyBuffers(eglDisplay, buff->Surface, (EGLNativePixmapType)frame)) {
            LOGE("MobileGlues-gl4es: Cannot use eglCopyBuffers, disabling it's use: ");
            CheckEGLErrors();
            direct_copy = 0;
        } else
            return;
    ...
    if(hardext.esversion==1) {
        if(Depth==16) {
            ...
            gl4es_gles_glReadPixels(0, 0, Width, Height, GL_BGRA, GL_UNSIGNED_BYTE, tmp);
        } else {
            gl4es_gles_glReadPixels(0, 0, Width, Height, GL_BGRA, GL_UNSIGNED_BYTE, (void*)pix);
        }
    } else 
    if(Depth==16)
        gl4es_gles_glReadPixels(0, 0, Width, Height, GL_RGB, GL_UNSIGNED_SHORT_5_6_5, (void*)pix);
    else
#endif
...
GLXContext gl4es_glXCreateContextAttribs(Display *dpy, GLXFBConfig config, GLXContext share_context, Bool direct, const int *attrib_list) {
    DBG(SHUT_LOGD("MobileGlues-gl4es: glXCreateContextAttribs(%p, %p, %p, %d, %p)\n", dpy, config, share_context, direct, attrib_list);)
    int ask_es = 0;
    int ask_shaders = 0;
    ...
    while (attr && (*attr)!= 0) {
        ...
        switch(name) {
            ...
            case GLX_CONTEXT_PROFILE_MASK_ARB:
                ...
                break;
            default: {
                DBG(SHUT_LOGD("MobileGlues-gl4es:  unknown Attrib %04X (value=%d)\n", name, pair);)
            }
        }   
    }
    if(majver*10+minver != 0) {
        DBG(SHUT_LOGD("MobileGlues-gl4es:  Required context version %d.%d\n", majver, minver);)
        if(majver*10+minver>21) {
            LOGE("MobileGlues-gl4es: Asked for unsupported context version %d.%d\n", majver, minver);
            return 0;
        }
        if(majver*10+minver>globals4es.gl) {
            LOGE("MobileGlues-gl4es: Asked for unsupported context version %d.%d (max version is %d.%d)\n", majver, minver, globals4es.gl/10, globals4es.gl%10);
            return 0;
        }
        if((mask&GLX_CONTEXT_ES2_PROFILE_BIT_EXT) && hardext.esversion<2) {
            LOGE("MobileGlues-gl4es: Asked for ES2 compatible context on GLES1.1 Backend\n");
            return 0;
        }
    }
...
void refreshMainFBO()
{
    ...
    if(eglSurface!=EGL_NO_SURFACE) {
        LOAD_EGL(eglQuerySurface);
        EGLint tmp;
        if(gl4es_egl_eglQuerySurface(eglDisplay, eglSurface, EGL_WIDTH, &tmp)==EGL_TRUE)
            glstate->fbo.mainfbo_width = tmp;
        if(gl4es_egl_eglQuerySurface(eglDisplay, eglSurface, EGL_HEIGHT, &tmp)==EGL_TRUE)
...
const char *gl4es_glXQueryExtensionsString(Display *display, int screen) {
    DBG(SHUT_LOGD("MobileGlues-gl4es: glXQueryExtensionString(%p, %d)\n", display, screen);)
    static const char *basic_extensions = 
        "GLX_ARB_create_context "
        "GLX_ARB_create_context_profile "
        "GLX_ARB_get_gl4es_proc_address "
        "GLX_ARB_multisample "
...
const char *gl4es_glXQueryServerString(Display *display, int screen, int name) {
    DBG(SHUT_LOGD("MobileGlues-gl4es: glXQueryServerString(%p, %d, %d)\n", display, screen, name);)
    switch (name) {
...
Bool gl4es_glXQueryExtension(Display *display, int *errorBase, int *eventBase) {
    DBG(SHUT_LOGD("MobileGlues-gl4es: glXQueryExtension(%p, %p, %p)\n", display, errorBase, eventBase);)
    if (errorBase)
        *errorBase = 0;
...
Bool gl4es_glXQueryVersion(Display *display, int *major, int *minor) {
    DBG(SHUT_LOGD("MobileGlues-gl4es: glXQueryVersion(%p, %p, %p)\n", display, major, minor);)
...
const char *gl4es_glXGetClientString(Display *display, int name) {
    DBG(SHUT_LOGD("MobileGlues-gl4es: glXGetClientString(%p, %d)\n", display, name);)
    switch (name) {
        case GLX_VENDOR: return "ptitSeb";
        case GLX_VERSION: return "1.4 GL4ES";

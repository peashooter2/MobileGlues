...
void Streaming_Initialize() {
    ...
    glTexBindStreamIMG =(PFNGLTEXBINDSTREAMIMGPROC*)gl4es_egl_eglGetProcAddress("glTexBindStreamIMG");
    glGetTexAttrIMG = (PFNGLGETTEXSTREAMDEVICEATTRIBUTEIVIMGPROC*)gl4es_egl_eglGetProcAddress("glGetTexStreamDeviceAttributeivIMG");
    glGetTexDeviceIMG = (PFNGLGETTEXSTREAMDEVICENAMEIMGPROC*)gl4es_egl_eglGetProcAddress("glGetTexStreamDeviceNameIMG");
...
int alloc_buff(int buff, int width, int height) {
	...
	if (ioctl(bc_cat[buff], BCIOREQ_BUFFERS, &buf_param) != 0) {
		SHUT_LOGD("MobileGlues-gl4es: LIBGL: BCIOREQ_BUFFERS failed\n");
		return 0;
	}
	if (ioctl(bc_cat[buff], BCIOGET_BUFFERCOUNT, &ioctl_var) != 0) {
		SHUT_LOGD("MobileGlues-gl4es: LIBGL: BCIOREQ_BUFFERCOUNT failed\n");
		return 0;
	}
	if (ioctl_var.output == 0) {
		SHUT_LOGD("MobileGlues-gl4es: LIBGL: Streaming, no texture buffer available\n");
		return 0;
	}
	const char *bcdev = glGetTexDeviceIMG(buff);
	if (!bcdev) {
		SHUT_LOGD("MobileGlues-gl4es: LIBGL: problem with getting the GL_IMG_texture_stream device\n");
		return 0;
	} else {
		bcdev_w = width;
		bcdev_h = height;
		...
		SHUT_LOGD("MobileGlues-gl4es: LIBGL: Streaming device = %s num: %d, width: %d, height: %d, format: 0x%x\n",
			bcdev, bcdev_n, bcdev_w, bcdev_h, bcdev_fmt);
		if (bcdev_w!=width) {
			SHUT_LOGD("MobileGlues-gl4es: LIBGL: Streaming not activate, buffer width != asked width\n");
			return 0;
		}
	}
/*	LOAD_GLES(glTexParameterf);
    gl4es_gles_glTexParameterf(GL_TEXTURE_STREAM_IMG, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
    gl4es_gles_glTexParameterf(GL_TEXTURE_STREAM_IMG, GL_TEXTURE_MAG_FILTER, GL_LINEAR);*/
	
	ioctl_var.input = 0;
	if (ioctl(bc_cat[buff], BCIOGET_BUFFERPHYADDR, &ioctl_var) != 0) {
		SHUT_LOGD("MobileGlues-gl4es: LIBGL: BCIOGET_BUFFERADDR failed\n");
		return 0;
	} else {
		...
		if (buf_vaddr[buff] == MAP_FAILED) {
			SHUT_LOGD("MobileGlues-gl4es: LIBGL: mmap failed\n");
			return 0;
		}
	}
...
//SHUT_LOGD("MobileGlues-gl4es: InitStreamingCache\n");
	if (streaming_inited)
		return gl_streaming;
	Streaming_Initialize();
	for (int i=0; i<10; i++) {
...
//SHUT_LOGD("MobileGlues-gl4es: GetStreamingBuffer(%i)\n", buff);
	if (!gl_streaming)
		return NULL;
	if ((buff<0) || (buff>9))
		return NULL;
...
//SHUT_LOGD("MobileGlues-gl4es: AddStreamed(%i, %i, %u)\n", width, height, ID);
	if (!gl_streaming)
		return -1;
	static int i =0;
    int j=0;
...
//SHUT_LOGD("MobileGlues-gl4es: FreeStreamed(%i)", ID);
	if (!gl_streaming)
		return;
	if ((ID<0) || (ID>9))
		return;
...
//SHUT_LOGD("MobileGlues-gl4es: FindTexID(%u)\n", ID);
	if (!gl_streaming)
		return -1;
	int i =0;
	while (i<10) {
...
//SHUT_LOGD("MobileGlues-gl4es: ApplyFilterID(%i, 0x%04X, 0x%04X)\n", ID, min_filter, mag_filter);
	if (!gl_streaming)
		return;
	if ((ID<0) || (ID>9))
		return;
...
//SHUT_LOGD("MobileGlues-gl4es: ActivateStreaming(%i)\n", ID);
    LOAD_GLES(glEnable);
	if (!gl_streaming)
		return;
	if ((ID<0) || (ID>9))
...
//SHUT_LOGD("MobileGlues-gl4es: DeactivateStreaming()\n");
    LOAD_GLES(glDisable);
	if (!gl_streaming)
		return;
